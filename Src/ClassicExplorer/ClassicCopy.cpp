...
bool CClassicCopyFile::Run(HWND hWnd, IAccessible* pAcc, CString* pLog)
{
	...
	if (!m_YesButton.first || m_YesButton.second != CHILDID_SELF || !m_NoButton.first || m_NoButton.second !=
		CHILDID_SELF || !m_Cancel.first)
	...
	int dlg = pTlsData->bCopyMultiFile
		          ? (IsLanguageRTL() ? IDD_FILEMULTIR : IDD_FILEMULTI)
		          : (IsLanguageRTL() ? IDD_FILER : IDD_FILE);
	...
	else if (res == IDNO)
	{
		...
		PumpMessages();
		// messages need to be pumped after every accessibility action. otherwise the next action doesn't work
...
void CClassicCopyFile::GetFileInfo(IAccessible* pAcc, bool bSrc)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		switch (i)
		{
		case 2: if (wcslen(name) < _countof(fname)) wcscpy_s(fname, name);
			break;
		case 3: if (wcslen(name) < _countof(dir)) wcscpy_s(dir, name);
			break;
		case 4: size = name;
			break;
		case 5: date = name;
...
INT_PTR CALLBACK CClassicCopyFile::DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_INITDIALOG)
	{
		...
		if (pThis->m_bSystem)
		{
			Sprintf(text,_countof(text),
			        FindTranslation(L"Copy.SubtitleSys", L"This folder already contains a system file named '%s'."),
			        pThis->m_FileName);
		...
		{
			Sprintf(text,_countof(text),
			        FindTranslation(L"Copy.SubtitleRO", L"This folder already contains a read-only file named '%s'."),
			        pThis->m_FileName);
			...
			Sprintf(text,_countof(text),
			        FindTranslation(L"Copy.Subtitle", L"This folder already contains a file named '%s'."),
			        pThis->m_FileName);
		...
		SetDlgItemText(hwndDlg,IDC_STATICPROMPT1,
		               FindTranslation(L"Copy.Prompt1", L"Do you want to replace the existing file:"));
...
bool CClassicCopyFolder::Run(HWND hWnd)
{
	...
	// pick the correct dialog template (for single and multiple files, for LTR and RTL)
	int dlg = bMulti
		          ? (IsLanguageRTL() ? IDD_FOLDERMULTIR : IDD_FOLDERMULTI)
		          : (IsLanguageRTL() ? IDD_FOLDERR : IDD_FOLDER);
...
INT_PTR CALLBACK CClassicCopyFolder::DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_INITDIALOG)
	{
		...
		Strcat(text,_countof(text),
		       FindTranslation(L"Folder.Prompt", L"Do you still want to move or copy the folder?"));
		SetDlgItemText(hwndDlg,IDC_STATICFNAME, text);
...
static LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
                                   DWORD_PTR dwRefData)
{
...
#pragma warning(suppress:4996)
								...
								fwprintf(f, L"version = %d.%d.%d - %d.%d\r\n\r\n", ver.dwMajorVersion,
								         ver.dwMinorVersion, ver.dwBuildNumber, ver.wServicePackMajor,
