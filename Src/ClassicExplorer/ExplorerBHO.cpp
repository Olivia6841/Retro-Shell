...
LRESULT CALLBACK CExplorerBHO::SubclassTreeParentProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
...
LRESULT CALLBACK CExplorerBHO::SubclassTreeProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
		...
		if (pidl)
		{
			...
			if (pThis->m_pBrowser && SUCCEEDED(pThis->m_pBrowser->QueryActiveShellView(&pView)))
			{
				...
				if (pView2 && SUCCEEDED(pView2->GetFolder(IID_IPersistFolder2,(void**)&pFolder)) && SUCCEEDED(
					pFolder->GetCurFolder(&pidl2)) && pidl2)
				{
	...
	if (uMsg == TVM_SETEXTENDEDSTYLE && wParam == (TVS_EX_FADEINOUTEXPANDOS | TVS_EX_AUTOHSCROLL | 0x80000000) && lParam
		== 0)
	{
		wParam &= 0x7FFFFFFF;
...
LRESULT CALLBACK CExplorerBHO::HookExplorer(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (nCode == HCBT_CREATEWND)
	{
		...
		else if (pCreate->lpcs->lpszClass > (LPTSTR)0xFFFF && _wcsicmp(pCreate->lpcs->lpszClass, L"DUIViewWndClassName")
			== 0)
		{
	...
	if (m_pBrowser && SUCCEEDED(m_pBrowser->QueryActiveShellView(&pView)))
	{
		...
		if (pView2 && SUCCEEDED(pView2->GetFolder(IID_IPersistFolder2,(void**)&pFolder)))
		{
			...
			if (bShowTip && SUCCEEDED(pView2->ItemCount(SVGIO_SELECTION,&count)) && count == 1)
			{
				...
				PITEMID_CHILD child;
				if (SUCCEEDED(pView2->Items(SVGIO_SELECTION,IID_IEnumIDList,(void**)&pEnum)) && pEnum && pEnum->Next(
					1, &child,NULL) == S_OK)
				{
					...
					if (pFolder2)
					{
						...
						if (SUCCEEDED(
							pFolder2->GetUIObjectOf(NULL,1,(PCUITEMID_CHILD*)&child,IID_IQueryInfo,NULL,(void**)&
								pQueryInfo)))
						{
							CComString pTip;
			...
			if (!res)
			{
				...
				if (!oldText)
				{
					...
					{
						fmt = (count == 1
							       ? FindTranslation(L"Status.ItemSelected", L"%s item selected")
							       : FindTranslation(L"Status.ItemsSelected", L"%s items selected"));
					}
					else if (SUCCEEDED(pView2->ItemCount(SVGIO_ALLVIEW,&count)) && count >= 0)
					{
						fmt = (count == 1
							       ? FindTranslation(L"Status.Item", L"%s item")
							       : FindTranslation(L"Status.Items", L"%s items"));
					}
					else
						buf2[0] = 0;
					if (fmt)
				...
				if (bShowSpace)
				{
					...
					if (SUCCEEDED(pFolder->GetCurFolder(&pidl)) && SHGetPathFromIDList(pidl, path) &&
						GetDiskFreeSpaceEx(path,NULL,NULL, &diskSize))
					{
						wchar_t str[100];
						StrFormatByteSize64(diskSize.QuadPart, str,_countof(str));
						Sprintf(buf, size, FindTranslation(L"Status.FreeSpace", L"%s (Disk free space: %s)"),
						        oldText ? oldText : buf2, str);
						res = true;
					}
					else if (!oldText)
			...
			pFolder->GetCurFolder(&pidl)))
		{
			...
			if (SUCCEEDED(pView2->ItemCount(SVGIO_ALLVIEW,&count)))
			{
				if ((FAILED(pView2->ItemCount(SVGIO_SELECTION,&selCount)) || selCount == 0))
					type = SVGIO_ALLVIEW;
				CComPtr<IEnumIDList> pEnum;
				if ((count < 10000 || selCount < 1000) && SUCCEEDED(pView2->Items(type,IID_IEnumIDList,(void**)&pEnum))
					&& pEnum)
				{
					PITEMID_CHILD child;
...
LRESULT CALLBACK CExplorerBHO::SubclassStatusProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                  UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	wchar_t buf[1024];
	if (uMsg == WM_CLEAR)
	...
	if (uMsg == SB_SETTEXT && LOWORD(wParam) == 0)
	{
		...
		if (((CExplorerBHO*)uIdSubclass)->GetStatusText(buf,_countof(buf), (wchar_t*)lParam,
		                                                (dwRefData & SHOW_INFOTIP) != 0,
		                                                (dwRefData & SHOW_FREE_SPACE) != 0))
			lParam = (LPARAM)buf;
	}
...
LRESULT CALLBACK CExplorerBHO::SubclassStatusProc8(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                   UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	wchar_t buf[1024];
	if (uMsg == WM_CLEAR)
	{
		...
		if (!((CExplorerBHO*)uIdSubclass)->GetStatusText(buf,_countof(buf),NULL, (dwRefData & SHOW_INFOTIP) != 0,
		                                                 (dwRefData & SHOW_FREE_SPACE) != 0))
			buf[0] = 0;
...
LRESULT CALLBACK CExplorerBHO::SubclassDUIViewProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                   UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	CExplorerBHO* pThis = (CExplorerBHO*)uIdSubclass;
	if (uMsg == WM_WINDOWPOSCHANGING && pThis->m_Status8)
	{
		...
		if (!(pPos->flags & SWP_NOSIZE))
		{
			...
			int parts[] = {
				pPos->cx - height - pThis->m_FileSizeWidth - pThis->m_ZoneWidth, pPos->cx - height - pThis->m_ZoneWidth,
				-1
			};
...
LRESULT CALLBACK CExplorerBHO::SubclassRebarProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                 UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	if (pThis->m_bRemapBands)
	{
		...
		if (uMsg == RB_GETBANDBORDERS || uMsg == RB_GETBANDINFO || uMsg == RB_GETRECT || uMsg == RB_SETBANDINFO || uMsg
			== RB_SETBANDWIDTH)
		{
...
LRESULT CALLBACK CExplorerBHO::SubclassBreadcrumbProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                      UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	CExplorerBHO* pThis = (CExplorerBHO*)uIdSubclass;
	if (*pThis->m_CurPath)
	{
		...
		if (uMsg == WM_PAINT)
		{
			...
			if (hdcPaint)
			{
				...
				int iconSize = GetSystemMetrics(SM_CXSMICON);
				if (pThis->m_CurIcon)
					DrawIconEx(hdcPaint, rc.left + 3, (rc.top + rc.bottom - iconSize) / 2, pThis->m_CurIcon, iconSize,
					           iconSize, 0,NULL,DI_NORMAL);
				rc.left += iconSize + 8;
				// Not a good idea to hard-code number of pixels, but seems to work fine for different DPI settings
				...
				if (theme && SUCCEEDED(DwmIsCompositionEnabled(&dwm)) && dwm)
				{
					...
					DrawThemeTextEx(theme, hdcPaint, 0, 0, pThis->m_CurPath, -1,
					                DT_NOPREFIX | DT_VCENTER | DT_SINGLELINE, &rc, &opts);
				}
				else
				{
...
LRESULT CALLBACK CExplorerBHO::SubclassProgressProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                    UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	CExplorerBHO* pThis = (CExplorerBHO*)uIdSubclass;
	...
	if (uMsg == WM_COMMAND && (HWND)lParam == pThis->m_ComboBox.m_hWnd)
	{
		if (HIWORD(wParam) == CBN_DROPDOWN)
		{
			...
			item.iItem = -1;
			for (std::vector<ComboItem>::const_iterator it = pThis->m_ComboItems.begin(); it != pThis->m_ComboItems.
			     end(); ++it)
			{
				...
				SHFILEINFO info;
				if (SHGetFileInfo((LPCTSTR)it->pidl, 0, &info, sizeof(info),
	...
	if (uMsg == pThis->m_NavigateMsg)
	{
		...
		if (wParam == 0 && pThis->m_NavigatePidl && !(pThis->m_CurPidl && ILIsEqual(
			pThis->m_NavigatePidl, pThis->m_CurPidl)))
...
HRESULT STDMETHODCALLTYPE CExplorerBHO::SetSite(IUnknown* pUnkSite)
{
	...
	if (pUnkSite)
	{
		...
		if (pProvider)
		{
			...
			if (m_pWebBrowser)
			{
				if (DispEvent1::m_dwEventCookie == 0xFEFEFEFE)
				...
				if (regKey.QueryDWORDValue(L"ShowedToolbar", val) != ERROR_SUCCESS || !val)
				{
					...
					if (GetSettingBool(L"NoInitialToolbar") || SUCCEEDED(
						m_pWebBrowser->ShowBrowserBar(&name,&show,NULL)))
			...
			if (m_pBrowser && SUCCEEDED(m_pBrowser->GetControlWindow(FCW_STATUS,&status)))
			{
				...
				if (rebar && m_UpButtonIndex)
				{
					...
					if (bDef)
						size = rc.bottom;
					m_Toolbar.Create(rebar,NULL,NULL,
					                 WS_CHILD | TBSTYLE_TOOLTIPS | TBSTYLE_FLAT | TBSTYLE_CUSTOMERASE | CCS_NODIVIDER |
					...
					{
						m_IconNormal = (HICON)LoadImage(g_Instance,MAKEINTRESOURCE(IDI_UP2NORMAL),IMAGE_ICON, size,
						                                size,LR_DEFAULTCOLOR);
						m_IconHot = (HICON)LoadImage(g_Instance,MAKEINTRESOURCE(IDI_UP2HOT),IMAGE_ICON, size, size,
						                             LR_DEFAULTCOLOR);
						m_IconPressed = (HICON)LoadImage(g_Instance,MAKEINTRESOURCE(IDI_UP2PRESSED),IMAGE_ICON, size,
						                                 size,LR_DEFAULTCOLOR);
						m_IconDisabled = (HICON)LoadImage(g_Instance,MAKEINTRESOURCE(IDI_UP2DISABLED),IMAGE_ICON, size,
						                                  size,LR_DEFAULTCOLOR);
					}
					...
					REBARBANDINFO info = {
						sizeof(info),
						RBBIM_CHILD | RBBIM_ID | RBBIM_CHILDSIZE | RBBIM_IDEALSIZE | RBBIM_SIZE | RBBIM_STYLE
					};
				...
				{
					DWORD flags = (GetSettingBool(L"ShowInfoTip") ? SHOW_INFOTIP : 0) | (
						GetSettingBool(L"ShowFreeSpace") ? SHOW_FREE_SPACE : 0);
					if (bWin8)
					{
						RECT rc;
						GetWindowRect(status, &rc);
						m_Status8 = CreateWindow(STATUSCLASSNAME, NULL,
						                         WS_CHILD|WS_VISIBLE|SBARS_SIZEGRIP|SBARS_TOOLTIPS, 0, 0,
						                         rc.right-rc.left, rc.bottom-rc.top, GetParent(status), NULL,
						                         g_Instance, NULL);
						HDC hdc = CreateCompatibleDC(NULL);
						HGDIOBJ font0 = SelectObject(hdc, (HFONT)SendMessage(m_Status8,WM_GETFONT, 0, 0));

						HFONT font = CreateFontSetting(GetSettingString(L"StatusBarFont"),
						...
						if (GetSettingBool(L"ShowZone"))
						{
							...
							m_pSecurityManager.CoCreateInstance(CLSID_InternetSecurityManager,NULL,
							...
							if (m_pZoneManager && m_pSecurityManager)
							{
								for (DWORD zone = URLZONE_LOCAL_MACHINE; zone <= URLZONE_UNTRUSTED; zone++)
								{
									...
									{
										if (GetTextExtentPoint32(hdc, attributes.szDisplayName,
										                         Strlen(attributes.szDisplayName),
										                         &size) && m_ZoneWidth < size.cx)
											m_ZoneWidth = size.cx;
									}
								}
								m_ZoneWidth += 50;
	...
	else
	{
		...
		if (m_IconNormal) DestroyIcon(m_IconNormal);
		m_IconNormal = NULL;
		if (m_IconHot) DestroyIcon(m_IconHot);
		m_IconHot = NULL;
		if (m_IconPressed) DestroyIcon(m_IconPressed);
		m_IconPressed = NULL;
				...
				if (pFolder && SUCCEEDED(pFolder->GetCurFolder(&m_CurPidl)) && m_CurPidl)
				{
					...
					if (m_bNoBreadcrumbs)
					{
						...
						if (SUCCEEDED(SHGetNameFromIDList(m_CurPidl,SIGDN_DESKTOPABSOLUTEEDITING,&pPath)))
							...
							Strcpy(m_CurPath,_countof(m_CurPath), pPath);
						// just in case DESKTOPABSOLUTE fails let's try the FILESYSPATH. probably not needed
						SHFILEINFO info;
						if (SUCCEEDED(
							SHGetFileInfo((LPCTSTR)(PIDLIST_ABSOLUTE)m_CurPidl,0,&info,sizeof(info),SHGFI_ICON|
