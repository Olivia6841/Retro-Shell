...
LRESULT CALLBACK CBandWindow::ToolbarSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
...
LRESULT CBandWindow::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	// create the toolbar
	if (bLabels && !bList)
		m_Toolbar = CreateWindow(TOOLBARCLASSNAME, L"",
		                         WS_CHILD|TBSTYLE_TOOLTIPS|TBSTYLE_FLAT|CCS_NODIVIDER|CCS_NOPARENTALIGN|CCS_NORESIZE, 0,
		                         0, 10, 10, m_hWnd, (HMENU)101, g_Instance, NULL);
	else
		m_Toolbar = CreateWindow(TOOLBARCLASSNAME, L"",
		                         WS_CHILD|TBSTYLE_TOOLTIPS|TBSTYLE_FLAT|TBSTYLE_LIST|CCS_NODIVIDER|CCS_NOPARENTALIGN|
		                         CCS_NORESIZE, 0, 0, 10, 10, m_hWnd, (HMENU)101, g_Instance, NULL);

	m_Toolbar.SendMessage(
		TB_SETEXTENDEDSTYLE, 0,TBSTYLE_EX_MIXEDBUTTONS | TBSTYLE_EX_DRAWDDARROWS | TBSTYLE_EX_HIDECLIPPEDBUTTONS);
	...
	m_ImgEnabled = ImageList_Create(iconSize, iconSize,ILC_COLOR32 | ILC_MASK | (IsLanguageRTL() ? ILC_MIRROR : 0), 0,
	                                mainCount);
	m_ImgHover = ImageList_Create(iconSize, iconSize, ILC_COLOR32 | ILC_MASK | (IsLanguageRTL() ? ILC_MIRROR : 0), 0,
	                              mainCount);
	m_ImgDisabled = ImageList_Create(iconSize, iconSize,ILC_COLOR32 | ILC_MASK | (IsLanguageRTL() ? ILC_MIRROR : 0), 0,
	                                 mainCount);
	...
	for (int i = 0; i < mainCount; i++)
	{
		...
		else
		{
			...
			if (item.submenu || bFolder)
				button.fsStyle |= (item.id < ID_CUSTOM || item.command || (item.link && item.submenu))
					                  ? BTNS_DROPDOWN
					                  : BTNS_WHOLEDROPDOWN;
		}
	}
...
{
	if (m_ImgEnabled) ImageList_Destroy(m_ImgEnabled);
	m_ImgEnabled = NULL;
	if (m_ImgDisabled) ImageList_Destroy(m_ImgDisabled);
	m_ImgDisabled = NULL;
	if (m_ImgHover) ImageList_Destroy(m_ImgHover);
	m_ImgHover = NULL;
	...
	if (regSettings.Open(HKEY_CURRENT_USER, GetSettingsRegPath()) == ERROR_SUCCESS)
	{
		...
		for (int idx = 0; idx < (int)m_Items.size(); idx++)
		{
			...
			if (!m_Items[idx].regName.IsEmpty())
			{
				...
				if (bMain)
				{
					if (bDisabled != m_Items[idx].bDisabled) m_Toolbar.SendMessage(
						TB_ENABLEBUTTON, idx + 1, bDisabled ? 0 : 1);
					if (bChecked != m_Items[idx].bChecked) m_Toolbar.SendMessage(
						TB_CHECKBUTTON, idx + 1, bChecked ? 1 : 0);
				}
...
static bool GetPidlPath(PIDLIST_ABSOLUTE pidl, wchar_t* path)
{
	...
	if (GetWinVersion() >= WIN_VER_WIN7)
	{
		...
		if (SUCCEEDED(SHCreateItemFromIDList(pidl,IID_IShellItem,(void**)&pShellItem)))
		{
			...
			if (SUCCEEDED(pLibrary.CoCreateInstance(CLSID_ShellLibrary)) && SUCCEEDED(
				pLibrary->LoadLibraryFromItem(pShellItem,STGM_READ)))
			{
				pShellItem = NULL;
				if (SUCCEEDED(pLibrary->GetDefaultSaveFolder(DSFT_DETECT,IID_IShellItem,(void**)&pShellItem)) &&
					pShellItem)
				{
					CComString pPath;
		...
		while (pEnum && pEnum->Next(1, &child,NULL) == S_OK)
		{
			...
			if (SUCCEEDED(pFolder->GetDisplayNameOf(child,SHGDN_INFOLDER|SHGDN_FORPARSING,&str)))
			{
				...
				if (std::find(items.begin(), items.end(), hash) == items.end())
				{
					...
					pView->SelectItem(
						child, SVSI_SELECT | SVSI_EDIT | SVSI_DESELECTOTHERS | SVSI_ENSUREVISIBLE | SVSI_FOCUSED);
					break;
				}
			}
			ILFree(child);
...
void CBandWindow::ExecuteCustomCommand(const wchar_t* pCommand)
{
	...
	if (SUCCEEDED(m_pBrowser->QueryActiveShellView(&pView)))
	{
		...
		CAbsolutePidl pidl;
		CComQIPtr<IFolderView> pView2(pView);
		if (pView2 && SUCCEEDED(pView2->GetFolder(IID_IPersistFolder2,(void**)&pFolder)) && SUCCEEDED(
			pFolder->GetCurFolder(&pidl)))
		{
			...
			if (bArg2)
			{
				...
				if (SUCCEEDED(pView2->ItemCount(SVGIO_SELECTION,&count)) && count == 1 && SUCCEEDED(
					pView2->Items(SVGIO_SELECTION,IID_IEnumIDList,(void**)&pEnum)) && pEnum)
				{
					PITEMID_CHILD child;
		...
		if (bArg1 || bArg2 || bArg3 || bArg4 || bArg5)
		{
			// expand %1, %2, %3, %4, %5
			DWORD_PTR args[100] = {
				(DWORD_PTR)path, (DWORD_PTR)file, (DWORD_PTR)input, (DWORD_PTR)input, (DWORD_PTR)output
			};
			FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_ARGUMENT_ARRAY | FORMAT_MESSAGE_FROM_STRING,
			              buf, 0, 0, (LPWSTR)&pBuf, 0, (va_list*)args);
		}
...
LRESULT CBandWindow::OnCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (id == ID_NAVPANE)
	{
		if (m_pBrowserBag)
		{
			...
			bool bNavPane = SUCCEEDED(m_pBrowserBag->Read(g_NavPaneVisible,&val,NULL)) && val.vt == VT_BOOL && val.
				boolVal;
			VariantClear(&val);
			val.vt = VT_BOOL;
	...
	if (id == ID_DETAILSPANE)
	{
		...
		{
			VARIANT val = {VT_EMPTY};
			bool bNavPane = SUCCEEDED(m_pBrowserBag->Read(g_DetailsPaneVisible,&val,NULL)) && val.vt == VT_BOOL && val.
				boolVal;
			VariantClear(&val);
			val.vt = VT_BOOL;
			val.boolVal = bNavPane ? VARIANT_FALSE : VARIANT_TRUE;
	...
	if (id == ID_PREVIEWPANE)
	{
		...
		{
			VARIANT val = {VT_EMPTY};
			bool bNavPane = SUCCEEDED(m_pBrowserBag->Read(g_PreviewPaneVisible,&val,NULL)) && val.vt == VT_BOOL && val.
				boolVal;
			VariantClear(&val);
			val.vt = VT_BOOL;
			val.boolVal = bNavPane ? VARIANT_FALSE : VARIANT_TRUE;
...
{
public:
	CMenuCallback(IShellBrowser* pBrowser)
	{
		m_bExecuted = false;
		m_pBrowser = pBrowser;
	}
	...
	case SMC_SFEXEC:
		{
			...
			if (SUCCEEDED(psmd->psf->GetAttributesOf(1,&psmd->pidlItem,&flags)))
			{
				...
				if (flags & SFGAO_LINK)
				{
					...
					if (SUCCEEDED(psmd->psf->GetUIObjectOf(NULL,1,&psmd->pidlItem,IID_IShellLink,NULL,(void**)&pLink))
						&& pLink)
...
LRESULT CBandWindow::OnDropDown(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	...
	{
		RECT rc;
		if (m_Toolbar.SendMessage(TB_GETITEMRECT, idx, (LPARAM)&rc) && memcmp(&rc, &pButton->rcButton, sizeof(RECT)) ==
			0)
		{
			pItem = &*it;
			break;
	...
	if (pItem && (pItem->submenu || pItem->link))
	{
		if (pItem->submenu)
		{
			...
			m_Toolbar.ClientToScreen(&params.rcExclude);
			// must not use MapWindowPoints because it produces wrong results in RTL cases
			HMENU menu = CreateDropMenu(pItem->submenu);
			int res = TrackPopupMenuEx(menu,TPM_RETURNCMD | TPM_VERTICAL, params.rcExclude.left,
			                           params.rcExclude.bottom, m_hWnd, &params);
			DestroyMenu(menu);
			if (res > 0)
		...
		else if (pItem->link)
		{
			...
			if (pFolder)
			{
				...
				if (pMenu)
				{
					...
					hr = pMenu->Initialize(&callback, -1,ANCESTORDEFAULT,
					                       SMINIT_TOPLEVEL | SMINIT_VERTICAL | SMINIT_RESTRICT_DRAGDROP);
					if (SUCCEEDED(hr))
					{
						CRegKey cRegOrder;
						...
						hr = pMenu->SetShellFolder(pFolder, pidl, cRegOrder.m_hKey,SMSET_BOTTOM | 0x00000008);
						// SMSET_USEBKICONEXTRACTION=0x00000008
						if (SUCCEEDED(hr))
						{
							cRegOrder.Detach();
							POINTL ptl = {params.rcExclude.left, params.rcExclude.bottom};
							RECTL rcl = {
								params.rcExclude.left, params.rcExclude.top, params.rcExclude.right,
...
LRESULT CBandWindow::OnChevron(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	...
	if (::SendMessage(pnmh->hwndFrom,RB_GETBANDINFO, pChevron->uBand, (LPARAM)&info) && info.hwndChild == m_Toolbar.
		m_hWnd)
	{
		RECT clientRect;
		...
		int res = TrackPopupMenuEx(menu,TPM_RETURNCMD | TPM_VERTICAL, params.rcExclude.left, params.rcExclude.bottom,
		                           m_hWnd, &params);
		DestroyMenu(menu);
			...
			if (m_MenuIconSize > 0)
			{
				if (pItem->iconPath)
				{
					if (_wcsicmp(pItem->iconPath, L"NONE") != 0)
					{
						...
						if (!hIcon)
							hIcon = (HICON)LoadImage(hShell32,MAKEINTRESOURCE(1),IMAGE_ICON, m_MenuIconSize,
							                         m_MenuIconSize,LR_DEFAULTCOLOR);
						if (hIcon)
						{
							HICON hIcon2 = pItem->iconPathD
								               ? LoadIcon(m_MenuIconSize, pItem->iconPathD, modules)
								               : NULL;
							if (!hIcon2)
...
void CBandWindow::UpdateToolbar(void)
{
	...
	if (m_pBrowser)
	{
		...
		if (pView)
		{
			...
			if (pView2)
			{
				...
				if (m_pBrowserBag)
				{
					...
					{
						VARIANT val = {VT_EMPTY};
						if (SUCCEEDED(m_pBrowserBag->Read(g_ComboPaneEnabled,&val,NULL)) && val.vt == VT_BOOL && !val.
							boolVal)
						{
					...
					{
						VARIANT val = {VT_EMPTY};
						if (SUCCEEDED(m_pBrowserBag->Read(g_DetailsPaneEnabled,&val,NULL)) && val.vt == VT_BOOL && !val.
							boolVal)
						{
							bDisableDetailsPane = true;
							bDetailsPane = false;
						...
						VariantClear(&val);
						if (SUCCEEDED(m_pBrowserBag->Read(g_PreviewPaneEnabled,&val,NULL)) && val.vt == VT_BOOL && !val.
							boolVal)
						{
							bDisablePreviewPane = true;
							bPreviewPane = false;
		...
		if (m_pBrowserBag)
		{
			...
			{
				VARIANT val = {VT_EMPTY};
				bNavPane = SUCCEEDED(m_pBrowserBag->Read(g_NavPaneVisible,&val,NULL)) && val.vt == VT_BOOL && val.
					boolVal;
				VariantClear(&val);
			}
			if (!bDisableDetailsPane)
			{
				VARIANT val = {VT_EMPTY};
				bDetailsPane = SUCCEEDED(m_pBrowserBag->Read(g_DetailsPaneVisible,&val,NULL)) && val.vt == VT_BOOL &&
					val.boolVal;
				VariantClear(&val);
			}
			if (!bDisablePreviewPane)
			{
				VARIANT val = {VT_EMPTY};
				bPreviewPane = SUCCEEDED(m_pBrowserBag->Read(g_PreviewPaneVisible,&val,NULL)) && val.vt == VT_BOOL &&
					val.boolVal;
				VariantClear(&val);
			}
		}
...
bool CBandWindow::HasFolderSettings(void) const
{
	for (int idx = 0; idx < (int)m_Items.size(); idx++)
	{
		if (m_Items[idx].id == ID_SHOW_EXTENSIONS || m_Items[idx].id == ID_HIDDEN_FILES || m_Items[idx].id ==
			ID_SYSTEM_FILES)
			return true;
	}
	return false;
...
LRESULT CALLBACK CExplorerBand::RebarSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                  UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
...
LRESULT CALLBACK CExplorerBand::ParentSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                   UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
...
STDMETHODIMP CExplorerBand::SetSite(IUnknown* pUnkSite)
{
	...
	else
	{
		...
		if (m_TopWindow)
			RemoveProp(m_TopWindow, g_LoadedSettingsAtom);
		m_TopWindow = NULL;
	}
	return S_OK;
}
