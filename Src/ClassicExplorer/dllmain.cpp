...
{
	{
		g_bLogLevel = false;
		CRegKey regKey;
		if (regKey.Open(HKEY_CURRENT_USER, L"Software\\OpenShell\\ClassicExplorer\\Settings",
...
extern "C" BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	if (dwReason == DLL_PROCESS_ATTACH)
	{
		...
		if (GetWinVersion() <= WIN_VER_WIN7)
		{
			...
			bReplaceUI = (GetSettingBool(settings[SETTING_REPLACE_FILE]) ||
				GetSettingBool(settings[SETTING_REPLACE_FOLDER]) || GetSettingBool(settings[SETTING_MORE]));
		}

		settings[SETTING_FILE_EXPLORER].LoadValue(regSettings, regSettingsUser, regPolicy, regPolicyUser);
		if (_wcsicmp(exe, L"regsvr32.exe") != 0 && _wcsicmp(exe, L"msiexec.exe") != 0 && _wcsicmp(
			exe, L"ClassicExplorerSettings.exe") != 0 && !g_bExplorerExe)
		{
			// some arbitrary app
			...
			if ((!GetSettingBool(settings[SETTING_SHARE_OVERLAY]) || GetSettingBool(settings[SETTING_SHARE_EXPLORER]))
				&& (!bReplaceUI || GetSettingBool(settings[SETTING_FILE_EXPLORER])))
				return FALSE;
