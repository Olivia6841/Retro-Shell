...
LRESULT CALLBACK CClassicIEBHO::SubclassStatusProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                   UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	if (uMsg == SB_SETPARTS && wParam > 0)
	{
		...
		else
		{
			...
			rc.top += 1;
			rc.bottom -= 1;
			SetWindowPos(pThis->m_ProgressBar,NULL, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top,
	...
	if (uMsg == WM_LBUTTONDBLCLK)
	{
		...
		if (PtInRect(&rc, pt))
		{
			...
			{
				wchar_t buf[1024];
				Sprintf(buf,_countof(buf), L"zone %u %s", (unsigned)(uintptr_t)GetAncestor(hWnd,GA_ROOT),
				        (const wchar_t*)url);
...
STDMETHODIMP CClassicIEBHO::OnNavigateComplete(IDispatch* pDisp, VARIANT* URL)
{
	...
	{
		if (m_StatusBar)
			RemoveWindowSubclass(m_StatusBar, SubclassStatusProc, (UINT_PTR)this);
		m_StatusBar = status;
		if (m_StatusBar)
		{
			m_TextWidth = 0;
	...
	if (!m_Tooltip)
	{
		m_Tooltip = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_TRANSPARENT,TOOLTIPS_CLASS,NULL,
		                           WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, 0, 0, 0, 0, m_StatusBar,NULL, g_Instance,
		                           NULL);
	...
	if (!m_ProgressBar)
	{
		m_ProgressBar = CreateWindowEx(0,PROGRESS_CLASS,NULL,WS_CHILD | PBS_SMOOTH, 0, 0, 0, 0, m_StatusBar,NULL,
		                               g_Instance,NULL);
	...
	if (m_pZoneManager && m_pSecurityManager && URL && URL->vt == VT_BSTR)
	{
		...
		if (SUCCEEDED(m_pSecurityManager->MapUrlToZone(URL->bstrVal,&zone,0)) && SUCCEEDED(
			m_pZoneManager->GetZoneAttributes(zone,&attributes)))
		{
			...
			else
			{
				...
				{
					HMODULE hModule =
						LoadLibraryEx(path,NULL,LOAD_LIBRARY_AS_DATAFILE | LOAD_LIBRARY_AS_IMAGE_RESOURCE);
					if (hModule)
					{
