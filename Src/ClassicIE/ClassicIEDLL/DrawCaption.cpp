...
static _declspec(thread) SIZE g_SysButtonSize;
// the size of the system buttons (close, minimize) for this thread's window
...
static LRESULT CALLBACK SubclassCaptionProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (uMsg == WM_PAINT)
	{
		...
		opts.crText = GetSettingInt(bMaximized
			                            ? (bActive ? L"MaxColor" : L"InactiveMaxColor")
			                            : (bActive ? L"TextColor" : L"InactiveColor")) & 0xFFFFFF;
		...
		if (SUCCEEDED(DwmIsCompositionEnabled(&bComposition)) && bComposition)
		{
			...
			HPAINTBUFFER hBufferedPaint =
				BeginBufferedPaint(hdc, &ps.rcPaint, BPBF_TOPDOWNDIB, &paintParams, &hdcPaint);
			if (hdcPaint)
			{
				...
				else
				{
					...
					if (bIcon)
					{
						DrawIconEx(hdcPaint, rc.left, rc.bottom - iconSize - g_CustomCaption[1].topPadding, hIcon,
						           iconSize, iconSize, 0,NULL,DI_NORMAL | DI_NOMIRROR);
						rc.left += iconSize;
					}
				...
				DrawThemeTextEx(theme, hdcPaint, 0, 0, caption, -1,
				                DT_VCENTER | DT_NOPREFIX | DT_SINGLELINE | DT_CALCRECT, &rcText, &opts);
				...
				if (bGlow)
				{
					...
					AlphaBlend(hdcPaint, rc.left + textWidth, rc.top, 11, rc.bottom - rc.top, hSrc, 13, 0, 11, 24,
					           func);
					SelectObject(hSrc, bmp0);
					DeleteDC(hSrc);
				}
				DrawThemeTextEx(theme, hdcPaint, 0, 0, caption, -1,
				                DT_VCENTER | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS, &rc, &opts);
		...
		else
		{
			...
			if (bIcon)
			{
				DrawIconEx(hdc, rc.left, rc.bottom - iconSize - g_CustomCaption[2].topPadding, hIcon, iconSize,
				           iconSize, 0,NULL,DI_NORMAL | DI_NOMIRROR);
				rc.left += iconSize;
			}
			...
			RECT rcText = {0, 0, 0, 0};
			opts.dwFlags |= DTT_CALCRECT;
			DrawThemeTextEx(theme, hdc, 0, 0, caption, -1,DT_VCENTER | DT_NOPREFIX | DT_SINGLELINE | DT_CALCRECT,
			                &rcText, &opts);
			int textWidth = rcText.right - rcText.left;
			if (bCenter && textWidth < rc.right - rc.left)
			{
			...
			DrawThemeTextEx(theme, hdc, 0, 0, caption, -1,DT_VCENTER | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS,
			                &rc, &opts);
