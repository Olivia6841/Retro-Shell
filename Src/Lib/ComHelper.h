...
public:
	...
	PIDLIST_ABSOLUTE* operator&(void)
	{
		Assert(m_Pidl==NULL);
		return &m_Pidl;
	}

	void Swap(CAbsolutePidl& pidl)
	{
		PIDLIST_ABSOLUTE q = pidl.m_Pidl;
		pidl.m_Pidl = m_Pidl;
		m_Pidl = q;
	}
	...
	PIDLIST_ABSOLUTE Detach(void)
	{
		PIDLIST_ABSOLUTE pidl = m_Pidl;
		m_Pidl = NULL;
		return pidl;
	}

	void Clone(PCIDLIST_ABSOLUTE pidl)
	{
		Clear();
...
public:
	...
	void Clear(void)
	{
		if (m_String) CoTaskMemFree(m_String);
		m_String = NULL;
	}
