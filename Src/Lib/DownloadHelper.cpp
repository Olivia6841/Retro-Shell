...
static TDownloadResult DownloadFile(const wchar_t* url, std::vector<char>& buf, CString* pFilename, bool bAcceptCached,
                                    CProgressDlg* pProgress, TSettingsComponent component)
{
	const wchar_t* compName = L"Retro-Shell";
	switch (component)
	{
	case COMPONENT_EXPLORER: compName = L"Classic Explorer";
		break;
	case COMPONENT_MENU: compName = L"Retro-Shell Menu";
		break;
	case COMPONENT_IE: compName = L"Classic IE";
		break;
	case COMPONENT_UPDATE: compName = L"Retro-Shell Update";
		break;
	}
	...
	wchar_t agent[100];
	Sprintf(agent,_countof(agent), L"%s %d.%d.%d (Win %x)", compName, compVersion >> 24, (compVersion >> 16) & 0xFF,
	        compVersion & 0xFFFF, GetWinVersion());
	...
	if (hConnect)
	{
		...
		if (res == DOWNLOAD_OK)
		{
			...
			HINTERNET hRequest = HttpOpenRequest(hConnect, L"GET", file,NULL,NULL, accept,
			                                     ((components.nScheme == INTERNET_SCHEME_HTTPS)
				                                      ? INTERNET_FLAG_SECURE
				                                      : 0) | (bAcceptCached ? 0 : INTERNET_FLAG_RELOAD), 0);
			if (hRequest)
			{
				if (pProgress && pProgress->IsCanceled())
					res = DOWNLOAD_CANCEL;
				...
				if (res == DOWNLOAD_OK)
				{
					if (HttpSendRequest(hRequest,NULL, 0,NULL, 0))
					{
						...
						DWORD index = 0;
						if (!HttpQueryInfo(hRequest,HTTP_QUERY_CONTENT_LENGTH | HTTP_QUERY_FLAG_NUMBER, &fileSize, &len,
						                   &index))
							fileSize = 0;
					}
					else
...
static DWORD WINAPI ThreadVersionCheck(void* param)
{
	...
	curTime += (rand() * TIME_PRECISION) / (RAND_MAX + 1) - (TIME_PRECISION / 2);
	// add between -30 and 30 minutes to randomize access
	if (!res)
	{
		g_bCheckingVersion = false;
...
static bool VerifyDigitalCertificate(const wchar_t* fname, const wchar_t* signer)
{
	...
	HCERTSTORE hStore;
	HCRYPTMSG hMsg;
	if (CryptQueryObject(CERT_QUERY_OBJECT_FILE, fname,CERT_QUERY_CONTENT_FLAG_ALL,CERT_QUERY_FORMAT_FLAG_ALL, 0,NULL,
	                     NULL,NULL, &hStore, &hMsg,NULL) && hStore && hMsg)
	{
		DWORD len;
		if (CryptMsgGetParam(hMsg,CMSG_SIGNER_INFO_PARAM, 0,NULL, &len) && len > 0)
		{
			...
			if (CryptMsgGetParam(hMsg,CMSG_SIGNER_INFO_PARAM, 0, pSignerInfo, &len))
			{
				...
				PCCERT_CONTEXT pCertContext = CertFindCertificateInStore(
					hStore,X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0,CERT_FIND_SUBJECT_CERT, &certInfo,NULL);
				if (pCertContext)
				{
...
static DWORD WINAPI ThreadDownloadFile(void* param)
{
	...
	std::vector<char> buf;
	params.downloadRes = DownloadFile(params.url, buf, params.fname.IsEmpty() ? &params.fname : NULL,
	                                  params.bAcceptCached, params.progress, params.component);
                         ...
                         CString& fname, CString& error)
{
	...
	{
		wchar_t msg[256];
		FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,NULL, params.saveRes, 0, msg,
		              _countof(msg),NULL);
