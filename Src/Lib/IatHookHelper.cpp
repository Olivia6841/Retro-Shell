...
IatHookData* SetIatHook(IMAGE_DOS_HEADER* dosHeader, DWORD iatOffset, DWORD intOfset, const char* targetProc,
                        void* newProc)
{
...
IatHookData* SetIatHook(HMODULE hPatchedModule, const char* targetModule, const char* targetProc, void* newProc)
{
	...
	IMAGE_IMPORT_DESCRIPTOR* importDescriptor = (IMAGE_IMPORT_DESCRIPTOR*)PtrFromRva(
		dosHeader, ntHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress);
	...
	for (; importDescriptor->Characteristics != 0; importDescriptor++)
	{
		...
		return SetIatHook(dosHeader, importDescriptor->FirstThunk, importDescriptor->OriginalFirstThunk, targetProc,
		                  newProc);
	}

	ImgDelayDescr* delayDescriptor = (ImgDelayDescr*)PtrFromRva(
		dosHeader, ntHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DELAY_IMPORT].VirtualAddress);
