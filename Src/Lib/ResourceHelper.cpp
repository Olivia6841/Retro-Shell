...
HICON LoadIcon(int iconSize, const wchar_t* path, std::vector<HMODULE>& modules)
{
	...
	if (c)
	{
		...
		*c = 0;
		...
		if (!text[0])
			return (HICON)LoadImage(_AtlBaseModule.GetResourceInstance(), res,IMAGE_ICON, iconSize, iconSize,
			                        LR_DEFAULTCOLOR);
		...
		if (SUCCEEDED(pFolder->GetUIObjectOf(NULL,1,&child,IID_IExtractIcon,NULL,(void**)&pExtract)))
		{
			...
			if (pExtract->GetIconLocation(0, location,_countof(location), &index, &flags) == S_OK)
			{
				if (flags & GIL_NOTFILENAME)
				{
					// extract the icon
					HICON hIcon2 = NULL;
					HRESULT hr = pExtract->Extract(location, index, bLarge ? &hIcon : &hIcon2,
					                               bLarge ? &hIcon2 : &hIcon, lSize);
					if (FAILED(hr))
						hIcon = hIcon2 = NULL;
					if (hr == S_FALSE)
						flags = 0;
					if (hIcon2) DestroyIcon(hIcon2);
					// HACK!!! Even though Extract should support NULL, not all implementations do. For example shfusion.dll crashes
				}
				if (!(flags & GIL_NOTFILENAME))
				{
					if (ExtractIconEx(location, index == -1 ? 0 : index, bLarge ? &hIcon : NULL, bLarge ? NULL : &hIcon,
					                  1) != 1)
						hIcon = NULL;
				}
			}
			...
			if (SUCCEEDED(pFolder->GetUIObjectOf(NULL,1,&child,IID_IExtractIconA,NULL,(void**)&pExtractA)))
			{
				...
				if (pExtractA->GetIconLocation(0, location,_countof(location), &index, &flags) == S_OK)
				{
					if (flags & GIL_NOTFILENAME)
					{
						// extract the icon
						HICON hIcon2 = NULL;
						HRESULT hr = pExtractA->Extract(location, index, bLarge ? &hIcon : &hIcon2,
						                                bLarge ? &hIcon2 : &hIcon, lSize);
						if (FAILED(hr))
							hIcon = hIcon2 = NULL;
						if (hr == S_FALSE)
					...
					if (!(flags & GIL_NOTFILENAME))
					{
						if (ExtractIconExA(location, index == -1 ? 0 : index, bLarge ? &hIcon : NULL,
						                   bLarge ? NULL : &hIcon, 1) != 1)
							hIcon = NULL;
					}
				}
			}
...
HICON ShExtractIcon(const wchar_t* path, int index, int iconSize)
{
	...
	typedef UINT (WINAPI *FSHExtractIconsW)(LPCWSTR pszFileName, int nIconIndex, int cxIcon, int cyIcon, HICON* phIcon,
...
HBITMAP LoadImageFile(const wchar_t* path, const SIZE* pSize, bool bUseAlpha, bool bPremultiply,
                      std::vector<unsigned int>* pButtonAnim)
{
	HBITMAP srcBmp = NULL;
	...
	if (srcBmp)
	{
		...
		if (FAILED(
			pFactory->CreateBitmapFromHBITMAP(srcBmp,NULL,bUseAlpha?WICBitmapUseAlpha:WICBitmapIgnoreAlpha,&pBitmap2)))
		{
			DeleteObject(srcBmp);
			return NULL;
		}
	...
	if (FAILED(pFactory->CreateFormatConverter(&pConverter)))
		return NULL;
	pConverter->Initialize(pBitmap, bPremultiply ? GUID_WICPixelFormat32bppPBGRA : GUID_WICPixelFormat32bppBGRA,
	...
	if (pButtonAnim)
	{
		...
		if (width >= 3)
		{
			...
			if (SUCCEEDED(pBitmap->CopyPixels(&rect,12,12,(BYTE*)data)) && (data[0] & 0xFFFFFF) == ANIM_BUTTON_TAG1 && (
				data[1] & 0xFFFFFF) == ANIM_BUTTON_TAG2)
			{
...
HBITMAP LoadImageResource(HMODULE hModule, const wchar_t* name, bool bTopDown, bool bPremultiply)
{
	...
	else
	{
		...
		else
		{
			CComPtr<IWICBitmapDecoder> pDecoder;
			if (FAILED(
				pFactory->CreateDecoderFromFilename(name,NULL,GENERIC_READ,WICDecodeMetadataCacheOnLoad,&pDecoder)))
				return NULL;
			CComPtr<IWICBitmapFrameDecode> pFrame;
			if (FAILED(pDecoder->GetFrame(0,&pFrame)))
				return NULL;
...
DWORD GetVersionEx(HINSTANCE hInstance, DWORD* pBuild)
{
	...
	return ((HIWORD(pVer->dwProductVersionMS) & 255) << 24) | ((LOWORD(pVer->dwProductVersionMS) & 255) << 16) | HIWORD(
		pVer->dwProductVersionLS);
}
...
HRESULT ShParseDisplayName(const wchar_t* pszName, PIDLIST_ABSOLUTE* ppidl, SFGAOF sfgaoIn, SFGAOF* psfgaoOut)
{
	...
	else
	{
		*ppidl = NULL;
		...
		HRESULT hr = pDesktop->ParseDisplayName(NULL,NULL, (wchar_t*)pszName,NULL, (PIDLIST_RELATIVE*)ppidl,
		                                        psfgaoOut ? &flags : NULL);
		if (FAILED(hr))
			return hr;
		if (psfgaoOut)
			*psfgaoOut = flags;
...
HFONT CreateFontSetting(const wchar_t* fontStr, int dpi)
{
	...
	return CreateFont(MulDiv(size, dpi, 72), 0, 0, 0, weight, bItalic ? 1 : 0, 0, 0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
	                  CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH, name);
}

static UINT WINAPI GetDpiForWindow(HWND hwnd)
{
	static auto p = static_cast<decltype(&GetDpiForWindow)>((void*)GetProcAddress(
