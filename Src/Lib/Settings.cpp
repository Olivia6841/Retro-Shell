		...
		for (const CSetting* pSetting = GetAllSettings(); pSetting->name; pSetting++)
		{
			if (_wcsnicmp(pSetting->name, name, len) == 0 && pSetting->name[len] == 0)
			{
				...
				if (checkEnabled && !pSetting->IsEnabled())
					return false;
				if ((pSetting->type == CSetting::TYPE_BOOL || pSetting->type == CSetting::TYPE_INT) && pSetting->
					GetValue().vt == VT_I4)
				{
				...
				if ((pSetting->type == CSetting::TYPE_STRING || pSetting->type == CSetting::TYPE_BITMAP || pSetting->
						type == CSetting::TYPE_BITMAP_JPG || pSetting->type == CSetting::TYPE_DIRECTORY) && pSetting->
					GetValue().vt == VT_BSTR)
				{
...
bool CSetting::ReadValue(CRegKey& regKey, const wchar_t* valName)
{
	...
	if (type == CSetting::TYPE_BOOL || (type == CSetting::TYPE_INT && this[1].type != CSetting::TYPE_RADIO) || type ==
		CSetting::TYPE_HOTKEY || type == CSetting::TYPE_HOTKEY_ANY || type == CSetting::TYPE_COLOR)
	{
		DWORD val;
...
public:
	...
	bool SetSettingsStyle(int style, int mask)
	{
		if (m_SettingsStyle == style && m_SettingsMask == mask) return false;
		m_SettingsStyle = style;
		m_SettingsMask = mask;
		return true;
	}

	void GetSettingsStyle(int& style, int& mask) const
	{
		style = m_SettingsStyle;
		mask = m_SettingsMask;
	}
...
int CSettingsManager::GetSettingInt(const wchar_t* name) const
{
	for (const CSetting* pSetting = m_pSettings; pSetting->name; pSetting++)
	{
		if ((pSetting->type == CSetting::TYPE_INT || pSetting->type == CSetting::TYPE_HOTKEY || pSetting->type ==
			CSetting::TYPE_HOTKEY_ANY || pSetting->type == CSetting::TYPE_COLOR) && _wcsicmp(pSetting->name, name) == 0)
		{
			Assert(!pSetting->pLinkTo);
			CSettingsLockRead lock;
...
static bool OpenSettingsKeys(const wchar_t* regPath, const wchar_t* gpPath, CRegKey& regSettings,
                             CRegKey& regSettingsUser, CRegKey& regPolicy, CRegKey& regPolicyUser)
{
	bool bUpgrade = false;
	if (*regPath)
	{
...
void CSettingsManager::LoadSettings(bool bShared)
{
	...
	bool bUpgrade = OpenSettingsKeys(bShared ? m_RegPathShared : m_RegPath, bShared ? m_GpPathShared : m_GpPath,
	                                 regSettings, regSettingsUser, regPolicy, regPolicyUser);
...
void CSettingsManager::SaveSettings(bool bShared)
{
	...
	for (const CSetting* pSetting = m_pSettings; pSetting->name; pSetting++)
	{
		...
		if (pSetting->type == CSetting::TYPE_BOOL || (pSetting->type == CSetting::TYPE_INT && pSetting[1].type !=
				CSetting::TYPE_RADIO) || pSetting->type == CSetting::TYPE_HOTKEY || pSetting->type ==
			CSetting::TYPE_HOTKEY_ANY || pSetting->type == CSetting::TYPE_COLOR)
		{
			DWORD val = 0;
...
CString CSettingsManager::LoadSettingsXml(const wchar_t* fname)
{
	...
	DWORD ver = 0;
	{
		...
		{
			CString error;
			error.Format(L"XML parsing error: This settings file is intended for another component '%s'.",
			             value.bstrVal);
			return error;
		}
				...
				else
				{
					...
					if (element)
					{
						...
						if (element->getAttribute(g_bstrValue, &value) == S_OK && value.vt == VT_BSTR)
						{
							...
							else if (pSetting->type == CSetting::TYPE_BOOL || (pSetting->type == CSetting::TYPE_INT &&
									pSetting[1].type != CSetting::TYPE_RADIO) || pSetting->type == CSetting::TYPE_HOTKEY
								||
								pSetting->type == CSetting::TYPE_HOTKEY_ANY || pSetting->type == CSetting::TYPE_COLOR)
							{
							...
							{
								int val = 0;
								for (CSetting* pRadio = pSetting + 1; pRadio->type == CSetting::TYPE_RADIO; pRadio++,
								     val++)
								{
...
CString CSettingsManager::SaveSettingsXml(const wchar_t* fname)
{
	...
	for (const CSetting* pSetting = m_pSettings; pSetting->name; pSetting++)
	{
		if (pSetting->type == CSetting::TYPE_GROUP || pSetting->type == CSetting::TYPE_RADIO || pSetting->pLinkTo)
			continue;
		if (pSetting->flags & (CSetting::FLAG_LOCKED_REG | CSetting::FLAG_NOSAVE | CSetting::FLAG_DEFAULT |
		...
		else if (pSetting->type == CSetting::TYPE_BOOL || (pSetting->type == CSetting::TYPE_INT && pSetting[1].type !=
				CSetting::TYPE_RADIO) || pSetting->type >= CSetting::TYPE_HOTKEY || pSetting->type >=
			CSetting::TYPE_HOTKEY_ANY || pSetting->type >= CSetting::TYPE_STRING)
		{
...
HIMAGELIST CSettingsManager::GetImageList(HWND tree)
{
	...
	m_ImageList = ImageList_Create(imageSize, imageSize,
	                               ILC_COLOR32 | ILC_MASK | ((GetWindowLong(tree,GWL_EXSTYLE) & WS_EX_LAYOUTRTL)
		                                                         ? ILC_MIRROR
		                                                         : 0), 0, 23);
	...
	for (int i = 0; i < 13; i++)
	{
		...
		if (i == 1)
		{
			HICON icon = (HICON)LoadImage(_AtlBaseModule.GetResourceInstance(),MAKEINTRESOURCE(IDI_ICONLOCK),IMAGE_ICON,
		...
		else if (i == 2 || i == 3)
		{
			...
			if (hShell32)
			{
				HICON icon = (HICON)LoadImage(hShell32,MAKEINTRESOURCE(151),IMAGE_ICON, iconSize, iconSize,
				                              LR_DEFAULTCOLOR);
				DrawIconEx(hdc, iconOffset, iconOffset, icon, iconSize, iconSize, 0,NULL,DI_NORMAL);
				DestroyIcon(icon);
			}
		...
		{
			HICON icon = (HICON)LoadImage(_AtlBaseModule.GetResourceInstance(),MAKEINTRESOURCE(IDI_ICONWARNING),
...
static LRESULT CALLBACK SubclassSearchBoxProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
                                              DWORD_PTR dwRefData)
{
...
LRESULT CSettingsDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (regSettings.Open(HKEY_CURRENT_USER, GetSettingsRegPath()) == ERROR_SUCCESS)
	{
		...
		bPosValid = (regSettings.QueryBinaryValue(L"CSettingsDlg", &pos, &size) == ERROR_SUCCESS && (size == sizeof(pos)
			|| size == sizeof(pos) - 4));
		regSettings.Close();
	...
	HICON icon = (HICON)LoadImage(hInstance,MAKEINTRESOURCE(1),IMAGE_ICON, GetSystemMetrics(SM_CXICON),
	                              GetSystemMetrics(SM_CYICON),LR_DEFAULTCOLOR);
	SendMessage(WM_SETICON,ICON_BIG, (LPARAM)icon);
	icon = (HICON)LoadImage(hInstance,MAKEINTRESOURCE(1),IMAGE_ICON, GetSystemMetrics(SM_CXSMICON),
	                        GetSystemMetrics(SM_CYSMICON),LR_DEFAULTCOLOR);
	SendMessage(WM_SETICON,ICON_SMALL, (LPARAM)icon);
	icon = (HICON)LoadImage(GetModuleHandle(L"shell32.dll"),MAKEINTRESOURCE(323),IMAGE_ICON,
	...
	if (parent)
	{
		...
		OffsetRect(&rc1, (rc2.left + rc2.right) / 2 - (rc1.left + rc1.right) / 2,
		           (rc2.top + rc2.bottom) / 2 - (rc1.top + rc1.bottom) / 2);
	...
	if (m_bDirty)
	{
		...
		if (res == IDYES)
		{
			if (IsTabValid())
			{
				...
				for (const CSetting* pSetting = m_pSettings; pSetting->name; pSetting++)
				{
					if (pSetting->type == CSetting::TYPE_GROUP || pSetting->type == CSetting::TYPE_RADIO || pSetting->
						pLinkTo)
						continue;
					if (pSetting->value != pSetting->tempValue)
						flags |= pSetting->flags & (CSetting::FLAG_WARM | CSetting::FLAG_COLD);
...
LRESULT CSettingsDlg::OnBackup(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	if (res == 1)
	{
		...
		ofn.Flags = OFN_DONTADDTORECENT | OFN_ENABLESIZING | OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT |
	...
	{
		// reset
		if (::MessageBox(m_hWnd, LoadStringEx(IDS_RESET_CONFIRM), LoadStringEx(IDS_RESET_TITLE),
...
void EditSettings(const wchar_t* title, bool bModal, int tab, const wchar_t* appId)
{
	...
	else
	{
		...
		g_SettingsDlg.SetWindowPos(HWND_TOPMOST, 0, 0, 0, 0,
		                           SWP_NOSIZE | SWP_NOMOVE | (g_SettingsDlg.GetOnTop() ? 0 : SWP_NOZORDER) |
		                           SWP_SHOWWINDOW);
...
bool IsSettingsMessage(MSG* msg)
{
	...
	if (msg->message == WM_KEYDOWN && msg->wParam == VK_F1 && GetKeyState(VK_CONTROL) >= 0 && GetKeyState(VK_SHIFT) >= 0
...
bool OpenSettingsKeys(TSettingsComponent component, CRegKey& regSettings, CRegKey& regSettingsUser, CRegKey& regPolicy,
                      CRegKey& regPolicyUser)
{
