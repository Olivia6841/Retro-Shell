		...
		if (!bTitle && !link.IsEmpty())
		{
			...
			if (pidl)
			{
				...
				if (SUCCEEDED(SHCreateItemFromIDList(pidl,IID_IShellItem,(void**)&pItem)))
				{
					...
					{
						CString name;
						name.Format(L"%s (%s)", (const wchar_t*)LoadStringEx(pStdCommand->displayNameId),
						            (const wchar_t*)pName);
						return name;
					}
				}
...
static LRESULT CALLBACK SubclassEditboxProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
                                            DWORD_PTR dwRefData)
{
...
LRESULT CSettingsTree::OnMouseMove(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (!bDelete)
	{
		...
		if (m_DropTarget && !m_pStdDragItem)
		{
			...
			for (HTREEITEM hItem = TreeView_GetParent(m_hWnd, m_DropTarget); hItem; hItem = TreeView_GetParent(
				     m_hWnd, hItem))
			{
				if (hItem == m_DragItem)
				{
		...
		if (m_DropTarget)
		{
			...
			else
			{
				...
				if (!item.lParam || ((CTreeItem*)item.lParam)->pStdCommand->IsSeparator() || (m_DropTarget == m_DragItem
					&& !m_pStdDragItem))
					m_DropTarget = NULL;
				else
...
{
	for (HTREEITEM hItem = hParent ? TreeView_GetChild(m_hWnd, hParent) : TreeView_GetRoot(m_hWnd); hItem; hItem =
	     TreeView_GetNextSibling(m_hWnd, hItem))
	{
...
HTREEITEM CSettingsTree::CreateDeepCopy(HTREEITEM hSource, HTREEITEM hParent, HTREEITEM hAfter, bool bRename)
{
	TVINSERTSTRUCT insert = {
		hParent, hAfter ? hAfter : TVI_FIRST,
		{TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE, hSource, 0,TVIS_EXPANDED}
	};
	...
	for (HTREEITEM hChild = TreeView_GetChild(m_hWnd, hSource); hChild; hChild =
	     TreeView_GetNextSibling(m_hWnd, hChild))
...
HTREEITEM CSettingsTree::CreateStdItem(const CStdCommand* pCommand, HTREEITEM hParent, HTREEITEM hAfter)
{
	TVINSERTSTRUCT insert = {
		hParent, hAfter ? hAfter : TVI_FIRST,
		{TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE | TVIF_TEXT,NULL,TVIS_EXPANDED,TVIS_EXPANDED}
	};
...
{
	if (m_DragMode != DRAG_NONE || (lParam && ((MSG*)lParam)->message == WM_KEYDOWN && ((MSG*)lParam)->wParam ==
		VK_RETURN))
		return DLGC_WANTALLKEYS;
	bHandled = FALSE;
	return 0;
		...
		if (m_HoverItem)
		{
			...
			if (!TreeView_GetChild(m_hWnd, m_HoverItem))
			{
				if (m_DropSubmenu)
					TreeView_DeleteItem(m_hWnd, m_DropSubmenu);
				TVINSERTSTRUCT insert = {
					m_HoverItem,TVI_FIRST,
					{
						TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE | TVIF_TEXT,NULL,TVIS_EXPANDED,
						TVIS_EXPANDED
					}
				};
...
void CEditCustomItemDlg::InitDialog(CWindow commandCombo, const CStdCommand* pStdcommands, int style, int mask,
                                    CWindow linkCombo, const KNOWNFOLDERID* const * pCommonLinks)
{
...
HRESULT STDMETHODCALLTYPE CBrowseLinkEvents::OnButtonClicked(IFileDialogCustomize* pfdc, DWORD dwIDCtl)
{
	...
	{
		SFGAOF attr;
		if (SUCCEEDED(pItem->GetAttributes(SFGAO_FOLDER|SFGAO_STREAM,&attr)) && (attr & (SFGAO_FOLDER | SFGAO_STREAM))
			!= SFGAO_FOLDER)
			pItem = NULL;
	}
	if (!pItem)
...
bool BrowseCommandHelper(HWND parent, wchar_t* text)
{
	...
	ofn.Flags = OFN_DONTADDTORECENT | OFN_ENABLESIZING | OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_HIDEREADONLY |
...
bool BrowseLinkHelper(HWND parent, wchar_t* text, bool bFoldersOnly)
{
	...
	if (bFoldersOnly) // set FOS_PICKFOLDERS option to use dialog in folder-only mode
		...
		pDialog->SetOptions(
			FOS_ALLNONSTORAGEITEMS | FOS_FILEMUSTEXIST | FOS_DONTADDTORECENT | FOS_DEFAULTNOMINIMODE |
...
LRESULT CCustomTreeDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	for (int i = 0; m_pStdCommands[i].name; i++)
	{
		...
		TVINSERTSTRUCT insert = {
			NULL,TVI_LAST,
			{TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE,NULL, 0, 0, (LPWSTR)(LPCWSTR)displayName}
		};
...
LRESULT CCustomTreeDlg::OnContextMenu(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (res == CMD_RESETALL)
	{
		if (::MessageBox(m_hWnd, LoadStringEx(m_bMenu ? IDS_RESET_MENU_WARN : IDS_RESET_TOOLBAR_WARN),
		                 LoadStringEx(m_bMenu ? IDS_RESET_MENU : IDS_RESET_TOOLBAR),MB_YESNO | MB_ICONWARNING) == IDYES)
		{
			{
...
LRESULT CCustomTreeDlg::OnGetInfoTip(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	...
	{
		if (idCtrl == IDC_TREECOMMANDS)
			Sprintf(pTip->pszText, pTip->cchTextMax, L"%s\n%s", LoadStringEx(pCommand->tipID),
			        LoadStringEx(m_bMenu ? IDS_COMMAND_SM_TIP : IDS_COMMAND_EXP_TIP));
		else
...
{
	if (!m_pSetting->IsLocked() && !pItem->pStdCommand->IsSeparator() && EditItem(
		pItem, m_Tree, hItem, m_Tree.m_Modules))
	{
		SerializeData();
...
void CCustomTreeDlg::CreateTreeItems(CSettingsParser& parser, HTREEITEM hParent,
                                     const CSettingsParser::TreeItem* pItems, int index)
{
	for (; !pItems[index].name.IsEmpty(); index++)
	{
		...
		TVINSERTSTRUCT insert = {
			hParent,TVI_LAST,
			{TVIF_STATE | TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE,NULL,TVIS_EXPANDED,TVIS_EXPANDED}
		};
...
LRESULT CBrowseForIconDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	DeleteDC(hdc);
	ListView_SetIconSpacing(list, GetSystemMetrics(SM_CXICON)+metrics.tmHeight+7,
	                        GetSystemMetrics(SM_CYICON)+metrics.tmHeight+7);
	if (IsAppThemed())
...
void CBrowseForIconDlg::UpdateIcons(int id)
{
	...
	HIMAGELIST images = ImageList_Create(GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON),
	                                     ILC_COLOR32 | ILC_MASK, 10, 10);
                                         ...
                                         DWORD_PTR dwRefData)
{
	...
	if (uMsg == WM_KEYDOWN)
	{
		if (wParam == VK_UP || wParam == VK_DOWN || wParam == VK_PRIOR || wParam == VK_NEXT || (wParam == 'P' &&
...
LRESULT CTreeSettingsDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	int height = TreeView_GetItemHeight(m_Tree);
	if (height < 18)
		TreeView_SetItemHeight(m_Tree, 18);
	...
	m_PlayButton = CreateWindow(L"BUTTON", L">", WS_CHILD|BS_PUSHBUTTON|BS_ICON, 0, 0, 0, 0, m_Tree, (HMENU)IDC_PLAYBTN,
	                            hInstance, NULL);
...
LRESULT CTreeSettingsDlg::OnBrowse(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	else if (m_EditMode == EDIT_BITMAP || m_EditMode == EDIT_BITMAP_JPG)
	{
		...
		if (_wcsicmp(PathFindExtension(str), L".bmp") == 0 || _wcsicmp(PathFindExtension(str), L".png") == 0 || (
			m_EditMode == EDIT_BITMAP_JPG && _wcsicmp(PathFindExtension(str), L".jpg") == 0))
		{
			Strcpy(text,_countof(text), str);
...
LRESULT CTreeSettingsDlg::OnGetInfoTip(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	...
	if (pSetting->IsLocked())
		Sprintf(pTip->pszText, pTip->cchTextMax, L"%s\n%s", LoadStringEx(pSetting->tipID),
		        LoadStringEx(pSetting->IsLockedReg() ? IDS_SETTING_LOCKED : IDS_SETTING_LOCKED_GP));
	else
...
void CTreeSettingsDlg::ItemSelected(HTREEITEM hItem, CSetting* pSetting, bool bEnabled)
{
	...
	if (pSetting)
	{
		...
		else if (pSetting->type == CSetting::TYPE_STRING || pSetting->type == CSetting::TYPE_ICON || pSetting->type ==
			CSetting::TYPE_BITMAP || pSetting->type == CSetting::TYPE_BITMAP_JPG || pSetting->type ==
			CSetting::TYPE_SOUND || pSetting->type == CSetting::TYPE_FONT || pSetting->type == CSetting::TYPE_DIRECTORY)
		{
	...
	if (mode == EDIT_ICON || mode == EDIT_BITMAP || mode == EDIT_BITMAP_JPG || mode == EDIT_SOUND || mode == EDIT_FONT
		|| mode == EDIT_COLOR || mode == EDIT_DIRECTORY)
	{
		RECT rc2 = rc;
	...
	if (mode != EDIT_NONE)
	{
		...
		SendMessage(WM_NEXTDLGCTL, (LPARAM)m_EditBox.m_hWnd,TRUE);
		SetWindowSubclass(m_EditBox, SubclassEditProc, 'CLSH',
		                  (mode == EDIT_HOTKEY) ? 1 : ((mode == EDIT_HOTKEY_ANY) ? 2 : 0));
	}
	else
...
void CTreeSettingsDlg::UpdateEditPosition(void)
{
	...
	if (m_EditMode == EDIT_ICON || m_EditMode == EDIT_BITMAP || m_EditMode == EDIT_BITMAP_JPG || m_EditMode == EDIT_FONT
		|| m_EditMode == EDIT_COLOR || m_EditMode == EDIT_DIRECTORY)
		size.cx += width;
	...
	if (m_EditMode == EDIT_ICON || m_EditMode == EDIT_BITMAP || m_EditMode == EDIT_BITMAP_JPG || m_EditMode ==
		EDIT_SOUND || m_EditMode == EDIT_FONT || m_EditMode == EDIT_COLOR || m_EditMode == EDIT_DIRECTORY)
	{
		RECT rc2 = rc;
		rc2.left = rc2.right - width;
		if (m_EditMode == EDIT_SOUND)
...
const CSetting* CTreeSettingsDlg::GetNextSetting(const CSetting* pSetting, const wchar_t* filter)
{
	...
	if (*filter)
	{
		while (pSetting->name && (pSetting->type == CSetting::TYPE_GROUP || !IsVisible(pSetting, filter) || !pSetting->
			MatchFilter(filter)))
		{
...
void CTreeSettingsDlg::SetGroup(CSetting* pGroup, const CString& filter, const CSetting* pSelect)
{
	...
	for (const CSetting* pSetting = GetNextSetting(pGroup, filter); pSetting; pSetting = GetNextSetting(
		     pSetting, filter))
	{
		wchar_t text[256];
		text[0] = 0;
		if (pSetting->type == CSetting::TYPE_BOOL || pSetting->type == CSetting::TYPE_RADIO || (pSetting->type ==
			CSetting::TYPE_INT && pSetting[1].type == CSetting::TYPE_RADIO))
		{
		...
		TVINSERTSTRUCT insert = {
			hParent,TVI_LAST,
			{
				TVIF_STATE | TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM,NULL,TVIS_EXPANDED,TVIS_EXPANDED,
				text, 0, SETTING_STATE_SETTING, SETTING_STATE_SETTING
			}
		};
	...
	if (hSelect)
	{
		...
		m_Tree.PostMessage(TVM_ENSUREVISIBLE, 0, (LPARAM)hSelect);
		// post instead of send, because the tree may be resized afterwards
	}
	m_Tree.Invalidate();
}
...
void CTreeSettingsDlg::UpdateGroup(const CSetting* pModified)
{
	for (HTREEITEM hItem = TreeView_GetRoot(m_Tree); hItem; hItem = GetNextItem(hItem))
	{
		...
		if (pSetting->type == CSetting::TYPE_INT && pSetting[1].type == CSetting::TYPE_RADIO)
		{
			...
			if (valVar.vt == VT_I4)
				val = valVar.intVal;
			for (HTREEITEM hRadio = TreeView_GetChild(m_Tree, hItem); hRadio; hRadio = TreeView_GetNextSibling(
				     m_Tree, hRadio))
			{
