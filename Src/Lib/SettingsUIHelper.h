...
template <class T>
class CResizeableDlg : public CDialogImpl<T>
{
public:
	CResizeableDlg(void)
	{
	...
	void Create(HWND hWndParent, DLGTEMPLATE* pTemplate)
	{
		...
		HWND hWnd = ::CreateDialogIndirect(_AtlBaseModule.GetResourceInstance(), pTemplate, hWndParent,
		                                   T::StartDialogProc);
...
protected:
	...
	void InitResize(int flags = MOVE_HORIZONTAL | MOVE_VERTICAL)
	{
		...
		if (!(m_Flags & MOVE_REINITIALIZE))
		{
			...
			if (m_Flags & MOVE_GRIPPER)
				m_Gripper.Create(L"SCROLLBAR", pThis->m_hWnd, rc,NULL,
	...
	{
		this->SetWindowPos(NULL, rc.left, rc.top, m_WindowSize.cx + rc.right, m_WindowSize.cy + rc.bottom,
...
private:
	...
	void CreateTreeItems(CSettingsParser& parser, HTREEITEM hParent, const CSettingsParser::TreeItem* pItems,
	                     int index);
};
...
protected:
	...
	void InitDialog(CWindow commandCombo, const CStdCommand* pStdcommands, int style, int mask, CWindow linkCombo,
	                const KNOWNFOLDERID* const * pCommonLinks);
