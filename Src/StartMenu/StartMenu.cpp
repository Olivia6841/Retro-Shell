...
static bool HookStartMenu(THookMode mode, HWND& menu)
{
	...
	HWND progWin = NULL;
	bool bFindAppManager = (mode == HOOK_STARTUP && GetWinVersion() >= WIN_VER_WIN8 && GetWinVersion() <=
		WIN_VER_WIN81);
	for (int i = 0; i < 120; i++) // retry for 1 minute
	{
		...
		if (!bFindAppManager)
		{
			...
			if (progWin) break;
			if (mode != HOOK_STARTUP && mode != HOOK_STARTUP2) return true;
			// the Progman window may not be created yet (if Explorer is currently restarting)
		}
	...
	for (int i = 0; i < 10; i++) // retry for 5 sec
	{
		...
		if (mode != HOOK_STARTUP && mode != HOOK_STARTUP2) return false;
		// the taskbar may not be created yet (if Explorer is currently restarting)
...
DWORD PerformOsUpgradeTask(bool silent)
{
	...
	if (!silent)
	{
		if (error)
		{
			...
			FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, error, 0, msg + len,
			              _countof(msg) - len, NULL);
			MessageBox(NULL, msg, DllLoadStringEx(IDS_APP_TITLE), MB_OK | MB_ICONERROR);
		}
		else
		{
			MessageBox(NULL, DllLoadStringEx(IDS_UPGRADE_SUCCESS), DllLoadStringEx(IDS_APP_TITLE),
...
)
{
	...
	*PathFindFileName(path) = 0;
		...
		if (GetLastError() == ERROR_ALREADY_EXISTS || GetLastError() == ERROR_ACCESS_DENIED)
		{
			if (open == MSG_TOGGLE || open == CMD_TOGGLE_NEW)
			{
				if (progWin)
				{
					...
					if (taskBar)
						PostMessage(taskBar,RegisterWindowMessage(L"OpenShellMenu.StartMenuMsg"),
						            (open == CMD_TOGGLE_NEW) ? MSG_TOGGLENEW : MSG_TOGGLE, 0);
					else
			...
			else if (open != CMD_NONE)
			{
				...
				if (hwnd)
					PostMessage(hwnd, WM_OPEN, open, 0);
			}
	...
	typedef BOOL (WINAPI *tChangeWindowMessageFilterEx)(HWND hWnd, UINT message, DWORD action,
	                                                    PCHANGEFILTERSTRUCT pChangeFilterStruct);
	HMODULE hUser32 = GetModuleHandle(L"user32.dll");
	tChangeWindowMessageFilterEx ChangeWindowMessageFilterEx = (tChangeWindowMessageFilterEx)GetProcAddress(
