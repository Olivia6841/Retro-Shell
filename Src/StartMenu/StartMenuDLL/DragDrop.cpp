...
public:
	...
	virtual HRESULT STDMETHODCALLTYPE DAdvise(FORMATETC* pformatetc, DWORD advf, IAdviseSink* pAdvSink,
	                                          DWORD* pdwConnection) { return OLE_E_ADVISENOTSUPPORTED; }
...
CComPtr<IDataObject> CMenuContainer::CreateMetroDataObject(const CItemManager::ItemInfo* pInfo)
{
	...
	if (pDataObject)
	{
		...
		{
			g_ItemManager.UpdateItemInfo(pInfo, CItemManager::INFO_EXTRA_LARGE_ICON | CItemManager::INFO_REFRESH_NOW,
			                             false);
...
bool CMenuContainer::DragOut(int index, bool bApp)
{
	...
	else
	{
		...
		{
			g_ItemManager.UpdateItemInfo(item.pItemInfo,
			                             CItemManager::INFO_EXTRA_LARGE_ICON | CItemManager::INFO_REFRESH_NOW, false);
			int iconSize = CItemManager::EXTRA_LARGE_ICON_SIZE;
			SHDRAGIMAGE di = {{iconSize, iconSize}, {iconSize / 2, iconSize},NULL,CLR_NONE};
	...
	m_DragIndex = index;
	s_bDragMovable = (item.id == MENU_NO && index < m_OriginalCount) || (item.jumpIndex >= 0 && s_JumpList.groups[
		LOWORD(item.jumpIndex)].type == CJumpGroup::TYPE_PINNED);
...
void CMenuContainer::SetDropTip(IDataObject* pDataObj, bool bPin)
{
	...
	Strcpy(desc.szMessage,_countof(desc.szMessage),
	       bPin ? FindTranslation(L"Menu.PinStart", L"Pin to Start menu") : L"");
...
void CMenuContainer::GetDragEffect(DWORD& grfKeyState, DWORD* pdwEffect)
{
	...
	if (m_pDragObject->QueryGetData(&format1) != S_OK && m_pDragObject->QueryGetData(&format2) != S_OK && m_pDragObject
		->QueryGetData(&format3) != S_OK)
	{
	...
	if (s_pDragSource)
	{
		if (s_pDragSource->m_Items[s_pDragSource->m_DragIndex].id == MENU_RECENT)
			...
			*pdwEffect &= (s_bDragMovable && m_bSubMenu && (s_pDragSource->m_Options & CONTAINER_PROGRAMS))
				              ? DROPEFFECT_MOVE
				              : DROPEFFECT_LINK; // dragging normal item to a different menu - default to move or link
	}

	if (m_pDragObject->QueryGetData(&format3) == S_OK)
	{
		if (m_Options & CONTAINER_APPS)
			*pdwEffect &= (s_pDragSource == this && !s_bDragFromTree) ? DROPEFFECT_MOVE : DROPEFFECT_NONE;
			// dragging a metro link to Apps folder
		else
...
HRESULT STDMETHODCALLTYPE CMenuContainer::DragEnter(IDataObject* pDataObj, DWORD grfKeyState, POINTL pt,
                                                    DWORD* pdwEffect)
{
...
HRESULT STDMETHODCALLTYPE CMenuContainer::DragOver(DWORD grfKeyState, POINTL pt, DWORD* pdwEffect)
{
	...
	if (index >= 0)
	{
		...
		if (!m_bSubMenu && index < m_OriginalCount && (!s_pDragSource || s_bDragFromTree || s_bDragMovable || m_Items[
			index].id != MENU_RECENT))
		{
	...
	if (index >= 0 && index < m_OriginalCount && m_Items[index].id != MENU_RECENT)
	{
		...
		{
			// insert above
			if (m_Items[index].id != MENU_NO && m_Items[index].id != MENU_EMPTY && m_Items[index].id != MENU_EMPTY_TOP
				&& (index == 0 || m_Items[index - 1].id != MENU_NO))
				mark = -1;
		}
		else
		{
			...
			bAfter = true;
			if (m_Items[index].id != MENU_NO && m_Items[index].id != MENU_EMPTY && m_Items[index].id != MENU_EMPTY_TOP
				&& (index == m_Items.size() - 1 || m_Items[index + 1].id != MENU_NO))
				mark = -1;
		}
		...
		if (!*pdwEffect || ((m_Options & CONTAINER_AUTOSORT) && s_pDragSource == this && !s_bDragFromTree &&
			s_bDragMovable))
			mark = -1;
	}
	else if (s_pDragSource == this && !s_bDragFromTree && s_bDragMovable && m_Items[m_DragIndex].jumpIndex >= 0 && index
		>= 0 && m_Items[index].jumpIndex >= 0)
	{
		int groupIndex = LOWORD(m_Items[m_DragIndex].jumpIndex);
		if (s_JumpList.groups[groupIndex].type == CJumpGroup::TYPE_PINNED && LOWORD(m_Items[index].jumpIndex) ==
			groupIndex && (*pdwEffect & DROPEFFECT_MOVE))
		{
			// reorder jump list
			*pdwEffect = DROPEFFECT_MOVE;
			int y = (rcItem.top + rcItem.bottom) / 2;
	...
	{
		if (!m_bSubMenu && index < m_OriginalCount && PtInRect(&m_rContent, p) && (s_pDragSource != this ||
			s_bDragFromTree || !s_bDragMovable || m_Items[m_DragIndex].id == MENU_RECENT))
		{
			if (s_bWin7Style)
			{
	...
	if (index >= 0 && index == m_DragHoverItem)
	{
		...
		if ((GetMessageTime() - m_DragHoverTime) > hoverTime && m_Submenu != m_DragHoverItem)
		{
			...
			if (index < m_OriginalCount && !m_Items[index].bHasJumpList && (!m_Items[index].bFolder || m_Items[index].
				pItem1))
			{
				ActivateData data;
	...
	if (before >= 0 && m_bInsertAfter && (before != 0 || (m_Items[0].id != MENU_EMPTY && m_Items[0].id !=
		MENU_EMPTY_TOP)))
		before++;
	if (before >= 0 && !m_bSubMenu && (*pdwEffect & DROPEFFECT_LINK) && (s_pDragSource != this || s_bDragFromTree || !
		s_bDragMovable))
	{
		*pdwEffect = DROPEFFECT_LINK;
...
HRESULT STDMETHODCALLTYPE CMenuContainer::Drop(IDataObject* pDataObj, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect)
{
	...
	if (s_pDragSource == this && !s_bDragFromTree && s_bDragMovable && (*pdwEffect & DROPEFFECT_MOVE) && m_DragIndex !=
		m_ProgramTreeIndex && m_Items[m_DragIndex].priority == (m_Items[min(before, (int)m_Items.size()-1)].priority &
			2))
	{
		...
		if (m_Items[m_DragIndex].jumpIndex >= 0 && s_JumpList.groups[LOWORD(m_Items[m_DragIndex].jumpIndex)].type ==
			CJumpGroup::TYPE_PINNED)
		{
	...
	else if (m_pDropFolder[folderIndex])
	{
		...
		if (!bDropped)
		{
			...
			if (!pDesktop || FAILED(pDesktop->BindToObject(m_Path1[folderIndex],NULL,IID_IShellFolder,(void**)&pFolder))
				|| FAILED(pFolder->CreateViewObject(g_OwnerWindow,IID_IDropTarget,(void**)&pTarget)))
				return S_OK;
...
HRESULT STDMETHODCALLTYPE CDropTargetProxy::DragEnter(IDataObject* pDataObj, DWORD grfKeyState, POINTL pt,
                                                      DWORD* pdwEffect)
{
