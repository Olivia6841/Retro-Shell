	...
	IResourceMap : public IUnknown
{
	...
	virtual HRESULT STDMETHODCALLTYPE GetStringForContext(IResourceContext* pContext, const wchar_t* propName,
	                                                      wchar_t* pString) = 0;
	virtual HRESULT STDMETHODCALLTYPE GetFilePath(const wchar_t* propName, wchar_t** pPath) = 0;
	virtual HRESULT STDMETHODCALLTYPE GetFilePathForContext(IResourceContext* pContext, const wchar_t* propName,
	                                                        wchar_t** pPath) = 0;
};
...
static HBITMAP BitmapFromMetroIcon(HICON hIcon, int bitmapSize, int iconSize, DWORD metroColor,
...
CItemManager::CItemManager(void)
{
	...
	m_StartEvent = m_WorkEvent = m_ExitEvent = m_DoneEvent = m_PreloadItemsThread = m_RefreshInfoThread =
	...
	m_bHasNewPrograms[0] = m_bHasNewPrograms[1] = m_bHasNewApps[0] = m_bHasNewApps[1] = m_bPreloadIcons =
		m_bPreloadFavorites = false;
...
void CItemManager::ResetTempIcons(void)
{
	...
	if (bResetMetro)
	{
		for (std::map<unsigned int, const ItemInfo*>::iterator it = m_MetroItemInfos10.begin(); it != m_MetroItemInfos10
		     .end();)
		{
			std::map<unsigned int, const ItemInfo*>::iterator next = it;
			++next;
	...
	}
	{
		...
		{
			std::list<ItemInfo*>::iterator next = it;
			++next;
			if ((*it)->bTemp)
			{
	...
	int metroFlags = bResetMetro ? INFO_METRO : 0;
	{
		...
		{
			std::multimap<unsigned int, ItemInfo>::iterator next = it;
			++next;
			if (it->second.bTemp)
			{
				Assert(
					it->second.largeIcon==m_DefaultLargeIcon && it->second.extraLargeIcon==m_DefaultExtraLargeIcon && (
						it->second.smallIcon==m_DefaultSmallIcon || it->second.smallIcon->bTemp));
				m_ItemInfos.erase(it);
			}
			else
			{
				if (it->second.smallIcon && (it->second.smallIcon->bTemp || (it->second.smallIcon->bMetro &&
					bResetMetro)))
				{
					it->second.smallIcon = m_DefaultSmallIcon;
					it->second.validFlags &= ~(INFO_SMALL_ICON | metroFlags);
				}
				if (it->second.mediumIcon && (it->second.mediumIcon->bTemp || (it->second.mediumIcon->bMetro &&
					bResetMetro)))
				{
					it->second.mediumIcon = m_DefaultMediumIcon;
					it->second.validFlags &= ~(INFO_MEDIUM_ICON | metroFlags);
				}
				if (it->second.largeIcon && (it->second.largeIcon->bTemp || (it->second.largeIcon->bMetro &&
					bResetMetro)))
				{
					it->second.largeIcon = m_DefaultLargeIcon;
					it->second.validFlags &= ~(INFO_LARGE_ICON | metroFlags);
				}
				if (it->second.extraLargeIcon && (it->second.extraLargeIcon->bTemp || (it->second.extraLargeIcon->bMetro
					&& bResetMetro)))
				{
...

	{
		...
		{
			std::multimap<unsigned int, IconInfo>::iterator next = it;
			++next;
...
const CItemManager::ItemInfo* CItemManager::GetItemInfo(IShellItem* pItem, PIDLIST_ABSOLUTE pidl, int refreshFlags,
                                                        TLocation location)
{
	...
	ItemInfo* pInfo = NULL;
	{
		...
		{
			if ((!PATH.IsEmpty() && wcscmp(PATH, it->second.PATH) == 0) || (PATH.IsEmpty() && ILIsEqual(
				pidl, it->second.GetPidl())))
			{
				pInfo = &it->second;
				break;
			}
...
const CItemManager::ItemInfo* CItemManager::GetCustomIcon(const wchar_t* location, int index,
                                                          TIconSizeType iconSizeType, bool bTemp)
{
	Assert(!RWLock::ThreadHasReadLock(RWLOCK_ITEMS));
...
const CItemManager::ItemInfo* CItemManager::GetLinkIcon(IShellLink* link, TIconSizeType iconSizeType)
{
	...
	if (link->GetIconLocation(location, _countof(location), &index) == S_OK && location[0])
		return GetCustomIcon(location, index, iconSizeType, (index == 0));
	// assuming that if index!=0 the icon comes from a permanent location like a dll or exe
	...
	if (store)
	{
		...
		static const PROPERTYKEY PKEY_AppUserModel_DestListLogoUri = {
...
void CItemManager::WaitForShortcuts(const POINT& balloonPos)
{
	if (m_PreloadItemsThreadId)
	{
		if (WaitForSingleObject(m_DoneEvent, 1000) != WAIT_TIMEOUT)
			return;
		CWindow tooltip = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | (IsLanguageRTL() ? WS_EX_LAYOUTRTL : 0),
		                                 TOOLTIPS_CLASS,NULL,WS_POPUP | TTS_BALLOON | TTS_NOPREFIX, 0, 0, 0, 0,NULL,
...
bool CItemManager::IsPathUsed(CRegKey& regKey, const wchar_t* path, const FILETIME& createstamp,
                              const KnownPathGuid* knownPaths, int knownPathsCount, bool bMetroApp)
{
	{
		unsigned int hash = CalcFNVHash(path);
		OldItemInfo key = {hash};
		std::vector<OldItemInfo>::const_iterator it = std::lower_bound(m_OldItemInfos.begin(), m_OldItemInfos.end(),
		                                                               key);
	...
	if (regKey.QueryBinaryValue(name, &data, &size) == ERROR_SUCCESS)
	{
		if (bMetroApp)
		{
			...
			return ((data.timestamp.dwLowDateTime | data.timestamp.dwHighDateTime) == 0 || CompareFileTime(
				&createstamp, &data.timestamp) < 0);
		}
		else
		{
...
void CItemManager::UpdateNewPrograms(const POINT& balloonPos)
{
	...
	*/
	...
	{
		CRegKey regKey;
		if (regKey.Open(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",KEY_READ) ==
			ERROR_SUCCESS)
		{
			DWORD time = 0;
		...
		SYSTEMTIME st;
		GetSystemTime(&st);
		LOG_MENU(LOG_NEW, L"Current time: %02d.%02d.%04d:%02d:%02d", st.wDay, st.wMonth, st.wYear, st.wHour,
		         st.wMinute);
		FileTimeToSystemTime(&ft, &st);
		LOG_MENU(LOG_NEW, L"Install time: %02d.%02d.%04d:%02d:%02d", st.wDay, st.wMonth, st.wYear, st.wHour,
		         st.wMinute);
	}
	...
	{
		RWLock lock(this, true, RWLOCK_ITEMS);
		for (std::multimap<unsigned int, ItemInfo>::const_iterator it = m_ItemInfos.begin(); it != m_ItemInfos.end(); ++
		     it)
		{
			...
			continue;
#endif
			LONGLONG timestamp = it->second.createstamp.dwLowDateTime | (((LONGLONG)it->second.createstamp.
				dwHighDateTime) << 32);
			...
			if (it->second.location == LOCATION_START_MENU)
			{
				HANDLE h = CreateFile(it->second.targetPATH,FILE_READ_ATTRIBUTES,
				...
				if (hours2 < 0)
				{
					LOG_MENU(LOG_NEW, L"Ignoring new: %s target change time too new - %d hours", it->second.path,
					         hours2);
					continue; // the exe is too old
				}
				if (hours2 > OLD_PROGRAMS_AGE)
				{
					LOG_MENU(LOG_NEW, L"Ignoring new: %s target change time too old - %d hours", it->second.path,
					         hours2);
					continue; // the exe is too old
				}
				...
				if (regKeyLink.m_hKey && IsPathUsed(regKeyLink, it->second.PATH, it->second.createstamp, knownPaths,
				                                    knownPathsCount, it->second.bMetroApp))
				{
			...
			if (it->second.location == LOCATION_METRO)
			{
				...
				CString appid = it->second.appid;
				appid.MakeUpper();
				if (regKeyExe.m_hKey && IsPathUsed(regKeyExe, appid, it->second.createstamp,NULL, 0,
				                                   it->second.bMetroApp))
				{
					LOG_MENU(LOG_NEW, L"Ignoring new: %s app id used after app was created", it->second.path);
					continue; // the exe was used after the shortcut was created
				}
				if (it->second.bLink && regKeyLink.m_hKey && IsPathUsed(regKeyLink, it->second.PATH,
				                                                        it->second.createstamp, knownPaths,
				                                                        knownPathsCount, it->second.bMetroApp))
				{
					LOG_MENU(LOG_NEW, L"Ignoring new: %s shortcut used after it was created", it->second.path);
					continue; // the shortcut was used after it was created
			...
			m_NewPrograms.push_back(&it->second);
			LOG_MENU(LOG_NEW,
			         L"Accepting new: highlighting %s, created %d hours, target changed %d hours, since install %d hours, %I64X",
			         it->second.path, hours1, hours2, hours3, timestamp);
		}
	}
...
bool CItemManager::IsNewProgram(PIDLIST_ABSOLUTE pidl, bool bFolder, bool bMetroApp)
{
	...
	if (!bMetroApp)
	{
		...
		bool bValid = false;
		for (std::vector<const ItemInfo*>::const_iterator it = m_NewProgramRoots.begin(); it != m_NewProgramRoots.end();
		     ++it)
		{
...
void CItemManager::RemoveNewItem(PIDLIST_ABSOLUTE pItem1, PIDLIST_ABSOLUTE pItem2, bool bFolder)
{
	std::vector<unsigned> hashes;
	{
		...
		for (size_t i = 0; i < m_NewPrograms.size(); i++)
		{
			...
			bool bFound = false;
			if (bFolder && (ILIsParent(pItem1, pInfo->GetPidl(),FALSE) || (pItem2 && ILIsParent(
				pItem2, pInfo->GetPidl(),FALSE))))
				bFound = true;
...
void CItemManager::AddOldItems(const std::vector<unsigned>& hashes)
{
	...
	if (m_OldItemInfos.empty())
		...
		regItems.SetBinaryValue(L"OldItems", &m_OldItemInfos[0],
		                        ULONG((size_t)m_OldItemInfos.size() * sizeof(OldItemInfo)));
}
...
void CItemManager::RefreshItemInfo(ItemInfo* pInfo, int refreshFlags, IShellItem* pItem0, bool bHasWriteLock)
{
	...
	bool bValidateIcons = !bStartScreen; // hack - don't mark the icon as valid, so we have to load it next time
	{
		...
		if (refreshFlags & INFO_DATA)
		{
			...
			if (pItem && SUCCEEDED(pItem->GetAttributes(SFGAO_LINK|SFGAO_FILESYSTEM,&flags)))
			{
						...
						if (SUCCEEDED(pLink->GetIDList(&newInfo.targetPidl)))
						{
							...
							if (SUCCEEDED(SHCreateItemFromIDList(newInfo.targetPidl, IID_PPV_ARGS(&target))))
							{
								...
								if (SUCCEEDED(target->BindToHandler(nullptr, BHID_PropertyStore, IID_PPV_ARGS(&store))))
								{
									...
									PROPVARIANT val;
									PropVariantInit(&val);
									if (SUCCEEDED(store->GetValue(PKEY_MetroAppLauncher, &val)) && (val.vt == VT_I4 ||
										val.vt == VT_UI4) && val.intVal)
									{
										newInfo.bLink = false;
				...
				if (pStore)
				{
					...
					if (!newInfo.appid.IsEmpty() && (refreshFlags & INFO_METRO) && (GetWinVersion() <= WIN_VER_WIN8 ||
						_wcsicmp(newInfo.appid,SEARCH_APP_ID) != 0))
					{
						PROPVARIANT val;
						PropVariantInit(&val);
						if (SUCCEEDED(pStore->GetValue(PKEY_MetroAppLauncher,&val)) && (val.vt == VT_I4 || val.vt ==
							VT_UI4) && val.intVal)
						{
							...
							if (newInfo.bMetroLink && wcsncmp(newInfo.PATH, m_RootCommonPrograms,
							                                  m_RootCommonPrograms.GetLength()) == 0)
							{
								const wchar_t* str = newInfo.PATH;
								newInfo.bProtectedLink = m_RootCommonPrograms.GetLength() == (PathFindFileName(str) -
									str);
							}
							if (SUCCEEDED(pStore->GetValue(PKEY_MetroIconColor,&val)) && (val.vt == VT_I4 || val.vt ==
								VT_UI4))
								newInfo.iconColor = val.intVal;
							if (pAppItem || SUCCEEDED(
								SHCreateItemInKnownFolder(FOLDERID_AppsFolder2,0,newInfo.appid,IID_IShellItem,(void**)&
									pAppItem)))
							{
								CComString pName;
					...
					if (!newInfo.bMetroLink && !newInfo.bMetroApp)
					{
						PROPVARIANT val;
						PropVariantInit(&val);
						if (SUCCEEDED(pStore->GetValue(PKEY_AppUserModel_PreventPinning,&val)) && val.vt == VT_BOOL &&
							val.boolVal)
							newInfo.bNoPin = true;
						PropVariantClear(&val);
						if (SUCCEEDED(pStore->GetValue(PKEY_AppUserModel_ExcludeFromShowInNewInstall,&val)) && val.vt ==
							VT_BOOL && val.boolVal)
							newInfo.bNoNew = true;
						PropVariantClear(&val);
					}
		...
		if (refreshFlags & INFO_ICON)
		{
			...
			if (newInfo.bIconOnly)
			{
				LoadCustomIcon(newInfo.iconPath, newInfo.iconIndex, refreshFlags & INFO_ICON, newInfo.smallIcon,
				               newInfo.mediumIcon, newInfo.largeIcon, newInfo.extraLargeIcon, newInfo.bTemp);
			}
			else if (newInfo.bMetroLink || newInfo.bMetroApp)
			{
				if (pAppItem || SUCCEEDED(
					SHCreateItemInKnownFolder(FOLDERID_AppsFolder2,0,newInfo.appid,IID_IShellItem,(void**)&pAppItem)))
				{
					int iconFlags = refreshFlags & INFO_ICON;
					LoadMetroIcon(pAppItem, iconFlags, newInfo.smallIcon, newInfo.mediumIcon, newInfo.largeIcon,
					              newInfo.extraLargeIcon, &newInfo.iconColor);
					if (iconFlags)
						LoadShellIcon(pItem ? pItem : pAppItem, iconFlags, newInfo.smallIcon, newInfo.mediumIcon,
						              newInfo.largeIcon, newInfo.extraLargeIcon, &newInfo.iconColor);
				}
			}
			else if (_wcsicmp(PathFindExtension(newInfo.path), L".settingcontent-ms") == 0)
			{
				...
				LoadCustomIcon(newInfo.iconPath, newInfo.iconIndex, refreshFlags & INFO_ICON, newInfo.smallIcon,
				               newInfo.mediumIcon, newInfo.largeIcon, newInfo.extraLargeIcon, false);
			}
			else
			{
				LoadShellIcon(pItem, (refreshFlags & INFO_ICON) | (bStartScreen ? INFO_STARTSCREEN_ICON : 0),
				              newInfo.smallIcon, newInfo.mediumIcon, newInfo.largeIcon, newInfo.extraLargeIcon,NULL);
			}
		}
	}
...
HBITMAP CItemManager::BitmapFromIcon(HICON hIcon, int iconSize, bool bDestroyIcon)
{
	...
	{
		pConverter->Initialize(pBitmap, GUID_WICPixelFormat32bppPBGRA, WICBitmapDitherTypeNone,NULL, 0,
	...
	else
	{
		...
		pConverter->Initialize(pScaler, GUID_WICPixelFormat32bppPBGRA, WICBitmapDitherTypeNone,NULL, 0,
		                       WICBitmapPaletteTypeMedianCut);
	}
...
HICON CItemManager::LoadShellIcon(int index, int iconSize)
{
	...
	{
		if (SUCCEEDED(data.m_pTempLists[sizeIndex]->ReplaceFromImageList(0,pImageList,index,NULL,0)) && SUCCEEDED(
			data.m_pTempLists[sizeIndex]->GetIcon(0,ILD_TRANSPARENT,&hIcon)))
		{
...
HICON CItemManager::LoadShellIcon(int iconSize, IExtractIcon* pExtractW, const wchar_t* location,
                                  IExtractIconA* pExtractA, const char* locationA, int index)
{
...
void CItemManager::LoadShellIcon(IShellItem* pItem, int refreshFlags, const IconInfo*& smallIcon,
                                 const IconInfo*& mediumIcon, const IconInfo*& largeIcon,
                                 const IconInfo*& extraLargeIcon, const DWORD* pMetroColor)
{
	if (!pItem)
		return;
	...
	if (refreshFlags & INFO_STARTSCREEN_ICON)
	{
		...
		if (SUCCEEDED(pItem->BindToHandler(NULL,BHID_SFUIObject,IID_IShellLink,(void**)&pLink)))
		{
			if (SUCCEEDED(pLink->GetIconLocation(location,_countof(location),&index)) && (!location[0] || _wcsicmp(
				PathFindFileName(location), L"StartScreen.exe") == 0))
			{
				...
				StoreInCache(hash,NULL, hSmallBitmap, hMediumBitmap, hLargeBitmap, hExtraLargeBitmap, refreshFlags,
				             smallIcon, mediumIcon, largeIcon, extraLargeIcon, false, false);
				return;
			}
		}
	}
	...
	if (pMetroColor && ParseMetroBitmapLocation(metroLocation))
	{
		...
		{
			metroLocation[1] = '#';
			StoreInCache(hash, metroLocation, hSmallBitmap, hMediumBitmap, hLargeBitmap, hExtraLargeBitmap,
			             refreshFlags, smallIcon, mediumIcon, largeIcon, extraLargeIcon, bTransient, true);
			return;
		}
	}
	...
	if (bNotFileName)
	{
		CAbsolutePidl pidl;
		SHFILEINFO info;
		if (SUCCEEDED(SHGetIDListFromObject(pItem,&pidl)) && SHGetFileInfo(
			(const wchar_t*)(PIDLIST_ABSOLUTE)pidl, 0, &info, sizeof(info),SHGFI_PIDL | SHGFI_SYSICONINDEX))
		{
			if (refreshFlags & INFO_SMALL_ICON)
	...
	{
		if (pMetroColor)
			hExtraLargeBitmap = BitmapFromMetroIcon(hExtraLargeIcon, EXTRA_LARGE_ICON_SIZE, extraLargeIconSize,
			                                        *pMetroColor);
		else
	...
	if (pMetroColor)
		location[1] = '#';
	StoreInCache(hash, bNotFileName ? NULL : location, hSmallBitmap, hMediumBitmap, hLargeBitmap, hExtraLargeBitmap,
	             refreshFlags, smallIcon, mediumIcon, largeIcon, extraLargeIcon, bTransient, false);
}
...
void CItemManager::LoadMetroIcon(IShellItem* pItem, int& refreshFlags, const IconInfo*& smallIcon,
                                 const IconInfo*& mediumIcon, const IconInfo*& largeIcon,
                                 const IconInfo*& extraLargeIcon, const DWORD* pMetroColor)
{
	unsigned int hash;
	...
	if ((refreshFlags & INFO_SMALL_ICON) && SetResContextTargetSize(pResContext, SMALL_ICON_SIZE))
	{
		...
		if (SUCCEEDED(pResMap->GetFilePath(iconName,&pLocation)))
		{
			...
			refreshFlags &= ~INFO_SMALL_ICON;
			StoreInCache(hash, L"", hSmallBitmap,NULL,NULL,NULL, INFO_SMALL_ICON, smallIcon, mediumIcon, largeIcon,
			             extraLargeIcon, false, true);
		}
	}
	if ((refreshFlags & INFO_MEDIUM_ICON) && SetResContextTargetSize(pResContext, MEDIUM_ICON_SIZE))
	{
		...
		if (SUCCEEDED(pResMap->GetFilePath(iconName, &pLocation)))
		{
			...
			refreshFlags &= ~INFO_MEDIUM_ICON;
			StoreInCache(hash, L"", NULL, hMediumBitmap, NULL, NULL, INFO_MEDIUM_ICON, smallIcon, mediumIcon, largeIcon,
			             extraLargeIcon, false, true);
		}
	}
	if ((refreshFlags & INFO_LARGE_ICON) && SetResContextTargetSize(pResContext, LARGE_ICON_SIZE))
	{
		...
		if (SUCCEEDED(pResMap->GetFilePath(iconName,&pLocation)))
		{
			...
			refreshFlags &= ~INFO_LARGE_ICON;
			StoreInCache(hash, L"",NULL,NULL, hLargeBitmap,NULL, INFO_LARGE_ICON, smallIcon, mediumIcon, largeIcon,
			             extraLargeIcon, false, true);
		}
	}
	if ((refreshFlags & INFO_EXTRA_LARGE_ICON) && SetResContextTargetSize(pResContext, EXTRA_LARGE_ICON_SIZE))
	{
		...
		if (SUCCEEDED(pResMap->GetFilePath(iconName,&pLocation)))
		{
			...
			refreshFlags &= ~INFO_EXTRA_LARGE_ICON;
			StoreInCache(hash, L"",NULL,NULL,NULL, hExtraLargeBitmap, INFO_EXTRA_LARGE_ICON, smallIcon, mediumIcon,
			             largeIcon, extraLargeIcon, false, true);
		}
	}
}

void CItemManager::FindInCache(unsigned int hash, int& refreshFlags, const IconInfo*& smallIcon,
...
void CItemManager::StoreInCache(unsigned int hash, const wchar_t* path, HBITMAP hSmallBitmap, HBITMAP hMediumBitmap,
                                HBITMAP hLargeBitmap, HBITMAP hExtraLargeBitmap, int refreshFlags,
                                const IconInfo*& smallIcon, const IconInfo*& mediumIcon, const IconInfo*& largeIcon,
                                const IconInfo*& extraLargeIcon, bool bTemp, bool bMetro)
{
...
void CItemManager::LoadCustomIcon(const wchar_t* iconPath, int iconIndex, int refreshFlags, const IconInfo*& smallIcon,
                                  const IconInfo*& mediumIcon, const IconInfo*& largeIcon,
                                  const IconInfo*& extraLargeIcon, bool bTemp)
{
	...
	auto ExtractIconAsBitmap = [&](int iconSize) -> HBITMAP
	{
		...
		if (!*iconPath)
			hIcon = (HICON)LoadImage(g_Instance, MAKEINTRESOURCE(-iconIndex), IMAGE_ICON, iconSize, iconSize,
			                         LR_DEFAULTCOLOR);
		else
	...
	StoreInCache(hash, bTemp ? nullptr : iconPath, hSmallBitmap, hMediumBitmap, hLargeBitmap, hExtraLargeBitmap,
	             refreshFlags, smallIcon, mediumIcon, largeIcon, extraLargeIcon, bTemp, false);
}
...
void CItemManager::LoadFolderItems(IShellItem* pFolder, int refreshFlags, int levels, TLocation location)
{
	...
	while (enumerator.GetNext(pChild, childPidl))
	{
		...
		{
			SFGAOF flags = 0;
			if (SUCCEEDED(pChild->GetAttributes(SFGAO_FOLDER|SFGAO_STREAM|SFGAO_LINK,&flags)) && (flags & (SFGAO_FOLDER
				| SFGAO_STREAM | SFGAO_LINK)) == SFGAO_FOLDER)
			{
...
{
	{
		FOLDERID_StartMenu, CItemManager::INFO_LINK | CItemManager::INFO_METRO | CItemManager::INFO_SMALL_ICON,
		MAX_FOLDER_LEVELS, CItemManager::LOCATION_START_MENU
	},
	{
		FOLDERID_CommonStartMenu, CItemManager::INFO_LINK | CItemManager::INFO_METRO | CItemManager::INFO_SMALL_ICON,
		MAX_FOLDER_LEVELS, CItemManager::LOCATION_START_MENU
	},
	...
	{
		FOLDERID_ClassicPinned, CItemManager::INFO_LINK | CItemManager::INFO_METRO | CItemManager::INFO_SMALL_ICON, 1,
		CItemManager::LOCATION_START_MENU
	},
...
void CItemManager::PreloadItemsThread(void)
{
	...
	while (1)
	{
		for (int i = 0; i < _countof(g_CacheFolders); i++)
		{
			...
			{
				handles[dirCount] = FindFirstChangeNotification(
					path,TRUE,
...
bool CItemManager::CompareModuleTimeStamp(const CString& PATH, const FILETIME& timestamp,
...
void CItemManager::LoadCacheFile(void)
{
	...
	if (tag == 'CLSH')
	{
		...
		if (tag == GetVersionEx(g_Instance, &build) && ReadCacheFile(file) == CACHE_FILE_VERSION)
		{
			...
			while (tag == 'ICON')
			{
				...
				if (size1 == SMALL_ICON_SIZE && size2 == MEDIUM_ICON_SIZE && size3 == LARGE_ICON_SIZE && size4 ==
					EXTRA_LARGE_ICON_SIZE && CompareModuleTimeStamp(info.PATH, info.timestamp, modules))
				{
			...
			while (!bError && tag == 'ITEM')
			{
				...
				info.extraLargeIcon = data.extraLargeIcon < (int)remapIcons.size()
					                      ? remapIcons[data.extraLargeIcon]
					                      : NULL;
	...
	{
		m_ItemInfos.clear();
		for (std::multimap<unsigned int, IconInfo>::const_iterator it = m_IconInfos.begin(); it != m_IconInfos.end(); ++
		     it)
		{
			if (it->second.bitmap)
				DeleteObject(it->second.bitmap);
...
DWORD CALLBACK CItemManager::SaveCacheFileThread(void* param)
{
	...
	std::vector<const std::pair<const unsigned int, IconInfo>*> iconInfos;
	{
		...
		for (std::multimap<unsigned int, IconInfo>::const_iterator it = pThis->m_IconInfos.begin(); it != pThis->
		     m_IconInfos.end(); ++it)
		{
	...
	std::vector<unsigned int> blackList;
	{
		...
		for (std::multimap<unsigned int, ItemInfo>::const_iterator it = pThis->m_ItemInfos.begin(); it != pThis->
		     m_ItemInfos.end(); ++it)
		{
			if (it->first != 0)
				itemInfos.push_back(&*it);
		}
		for (std::set<unsigned int>::const_iterator it = pThis->m_BlackListInfos10.begin(); it != pThis->
		     m_BlackListInfos10.end(); ++it)
	...
	for (std::vector<const std::pair<const unsigned int, IconInfo>*>::const_iterator it = iconInfos.begin(); it !=
	     iconInfos.end(); ++it)
	{
		RWLock lock(pThis, false, RWLOCK_ICONS);
	...
	for (std::vector<const std::pair<const unsigned int, ItemInfo>*>::const_iterator it = itemInfos.begin(); it !=
	     itemInfos.end(); ++it)
	{
		RWLock lock(pThis, false, RWLOCK_ITEMS);
...
static CString GetKnownFolderSetting(REFKNOWNFOLDERID rfid)
{
	...
	else if (rfid == FOLDERID_Favorites)
	{
		if (IsEdgeDefaultBrowser())
			Strcpy(path,_countof(path),
			       L"%LOCALAPPDATA%\\Packages\\Microsoft.MicrosoftEdge_8wekyb3d8bbwe\\AC\\MicrosoftEdge\\User\\Default\\Favorites");
	}
	if (!path[0])
		return CString();
	DoEnvironmentSubst(path,_countof(path));
