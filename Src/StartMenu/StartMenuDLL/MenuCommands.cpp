...
static bool SetShutdownPrivileges()
{
	...
	if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken))
	{
		...
		if (LookupPrivilegeValue(NULL, L"SeShutdownPrivilege", &tp.Privileges[0].Luid))
		{
			...
			if (AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), NULL, NULL) && GetLastError() ==
				ERROR_SUCCESS)
				retval = true;
		}
		CloseHandle(hToken);
...
static void DoSearchSubst(wchar_t* buf, int size, const wchar_t* search)
{
	...
	wchar_t* pBuf = buf;
	FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_ARGUMENT_ARRAY | FORMAT_MESSAGE_FROM_STRING, buf, 0,
	              0, (LPWSTR)&pBuf, 0, (va_list*)args);
...
static DWORD WINAPI NewShortcutThread(void* param)
{
	...
	HANDLE hFile = CreateFile(pParams->fname, 0,FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,NULL,
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		if (CreateProcess(exe, cmdLine,NULL,NULL,FALSE, 0,NULL, pParams->temp, &startupInfo, &processInfo))
		{
			...
			if (bInfo)
			{
				...
				int len = Sprintf(pParams->fname,_countof(pParams->fname) - 1, L"%s\\%s", pParams->temp,
				                  PathFindFileName(nameInfo.FileName));
...
DWORD WindowsUpdateAdjustShutdownFlags(DWORD flags)
{
...

	{
		...
		CRegKey key;
		if (key.Open(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\WindowsUpdate\\Orchestrator", KEY_READ) !=
			ERROR_SUCCESS)
			return retval;
	...
	static const GUID CLSID_UpdateSessionOrchestrator = {
		0xb91d5831, 0xb1bd, 0x4608, {0x81, 0x98, 0xd7, 0x2e, 0x15, 0x50, 0x20, 0xf7}
	};

	CComPtr<IUpdateSessionOrchestrator> updateSessionOrchestrator;
	if (SUCCEEDED(
		updateSessionOrchestrator.CoCreateInstance(CLSID_UpdateSessionOrchestrator, nullptr, CLSCTX_LOCAL_SERVER)))
	{
...
static TOKEN_ELEVATION_TYPE GetCurrentTokenElevationType()
{
	...
	if (OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &token))
	{
		...
		DWORD returnLength;
		if (GetTokenInformation(token, TokenElevationType, &elevationType, sizeof(elevationType), &returnLength) &&
			returnLength == sizeof(elevationType))
...
{
	static auto p = static_cast<decltype(&WinStationGetLoggedOnCount)>((void*)GetProcAddress(
		GetModuleHandle(L"winsta.dll"), "WinStationGetLoggedOnCount"));
	if (p)
...
static bool ProceedWithShutdown(DWORD flags)
{
	...
	return MessageBox(NULL, message, L"Retro-Shell",
	                  MB_YESNO | MB_ICONEXCLAMATION | MB_DEFBUTTON1 | MB_SYSTEMMODAL | MB_SETFOREGROUND |
	                  MB_SERVICE_NOTIFICATION) != IDNO;
}
...
static bool ExecuteShutdownCommand(TMenuID menuCommand)
{
	...
	case MENU_SHUTDOWN_HYBRID: // hybrid shutdown
		...
		{
			CRegKey regPower;
			if (regPower.Open(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Power",
...
static bool ConnectedStandby()
{
	...
	if (powerCaps.AoAc)
	{
		static auto pNtPowerInformation = static_cast<decltype(&NtPowerInformation)>((void*)GetProcAddress(
			GetModuleHandle(L"ntdll.dll"), "NtPowerInformation"));
...
static bool ExecuteSysCommand(TMenuID menuCommand)
{
	...
	case MENU_SWITCHUSER: // switch_user
		if (GetWinVersion() >= WIN_VER_WIN10)
		{
			...
			if (regSwitch.Create(
				HKEY_LOCAL_MACHINE,
				L"Software\\Microsoft\\Windows\\CurrentVersion\\Authentication\\LogonUI\\UserSwitch",NULL, 0,
...
static HRESULT CreatePinLink(PCIDLIST_ABSOLUTE sourcePidl, const wchar_t* name, const wchar_t* iconPath, int iconIndex)
{
	...
	HANDLE h = CreateFile(finalPath,GENERIC_WRITE,FILE_SHARE_READ | FILE_SHARE_WRITE,NULL,OPEN_EXISTING,
...
void CMenuContainer::ActivateItem(int index, TActivateType type, const POINT* pPt, ActivateData* pData)
{
	...
	bool bKeepOpen = (type == ACTIVATE_EXECUTE) && bShift && !bCtrl && (!item.bMetroLink || GetWinVersion() >=
		WIN_VER_WIN10);
	...
	if (s_RecentPrograms != RECENT_PROGRAMS_NONE)
	{
		if (item.id == MENU_RECENT)
			...
			bTrackRecent = item.id == MENU_NO && (!item.bFolder || item.bHasJumpList) && !item.pStdItem && (item.
				categoryHash & 15) <= CSearchManager::CATEGORY_SETTING;
	}
	...
	if (type == ACTIVATE_EXECUTE)
	{
		...
		if (item.id == MENU_SEARCH_CATEGORY)
		{
			...
			{
				m_SearchCategoryHash = (m_SearchCategoryHash == item.categoryHash)
					                       ? CSearchManager::CATEGORY_INVALID
					                       : item.categoryHash;
				RefreshSearch();
			}
			return;
		...
		if (item.jumpIndex >= 0)
		{
			if (item.id == MENU_NO)
			{
				...
				PlayMenuSound(SOUND_COMMAND);
				ExecuteJumpItem(s_JumpAppInfo, s_JumpList.groups[LOWORD(item.jumpIndex)].items[HIWORD(item.jumpIndex)],
				                g_OwnerWindow);
			}
			return;
		}
		...
		if (!bKeepOpen)
		{
			...
			else
			{
				...
				// hack: prevents any other closing behavior to occur while the item is being executed
				CloseSubMenus(CLOSE_POST,NULL);
	...
	bool bCommand = (item.pStdItem && item.pStdItem->command && *item.pStdItem->command) || item.id ==
		MENU_SEARCH_EXECUTE || item.bStartScreen || !pItemPidl1; // this is a special executable command
	...
	if (pItemPidl1)
		bHasMenu = true;
	else if (type == ACTIVATE_MENU && (item.id == MENU_EMPTY || item.id == MENU_EMPTY_TOP || item.id ==
		MENU_SEARCH_CATEGORY))
		bHasMenu = true;
	else if (type == ACTIVATE_MENU && item.id == MENU_APPS && (g_ItemManager.HasNewApps(true) || (pData && pData->
		bProgramsTree)))
		bHasMenu = true;
	if (!bHasMenu)
		return;
	...
	else if (item.id == MENU_EMPTY || item.id == MENU_EMPTY_TOP || item.id == MENU_SEARCH_CATEGORY || type ==
		ACTIVATE_MENU_BACKGROUND)
	{
		insertBefore = 0;
	}
	else
	{
		...
		if (item.jumpIndex >= 0)
		{
			...
			bQueryMenu = ((type == ACTIVATE_MENU || type == ACTIVATE_PROPERTIES) && (jumpItem.type ==
					CJumpItem::TYPE_ITEM || (jumpItem.type == CJumpItem::TYPE_LINK && !jumpItem.bHasArguments)) && !
				_path.
				IsEmpty());
		}
		if (bQueryMenu)
		{
			...
			if (pItem2 &&
				...
				// don't update search items because we don't have the right bind context for them
				pItem2->Update(NULL);
		...
		if (item.bFolder && pItemPidl2)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				char command[256];
				if (id < CMD_LAST || id > CMD_MAX || FAILED(
					pMenu->GetCommandString(id-CMD_LAST,GCS_VERBA,NULL,command,_countof(command))))
					command[0] = 0;
				if (_stricmp(command, "open") == 0)
				{
					if (GetSettingBool(L"EnableExplorer"))
					{
						if (!s_bNoCommonFolders)
							InsertMenu(menu, i + 1,MF_BYPOSITION | MF_STRING, CMD_OPEN_ALL,
		...
		else if (type == ACTIVATE_MENU && item.id == MENU_RECENT)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				else
				{
					...
					{
						if (_stricmp(command, "open") == 0 || _stricmp(command, "opencontaining") == 0 ||
							_stricmp(command, "runas") == 0 || _stricmp(command, "runasuser") == 0
							|| _stricmp(command, "taskbarpin") == 0 || _stricmp(command, "taskbarunpin") == 0 ||
							_stricmp(command, "pin_classic") == 0 || _stricmp(command, "properties") == 0)
							continue;
			...
			if (item.bMetroLink)
			{
				...
				{
					InsertMenu(menu, insertBefore,MF_BYPOSITION | MF_STRING, CMD_UNINSTALL,
					           FindTranslation(L"Menu.Uninstall", L"&Uninstall"));
					insertBefore++;
				}
				insertSecondary = 1;
				n++;
			...
			InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_DELETEMRU,
			           FindTranslation(L"Menu.RemoveList", L"Remove &from this list"));
			if (s_RecentPrograms == RECENT_PROGRAMS_RECENT)
				InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_DELETEALL,
				           FindTranslation(L"Menu.RemoveAll", L"C&lear recent items list"));
			if (pItemPidl1 && GetSettingBool(L"EnableExplorer"))
				InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_EXPLORE,
				           FindTranslation(L"Menu.Explore", L"&Explore"));
			InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_SEPARATOR, 0, 0);
		}
		else if (type == ACTIVATE_MENU && item.bMetroLink && (_bProtectedLink || (m_Options & (CONTAINER_APPS |
			CONTAINER_SEARCH))))
		{
		...
		else if (type == ACTIVATE_MENU && ((m_Options & CONTAINER_SEARCH) || (s_MenuMode == MODE_SEARCH && index >
			m_OriginalCount)))
		{
			...
			CSearchManager::TItemCategory cat = (CSearchManager::TItemCategory)(item.categoryHash &
				CSearchManager::CATEGORY_MASK);
			if (pItemPidl1 && (cat <= CSearchManager::CATEGORY_FILE || cat == CSearchManager::CATEGORY_AUTOCOMPLETE))
			{
				...
				bool bPin = !s_PinFolder.IsEmpty() && (cat == CSearchManager::CATEGORY_SETTING || cat ==
					CSearchManager::CATEGORY_METROSETTING);
				if (bExplore || bPin)
				{
					if (n > 0)
						InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_SEPARATOR, 0, 0);
					if (bPin)
						InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_PINSETTING,
						           FindTranslation(L"Menu.PinStartCs", L"Pin to Start menu (Retro-Shell)"));
					if (bExplore)
						InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_EXPLORE,
						           FindTranslation(L"Menu.Explore", L"&Explore"));
					if (!last)
		...
		else if (item.jumpIndex >= 0)
		{
			...
			InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_OPEN,
			           FindTranslation(L"Menu.Open", L"&Open"));
			SetMenuDefaultItem(menu, 0,TRUE);
			if (group.type != CJumpGroup::TYPE_TASKS)
			{
				InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_SEPARATOR, 0, 0);
				if (group.type == CJumpGroup::TYPE_PINNED)
					InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_PIN,
					           FindTranslation(L"JumpList.Unpin", L"&Unpin from this list"));
				else
				{
					InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_PIN,
					           FindTranslation(L"JumpList.Pin", L"P&in to this list"));
					InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_DELETEMRU,
					           FindTranslation(L"JumpList.Remove", L"Remove &from this list"));
				}
				if (n > 0)
				{
					InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_SEPARATOR, 0, 0);
		...
		else
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				else if (item.bStartScreen)
				{
					if (_stricmp(command, "rename") == 0 || _stricmp(command, "delete") == 0)
						...
						EnableMenuItem(menu, i,MF_GRAYED | MF_BYPOSITION);
					else if (_stricmp(command, "open") == 0 || _stricmp(command, "opencontaining") == 0 || _stricmp(
						command, "runas") == 0)
						bDelete = true;
				}
				if (bDelete)
			...
			else if (item.bMetroApp && !bHasUninstall && bUninstallPolicy && IsWin10RS1() &&
				CanUninstallMetroApp(_appId))
			{
				InsertMenu(menu, insertBefore,MF_BYPOSITION | MF_STRING, CMD_UNINSTALL,
				           FindTranslation(L"Menu.Uninstall", L"&Uninstall"));
				insertBefore++;
	...
	if (type == ACTIVATE_EXECUTE)
	{
		...
		if (bShift && bCtrl)
		{
			...
			{
				int id = GetMenuItemID(menu, i);
				if (id >= CMD_LAST && id <= CMD_MAX && SUCCEEDED(
					pMenu->GetCommandString(id-CMD_LAST,GCS_VERBA,NULL,command,_countof(command))))
				{
	...
	else if (type == ACTIVATE_RENAME || type == ACTIVATE_DELETE || type == ACTIVATE_PROPERTIES)
	{
		if ((type == ACTIVATE_RENAME || type == ACTIVATE_DELETE) && item.bStartScreen)
			...
			res = CMD_DELETEMRU;
		else if ((type == ACTIVATE_RENAME || type == ACTIVATE_DELETE) && item.bMetroLink && (_bProtectedLink || (
			m_Options & (CONTAINER_APPS | CONTAINER_SEARCH))))
			res = 0;
		else
		{
			const char* name;
			switch (type)
			{
			case ACTIVATE_RENAME: name = "rename";
				break;
			case ACTIVATE_DELETE: name = "delete";
				break;
			case ACTIVATE_PROPERTIES: name = "properties";
				break;
			}
			char command[256];
			int n = GetMenuItemCount(menu);
	...
	else
	{
		if (!GetSettingBool(L"EnableExplorer"))
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				if (id >= CMD_LAST && id <= CMD_MAX && SUCCEEDED(
					pMenu->GetCommandString(id-CMD_LAST,GCS_VERBA,NULL,command,_countof(command))))
				{
					if ((item.bFolder && !item.bSplit && pItemPidl1 && _stricmp(command, "open") == 0) || _stricmp(
						command, "opencontaining") == 0)
					{
		...
		if ((item.id == MENU_NO || item.id == MENU_EMPTY || type == ACTIVATE_MENU_BACKGROUND || (item.id == MENU_APPS &&
			pData && pData->bProgramsTree)) && item.jumpIndex < 0 && index < m_OriginalCount && (m_Options &
			CONTAINER_DROP)) // clicked on a movable item or the background
		{
			HMENU menu2 = menu;
			int subMenuIdx = -1;
			InsertMenu(menu, insertBefore++,MF_BYPOSITION | MF_SEPARATOR, 0, 0);
			...
			{
				bNew = !(item.pItemInfo && _location == CItemManager::LOCATION_METRO) && !pData->bApps &&
					GetSettingBool(L"ShowNewFolder");
				bSort = true;
			...
			else
			{
				...
				wchar_t path[_MAX_PATH];
				if (!(m_Options & CONTAINER_APPS) && !(item.pItemInfo && _location == CItemManager::LOCATION_METRO) && !
					(m_Options & CONTAINER_NONEWFOLDER) && GetSettingBool(L"ShowNewFolder") && SHGetPathFromIDList(
						m_Path1[item.priority == 2 ? 1 : 0], path))
					bNew = true;
			...
			if (bSort)
				InsertMenu(menu2, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_SORT,
				           FindTranslation(L"Menu.SortByName", L"Sort &by Name"));

			if (m_FolderHash[0])
				InsertMenu(menu2, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_AUTOSORT,
				           FindTranslation(L"Menu.AutoArrange", L"&Auto Arrange"));
			if (bAutoSort)
			{
			...
			if (bNew)
			{
				InsertMenu(menu2, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_NEWFOLDER,
				           FindTranslation(L"Menu.NewFolder", L"New Folder"));
				InsertMenu(menu2, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_NEWSHORTCUT,
				           FindTranslation(L"Menu.NewShortcut", L"New Shortcut"));
			}
			if (bMarkOld)
				InsertMenu(menu2, insertBefore++,MF_BYPOSITION | MF_STRING, CMD_MARKOLD,
				           FindTranslation(L"Menu.RemoveHighlight", L"Remove highlight"));
			...
			if (bNew || menu != menu2)
			{
				...
				{
					HMODULE hShell32 = GetModuleHandle(L"shell32.dll");
					HICON hIcon = (HICON)LoadImage(hShell32,MAKEINTRESOURCE(319),IMAGE_ICON, size, size,
					                               LR_DEFAULTCOLOR);
					if (hIcon)
					{
				...
				if (menu != menu2)
				{
					InsertMenu(menu, subMenuIdx,MF_BYPOSITION | MF_POPUP, (UINT_PTR)menu2,
					           FindTranslation(L"Menu.Organize", L"Organize Start menu"));
					HICON hIcon = (HICON)LoadImage(g_Instance,MAKEINTRESOURCE(IDI_APPICON),IMAGE_ICON, size, size,
					                               LR_DEFAULTCOLOR);
					if (hIcon)
					{
		...
		if (item.id == MENU_SEARCH_CATEGORY)
		{
			InsertMenu(menu, 0,MF_BYPOSITION | MF_STRING, CMD_TOGGLE,
			           item.categoryHash == m_SearchCategoryHash
				           ? FindTranslation(L"Menu.Collapse", L"Coll&apse")
				           : FindTranslation(L"Menu.Expand", L"Exp&and"));
			if (item.categoryHash >= CSearchManager::CATEGORY_FILE)
			{
				wchar_t text[256];
				Sprintf(text,_countof(text), L"%s\t(Ctrl+Enter)",
				        FindTranslation(L"Menu.MoreResults", L"See more results"));
		...
		if (GetMenuItemCount(menu) > 0)
		{
			...
			res = TrackPopupMenuEx(
				menu,TPM_RIGHTBUTTON | TPM_RETURNCMD | TPM_VERTICAL | (IsLanguageRTL() ? TPM_LAYOUTRTL : 0), pt2.x,
				pt2.y, m_hWnd, pParams);
	...
	if (res == CMD_SORT)
	{
		...
		{
			m_pProgramsTree->OrderElements(pData->hTreeItem,
			                               TreeView_GetParent(m_pProgramsTree->m_hWnd, pData->hTreeItem),
			                               std::vector<unsigned int>(), false, true);
		}
		else
		{
	...
	if (res == CMD_MARKOLD)
	{
		...
		{
			g_ItemManager.RemoveNewItems(
				true, s_bWin7Style && GetWinVersion() >= WIN_VER_WIN8 && GetSettingBool(L"AllProgramsMetro"));
			if (m_pProgramsTree)
	...
	if (res == CMD_AUTOSORT)
	{
		if (pData && pData->bProgramsTree)
		{
			m_pProgramsTree->OrderElements(pData->hTreeItem,
			                               TreeView_GetParent(m_pProgramsTree->m_hWnd, pData->hTreeItem),
			                               std::vector<unsigned int>(), !pData->bAutoSort, true);
		}
		else
		{
			if (m_FolderHash[0])
	...
	if (res == CMD_NEWFOLDER)
	{
		...
		// have to use IShellFolder for renaming because it's the only one that supports changing the display name
		if (pItemPidl1)
		{
		...
		if (pFolder)
		{
			...
			s_bPreventClosing = true;
			if (newPidl && DialogBox(g_Instance, MAKEINTRESOURCE(s_bRTL?IDD_RENAMER:IDD_RENAME), g_OwnerWindow,
			                         RenameDlgProc))
			{
	...
	if (res >= CMD_LAST)
	{
		...
		if (_stricmp(command, "rename") == 0)
		{
			...
			s_bPreventClosing = true;
			STRRET str;
			if (SUCCEEDED(SHBindToParent(pItemPidl1,IID_IShellFolder,(void**)&pFolder,&pidl)) && SUCCEEDED(
				pFolder->GetDisplayNameOf(pidl,SHGDN_FOREDITING,&str)))
			{
				CComString pName;
			...
			SetContextItem(index);
			InvalidateItem(index);
			bool bRenamed = DialogBox(g_Instance, MAKEINTRESOURCE(s_bRTL?IDD_RENAMER:IDD_RENAME), g_OwnerWindow,
			                          RenameDlgProc) != 0;
			SetContextItem(-1);
			if (m_HotItem < 0) SetHotItem(index);
			...
			if (bRenamed)
			{
				...
				if (SUCCEEDED(pFolder->SetNameOf(g_OwnerWindow,pidl,g_RenameText,SHGDN_INFOLDER,&newPidl)))
				{
					...
					if (SUCCEEDED(pFolder->GetDisplayNameOf(newPidl,SHGDN_INFOLDER|SHGDN_FORPARSING,&str)))
					{
						...
						if (!m_bSubMenu && item.id == MENU_NO && GetWinVersion() <= WIN_VER_WIN7 && !_appId.IsEmpty() &&
							_bIsLink && !_bExplicitAppId && SUCCEEDED(
								pFolder->GetDisplayNameOf(newPidl,SHGDN_FORPARSING,&str)))
						{
							CComString pPath;
							...
							if (pFile && SUCCEEDED(pFile->Load(pPath,STGM_READWRITE)))
							{
								...
								if (pStore)
								{
									...
									if (SUCCEEDED(pStore->SetValue(PKEY_AppUserModel_ID,val)) && SUCCEEDED(
										pStore->Commit()))
						...
						if (pData)
						{
							CComPtr<IShellItem> pNewItem;
							if (SUCCEEDED(
								SHCreateItemWithParent(NULL,pFolder,newPidl,IID_IShellItem,(void**)&pNewItem)))
							{
								CAbsolutePidl newAbsPidl;
								if (SUCCEEDED(SHGetIDListFromObject(pNewItem,&newAbsPidl)))
									pData->pNewItemInfo = g_ItemManager.GetItemInfo(pNewItem, newAbsPidl, 0);
		...
		else
		{
			...
			{
				if (_stricmp(command, "open") == 0 && GetSettingBool(L"OpenTruePath") && GetFakeFolder(
					targetlnkPath,_countof(targetlnkPath), dir))
					bOpenTruePath = true;
				else
				{
			...
			if (type == ACTIVATE_EXECUTE && executeSuccess)
			{
				...
				if (!(m_Options & CONTAINER_LINK))
				{
					...
					else if (SUCCEEDED(pItem->GetDisplayName(SIGDN_PARENTRELATIVEPARSING,&pName)))
					// can't use item.name because the extension may be removed
					{
						pName.MakeUpper();
...
void CMenuContainer::ActivateTreeItem(const void* treeItem, RECT& itemRect, TActivateType type, const POINT* pPt,
                                      ActivateData* pData)
{
...
void CMenuFader::Create(void)
{
	...
	CWindowImpl<CMenuFader>::Create(NULL, &m_Rect,NULL,WS_POPUP,
	                                WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_LAYERED | (bRtl ? WS_EX_LAYOUTRTL : 0));
