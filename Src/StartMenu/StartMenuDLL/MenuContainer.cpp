...
static StdMenuOption g_StdOptions[] =
{
	...
	{MENU_FEATURES, MENU_ENABLED},
	// no setting (prevents the Programs and Features from expanding), check policy (for control panel)
...
void CUserWindow::Init(CMenuContainer* pOwner)
{
	...
	m_bUserBitmapMask = (skin.User_mask.bIsBitmap && skin.User_mask.bIs32 && skin.User_maskSize.cx == skin.
		User_image_size && skin.User_maskSize.cy == skin.User_image_size);
...
void CUserWindow::SetImage(HBITMAP bmp, bool bAnimate)
{
	...
	else
	{
		if (m_UserBitmap)
		{
			...
			BitBlt(hdst, CMenuContainer::s_Skin.User_image_offset.x, CMenuContainer::s_Skin.User_image_offset.y,
			       iconSize, iconSize, hsrc, 0, 0,SRCCOPY);
		}
...
int CMenuContainer::s_BackgroundW1, CMenuContainer::s_BackgroundW2, CMenuContainer::s_BackgroundH1,
    CMenuContainer::s_BackgroundH2;
...
LRESULT CALLBACK CMenuContainer::SubclassSearchBox(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                   UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	if (uMsg == WM_PRINTCLIENT)
	{
		...
		HDC hdc = (HDC)wParam;
		if ((lParam & PRF_CLIENT) && ::GetWindowTextLength(hWnd) == 0 && ((GetSettingInt(L"SearchBox") ==
			SEARCHBOX_NORMAL && GetSettingBool(L"SearchSelect")) || GetFocus() != hWnd) && pParent->m_SearchIndex >= 0)
		{
			RECT rc;
			...
			if (GetSettingBool(L"SearchHint"))
				DrawText(hdc, GetSettingString(L"SearchHintText"), -1, &rc,
				         DT_SINGLELINE | DT_EDITCONTROL | (s_bRTL ? DT_RIGHT : DT_LEFT));
			else
				DrawText(hdc, pParent->m_Items[pParent->m_SearchIndex].name, -1, &rc,
				         DT_SINGLELINE | DT_EDITCONTROL | (s_bRTL ? DT_RIGHT : DT_LEFT));
		...
		if (wParam == VK_RETURN)
		{
			...
			if ((pSearchMenu->m_Options & CONTAINER_SEARCH) || (!pSearchMenu->m_bSubMenu && s_MenuMode == MODE_SEARCH))
			{
				if (pSearchMenu->m_HotItem >= 0 && pSearchMenu->m_HotItem < (int)pSearchMenu->m_Items.size() &&
					pSearchMenu->m_Items[pSearchMenu->m_HotItem].id != MENU_EMPTY && pSearchMenu->m_Items[pSearchMenu->
						m_HotItem].id != MENU_SEARCH_EMPTY)
				{
	...
	if (uMsg == WM_CHAR && (wParam == VK_RETURN || wParam == 10 || wParam == VK_ESCAPE || wParam == VK_TAB || wParam ==
		127))
	{
...
LRESULT CALLBACK CMenuContainer::SubclassScrollbar(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                   UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	CMenuContainer* pParent = (CMenuContainer*)uIdSubclass;
...
CMenuContainer::CMenuContainer(CMenuContainer* pParent, int index, int options, const StdMenuItem* pStdItem,
                               PIDLIST_ABSOLUTE path1, PIDLIST_ABSOLUTE path2)
{
	m_RefCount = 1;
	m_bSubMenu = (index >= 0);
	// this may be true even if pParent is NULL (in case you want to show only sub-menus somewhere, use index=0 and pParent=NULL)
	...
	if (std::find(s_Menus.begin(), s_Menus.end(), m_pParent) != s_Menus.end()) // check if the parent is still alive
	{
		if (m_pParent->m_Submenu == m_ParentIndex)
		{
			if (!m_pParent->m_bDestroyed)
			{
				...
				if (m_pParent->m_HotItem < 0 && !(m_Options & CONTAINER_SEARCH) && (!m_pParent->m_pProgramsTree || !
					m_pParent->m_pProgramsTree->m_hWnd || m_pParent->m_pProgramsTree->m_hWnd != GetFocus()))
	...
	while (enumerator.GetNext(pChild, childPidl))
	{
		...
		if (SUCCEEDED(pChild->GetDisplayName(SIGDN_PARENTRELATIVEPARSING,&pName)))
		{
			...
			else
			{
				...
				{
					if ((options & CONTAINER_CONTROLPANEL) && (_wcsnicmp(
						pName2, L"::{26EE0668-A00A-44D7-9371-BEB064C98683}\\0", 42) == 0 || Strlen(pName2) == 0))
						continue;
					// if the display name starts with the control panel GUID, something's wrong (like the Intel crap)
					pName.MakeUpper();
					item.nameHash = CalcFNVHash(pName);
					pName.Clear();
				...
				item.bFolder = (!(options & CONTAINER_CONTROLPANEL) && !(options & CONTAINER_NOSUBFOLDERS) && (flags &
					SFGAO_FOLDER) && (!(flags & (SFGAO_STREAM | SFGAO_LINK)) || (s_bExpandLinks && item.bLink)));
				{
					CItemManager::RWLock lock(&g_ItemManager, false, CItemManager::RWLOCK_ITEMS);
					if (item.pItemInfo->IsMetroLink())
					{
						if ((m_bSubMenu && GetSettingBool(L"HideProgramsMetro")) || _wcsicmp(
							item.pItemInfo->GetAppid(),DESKTOP_APP_ID) == 0)
							continue;
...
void CMenuContainer::AddStandardItems(void)
{
	if (m_pStdItem && m_pStdItem->id != MENU_NO)
	{
		...
		for (const StdMenuItem* pStdItem = m_pStdItem;; pStdItem++)
		{
			...
			if (s_bWin7Style && m_bSubMenu && pStdItem->command && (pStdItem->id == MENU_SWITCHUSER || pStdItem->id ==
				MENU_SECURITY || pStdItem->id == MENU_LOGOFF || pStdItem->id == MENU_LOCK || pStdItem->id ==
				MENU_DISCONNECT
				|| pStdItem->id == MENU_UNDOCK || pStdItem->id == MENU_RESTART || pStdItem->id == MENU_SLEEP || pStdItem
				->id == MENU_HIBERNATE || pStdItem->id == MENU_SHUTDOWN
			...
			else if (pStdItem->link)
			{
				...
				if (!bLibrary && second && !(item.pStdItem->settings & StdMenuItem::MENU_NOEXPAND))
				{
					...
					{
						item.bLink = (flags & SFGAO_LINK) != 0;
						item.bFolder = ((flags & SFGAO_FOLDER) && (!(flags & (SFGAO_STREAM | SFGAO_LINK)) || (
							s_bExpandLinks && item.bLink)));
					}
				}
				else if (SUCCEEDED(MenuParseDisplayName(buf,&item.pItem1,&flags,&networkType)))
				{
					if (bLibrary) flags &= ~SFGAO_STREAM;
					item.bLink = (flags & SFGAO_LINK) != 0;
					item.bFolder = ((flags & SFGAO_FOLDER) && !(item.pStdItem->settings & StdMenuItem::MENU_NOEXPAND) &&
						(!(flags & (SFGAO_STREAM | SFGAO_LINK)) || (s_bExpandLinks && item.bLink)));
				}
			}
			if ((pStdItem->submenu && pStdItem->id != MENU_SEARCH_BOX && (stdOptions & MENU_EXPANDED)) || pStdItem->id
			...
			else if (pStdItem->label)
			{
				...
				else if (item.id == MENU_SHUTDOWN_BUTTON)
				{
					...
					if (shutdown == SHUTDOWN_TYPE_SHUTDOWN)
					{
						...
						if (s_bHasUpdates)
							const_cast<StdMenuItem*>(item.pStdItem)->tip = FindTranslation(
								L"Menu.ShutdownUpdate", L"Update and shut down");
						else
					...
					if (shutdown == SHUTDOWN_TYPE_RESTART)
					{
						...
						if (s_bHasUpdates && GetWinVersion() >= WIN_VER_WIN8)
							const_cast<StdMenuItem*>(item.pStdItem)->tip = FindTranslation(
								L"Menu.RestartUpdate", L"Update and restart");
						else
			...
			{
				if ((pStdItem->command && wcscmp(pStdItem->command, L"<blank>") != 0) || pStdItem->link || pStdItem->
					folder1 || pStdItem->submenu || pStdItem->id != MENU_CUSTOM)
...
struct UserAssistItem
{
	...
	bool operator<(const UserAssistItem& item)
	{
		return (rank > item.rank) || (rank == item.rank && CompareFileTime(&timestamp, &item.timestamp) > 0);
	}
};
...
void CMenuContainer::GetRecentPrograms(std::vector<MenuItem>& items, int maxCount)
{
	...
	int iconSizeFlag = (!m_bSubMenu && s_Skin.Main_icon_size == MenuSkin::ICON_SIZE_LARGE)
		                   ? CItemManager::INFO_LARGE_ICON
	...
	if (s_RecentPrograms == RECENT_PROGRAMS_RECENT)
	{
		for (int i = 0; i < MRU_PROGRAMS_COUNT; i++)
		{
			...
			{
				if (SUCCEEDED(
					SHCreateItemInKnownFolder(FOLDERID_AppsFolder2,0,(const wchar_t*)s_MRUShortcuts[i]+4,IID_IShellItem,
						(void**)&pItem)) && FAILED(SHGetIDListFromObject(pItem,&pidl)))
					pItem = NULL;
			}
			...
			if (pItem)
			{
				...
				if (SUCCEEDED(pItem->GetDisplayName(SIGDN_NORMALDISPLAY,&pName)))
				{
					...
					item.bLink = true;
					int refreshFlags = CItemManager::INFO_LINK | CItemManager::INFO_LINK_APPID |
						CItemManager::INFO_METRO | iconSizeFlag;
					...
					bool bFound = false;
					{
						...
						for (std::vector<MenuItem>::const_iterator it = mainItems.begin(); it != mainItems.end(); ++it)
						{
							...
							{
								if (!item.pItemInfo->GetTargetPATH().IsEmpty() && wcscmp(
									item.pItemInfo->GetTargetPATH(), it->pItemInfo->GetTargetPATH()) == 0)
								{
									bFound = true;
									break;
								}
	...
	else if (s_RecentPrograms == RECENT_PROGRAMS_FREQUENT)
	{
		...
		for (int idx = 0;; idx++)
		{
			...
			uaItem.pLinkInfo = g_ItemManager.GetItemInfo(uaItem.name,
			                                             CItemManager::INFO_LINK | CItemManager::INFO_LINK_APPID |
			                                             CItemManager::INFO_METRO | CItemManager::INFO_VALIDATE_FILE);
			if (!uaItem.pLinkInfo)
			{
			...
			{
				CItemManager::RWLock lock(&g_ItemManager, false, CItemManager::RWLOCK_ITEMS);
				if (uaItem.pLinkInfo->GetLocation() != CItemManager::LOCATION_START_MENU && uaItem.pLinkInfo->
					GetLocation() != CItemManager::LOCATION_METRO && uaItem.pLinkInfo->GetLocation() !=
					CItemManager::LOCATION_GAMES)
				{
				...
				if (!uaItem.pLinkInfo->GetTargetPATH().IsEmpty())
				{
					...
					for (int i = 0; i < _countof(g_MfuIgnoreExes); i++)
						...
						{
							LOG_MENU(LOG_MFU, L"UserAssist: Dropping: Target contains bad word: '%s'",
							         uaItem.pLinkInfo->GetTargetPATH());
							bIgnore = true;
							break;
						}
				}
		...
		if (GetWinVersion() >= WIN_VER_WIN10 && bShowMetro)
		{
			...
			for (int idx = 0;; idx++)
			{
				...
				{
					CItemManager::RWLock lock(&g_ItemManager, false, CItemManager::RWLOCK_ITEMS);
					if (uaItem.pLinkInfo->GetMetroName().IsEmpty() || wcsncmp(
						uaItem.pLinkInfo->GetMetroName(), L"@{", 2) == 0)
					{
						LOG_MENU(LOG_MFU, L"UserAssist: Dropping: No metro name");
						continue;
					}
					...
					if (uaItem.pLinkInfo->GetPackagePath().IsEmpty() || GetFileAttributes(
						uaItem.pLinkInfo->GetPackagePath()) == INVALID_FILE_ATTRIBUTES)
					{
		...
		g_ItemManager.WaitForShortcuts(CPoint((s_StartRect.left + s_StartRect.right) / 2,
		                                      (s_StartRect.top + s_StartRect.bottom) / 2));
		...
		for (int i = 0; i < (int)uaItems.size(); i++)
		{
			...
			if (bExplicitAppId)
			{
				...
				if (keyPin.Open(
					HKEY_LOCAL_MACHINE,
					L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileAssociation\\NoStartPageAppUserModelIDs",
			...
			else if (item.name.IsEmpty() && SUCCEEDED(
				SHCreateItemFromIDList(uaItem.pLinkInfo->GetPidl(),IID_IShellItem,(void**)&pItem)))
			{
				CComString pName;
				if (SUCCEEDED(pItem->GetDisplayName(SIGDN_NORMALDISPLAY,&pName)))
				{
...
void CMenuContainer::ClearItems(const std::vector<MenuItem>::iterator& begin,
                                const std::vector<MenuItem>::iterator& end)
{
			...
			if (item.pItem1 && !item.pItemInfo)
			{
				...
				if (SUCCEEDED(SHCreateItemFromIDList(item.pItem1,IID_IShellItem,(void**)&pItem)))
				{
					...
					if (SUCCEEDED(pItem->GetDisplayName(SIGDN_DESKTOPABSOLUTEPARSING,&pName)))
					{
						...
						CAbsolutePidl pidl2;
						if (SUCCEEDED(MenuParseDisplayName(pName,&pidl2,NULL,NULL)) && SUCCEEDED(
							SHCreateItemFromIDList(pidl2,IID_IShellItem,(void**)&pItem2)))
						{
							int order;
...
void CMenuContainer::InitItems(void)
{
		...
		if (SUCCEEDED(pLink.CoCreateInstance(CLSID_ShellLink)))
		{
			...
			if (pFile)
			{
				...
				for (std::vector<Document>::const_iterator it = docs.begin(); it != docs.end(); ++it)
				{
					...
					if (SUCCEEDED(pFile->Load(it->name,STGM_READ)) && SUCCEEDED(
						pLink->GetPath(path,_countof(path),&data,0)))
					{
						...
						if (path[0] && !(data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
						{
							...
							CComString pName;
							if (pidl && SUCCEEDED(SHCreateItemFromIDList(pidl,IID_IShellItem,(void**)&pItem)) &&
								SUCCEEDED(pItem->GetDisplayName(SIGDN_NORMALDISPLAY,&pName)))
							{
								if (_wcsicmp(PathFindExtension(pName), L".lnk") == 0)
									continue;
								MenuItem item(MENU_NO);
								item.pItem1 = pidl.Detach();
								item.pItemInfo = g_ItemManager.GetItemInfo(
									pItem, item.pItem1, CItemManager::INFO_SMALL_ICON);
								item.SetName(pName, (m_Options & CONTAINER_NOEXTENSIONS) != 0);
	...
	if ((m_Options & CONTAINER_APPS) && GetWinVersion() >= WIN_VER_WIN8)
	{
		...
		{
			const CItemManager::ItemInfo* pInfo = g_ItemManager.GetItemInfo(
				it->pItem, it->pidl, CItemManager::INFO_METRO | CItemManager::INFO_SMALL_ICON);
			CItemManager::RWLock lock(&g_ItemManager, false, CItemManager::RWLOCK_ITEMS);
...
int CMenuContainer::AddSearchItems(const std::vector<SearchItem>& items, const CString& categoryName,
                                   unsigned int categoryHash, int originalCount)
{
	...
	for (std::vector<SearchItem>::const_iterator it = items.begin(); it != items.end(); ++it)
	{
		...
		CComString pName;
		if (SUCCEEDED(
			pItem->GetDisplayName(categoryHash==CSearchManager::CATEGORY_AUTOCOMPLETE?SIGDN_PARENTRELATIVEEDITING:
				SIGDN_NORMALDISPLAY,&pName)))
		{
			if (bFirst)
			{
				bFirst = false;
				if (!categoryName.IsEmpty())
				{
					...
					if (categoryHash != CSearchManager::CATEGORY_PROGRAM || categoryHash !=
						CSearchManager::CATEGORY_SETTING)
...
bool CMenuContainer::InitSearchItems(void)
{
	...
	if (!bAutoComlpete && !s_bNoRun && s_SearchResults.programs.empty() && s_SearchResults.settings.empty() &&
		s_SearchResults.metrosettings.empty())
	{
		if (s_bWin7Style)
		...
		else if (wcsncmp(runCommand, L"\\\\", 2) != 0 && SUCCEEDED(
			SHEvaluateSystemCommandTemplate(runCommand,&runExe,NULL,NULL)))
			runCategoryHash = CSearchManager::CATEGORY_PROGRAM;
		if (runCategoryHash)
	...
	if (s_bWin7Style)
	{
		sepHeight = s_Skin.ItemSettings[s_Skin.More_bitmap_Size.cx
			                                ? MenuSkin::LIST_SEPARATOR_SPLIT
			                                : MenuSkin::LIST_SEPARATOR].itemHeight;
		itemHeight = s_Skin.ItemSettings[MenuSkin::LIST_ITEM].itemHeight;
	...
	if (bAutoComlpete)
	{
		...
		for (std::vector<const CItemManager::ItemInfo*>::const_iterator it = s_SearchResults.autocomplete.begin(); it !=
		     s_SearchResults.autocomplete.end() && (int)items.size() < MAX_MENU_ITEMS; ++it)
	...
	if (s_bWin7Style)
	{
		...
		for (std::list<CSearchManager::SearchCategory>::const_iterator it = s_SearchResults.indexed.begin(); it !=
		     s_SearchResults.indexed.end(); ++it)
		{
			if (!it->items.empty())
			{
	...
	for (size_t idx = 0; idx < s_SearchResults.indexed.size() + 3; idx++)
	{
		...
		if (idx == 0)
		{
			...
			if (count > originalCount)
				count = originalCount;
			items.reserve(count);
			for (std::vector<const CItemManager::ItemInfo*>::const_iterator it = s_SearchResults.programs.begin(); it !=
			     s_SearchResults.programs.end() && (int)items.size() < count; ++it)
				items.push_back(SearchItem(*it));
		...
		else if (idx == 1)
		{
			...
			if (count > originalCount)
				count = originalCount;
			items.reserve(count);
			for (std::vector<const CItemManager::ItemInfo*>::const_iterator it = s_SearchResults.metrosettings.begin();
			     it != s_SearchResults.metrosettings.end() && (int)items.size() < count; ++it)
				items.push_back(SearchItem(*it));
		...
		else if (idx == 2)
		{
			...
			if (count > originalCount)
				count = originalCount;
			items.reserve(count);
			for (std::vector<const CItemManager::ItemInfo*>::const_iterator it = s_SearchResults.settings.begin(); it !=
			     s_SearchResults.settings.end() && (int)items.size() < count; ++it)
				items.push_back(SearchItem(*it));
		...
		if (idx == 0 && runCategoryHash)
		{
			...
			item1.name = runCategoryHash == CSearchManager::CATEGORY_INTERNET
				             ? FindTranslation(L"Search.CategoryInternet", L"Internet")
				             : FindTranslation(L"Search.CategoryPrograms", L"Programs");
...
void CMenuContainer::InitWindowInternal(bool bDontShrink, const POINT& corner, RECT& menuRect)
{
	...
	{
		s_Tooltip = CreateWindowEx(
			WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_TRANSPARENT | (s_bRTL ? WS_EX_LAYOUTRTL : 0),TOOLTIPS_CLASS,NULL,
			WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, 0, 0, 0, 0,NULL,NULL, g_Instance,NULL);
	...
	if (m_bSubMenu)
	{
		...
		maxItemWidth[0] = numChar
			                  ? s_Skin.ItemSettings[MenuSkin::SUBMENU_ITEM].textMetrics.tmAveCharWidth * numChar
			                  : 65536;
	}
	else
	{
		...
		s_MenuWidthJump = width * (s_ProgramsWidth + s_JumplistWidth) + s_Skin.Main_padding.left + s_Skin.Main_padding.
			right + s_Skin.Main_jump_padding.left + s_Skin.Main_jump_padding.right;
	}
...

	{
		...
		for (int i = 0; i < (int)m_Items.size(); i++)
		{
			...
			item.bNew = false;
			if (m_bTwoColumns && i < m_OriginalCount && s_MenuMode != MODE_NORMAL && (i <= m_SearchIndex -
				m_SearchItemCount || i > m_SearchIndex) && item.id != MENU_SHUTDOWN_BUTTON)
			{
			...
			if (item.id != MENU_RECENT && !(m_Options & CONTAINER_SEARCH) && s_Skin.bHasNewItem && i < m_OriginalCount)
			{
				if (item.id == MENU_PROGRAMS)
				{
					...
					item.bNew = (item.pItem1 && g_ItemManager.IsNewProgram(item.pItem1, true, false)) || (item.pItem2 &&
						g_ItemManager.IsNewProgram(item.pItem2, true, false));
				...
				m_OriginalCount)
			{
				if (item.id == MENU_SEPARATOR)
					...
					item.drawType = (s_JumpList.groups[LOWORD(item.jumpIndex)].type == CJumpGroup::TYPE_TASKS)
						                ? MenuSkin::LIST_ITEM
						                : MenuSkin::LIST_SPLIT;
			}
			...
			else if (m_bSubMenu)
			{
				if (item.id == MENU_SEPARATOR) item.drawType = MenuSkin::SUBMENU_SEPARATOR;
					...
					item.drawType = (item.bSplit)
						                ? MenuSkin::SUBMENU_SEPARATOR_SPLIT
						                : MenuSkin::SUBMENU_SEPARATOR;
			...
			if (item.id == MENU_PROGRAMS_TREE)
				h = 0; // hide it for now
			else if (!s_bShowTopEmpty && m_Items.size() > 1 && (m_Items[i].id == MENU_EMPTY_TOP || (i > 0 && m_Items[i -
				1].id == MENU_EMPTY_TOP)))
			...
			{
				h = settings.textMetrics.tmHeight * 12 / 8 + s_Skin.Search_padding.top + s_Skin.Search_padding.bottom;
				// 12 DLUs
				w = settings.textMetrics.tmAveCharWidth * 25;
			}
			else if (s_bWin7Style && i > m_SearchIndex - m_SearchItemCount && i < m_SearchIndex && (s_MenuMode !=
				MODE_SEARCH || (item.id == MENU_MORE_RESULTS && !HasMoreResults())))
			{
				h = 0;
			}
			else if (item.id == MENU_SHUTDOWN_BUTTON)
			{
				...
				w += s_Skin.Shutdown_padding.left + s_Skin.Shutdown_padding.right + settings.textPadding.left + settings
					.textPadding.right +
			...
			else
			{
				h = settings.itemHeight;
				RECT rcText = {0, 0, 0, 0};
				DrawText(hdc, item.name, -1, &rcText,
				         DT_CALCRECT | DT_SINGLELINE | (item.id == MENU_NO ? DT_NOPREFIX : DT_HIDEPREFIX));
				w = rcText.right;
				if (w > maxItemWidth[index]) w = maxItemWidth[index];
				w += settings.iconPadding.left + settings.iconPadding.right + settings.textPadding.left + settings.
					textPadding.right + arrowSize[index] + iconSize;
			}
			...
			else if (item.id == MENU_SEPARATOR && !item.bBlankSeparator && !item.bInline && m_bTwoColumns && column == 0
				&& i + 1 < (int)m_Items.size() && m_Items[i + 1].bBreak)
				h = 0;
			item.row = row;
			item.column = column;
			item.itemRect.top = y;
	...
	else if (columnWidths.size() == 2 && s_bWin7Style && !m_bSubMenu && columnWidths[1] <
		CItemManager::EXTRA_LARGE_ICON_SIZE)
	...
	int scrollWidth = GetSystemMetrics(SM_CXVSCROLL);
	{
		...
		for (int i = 0; i < (int)m_Items.size(); i++)
		{
			...
			{
				item.itemRect.left = 0;
				item.itemRect.right = s_Skin.ItemSettings[MenuSkin::COLUMN1_ITEM].textMetrics.tmAveCharWidth *
					s_ProgramsWidth;
			}
			else
			{
				...
				if (s_MenuMode == MODE_SEARCH && !m_bSubMenu && (item.id == MENU_NO || item.id == MENU_SEARCH_EXECUTE)
					&& i >= m_OriginalCount)
				{
	...
	}
	{
		...
		if (!m_bSubMenu)
		{
			...
			else if (s_Skin.Main_bitmap.GetBitmap() || s_Skin.User_image_size || m_bTwoColumns || s_Skin.
				User_name_position.left != s_Skin.User_name_position.right)
			{
				CreateBackground(w1, w2, h1, h2, totalWidth, totalHeight, true);
			}
			else
	...
	if (m_SearchIndex >= 0)
	{
		...
		{
			m_SearchBox.Create(L"EDIT", m_hWnd, itemRect,NULL,
			                   WS_CHILD | (s_Skin.Search_frame ? WS_BORDER : 0) | ES_AUTOHSCROLL | ES_WANTRETURN);
			...
			m_SearchIcons = (HBITMAP)LoadImage(g_Instance, MAKEINTRESOURCE(IDB_SEARCH_ICONS), IMAGE_BITMAP, 0, 0,
			                                   LR_CREATEDIBSECTION);
	...
	if (m_bTwoColumns && s_MenuMode == MODE_JUMPLIST)
	{
		...
		if (m_SearchIndex >= 0)
		{
			int h = m_Items[m_SearchIndex].itemRect.top - s_Skin.Search_padding.top - s_Skin.
				Search_background_jump_padding.top;
			if (maxH > h) maxH = h;
		}
		if (shutdownIndex >= 0)
		{
	...
	if (m_Options & CONTAINER_SEARCH)
	{
		...
		dy = (m_Options & CONTAINER_TOP)
			     ? (itemRect.top - menuPadding[0].top - menuRect.top)
			     : (itemRect.bottom + menuPadding[0].bottom - menuRect.bottom);
	}
	else
	{
	...
	if (s_UserPicture.m_hWnd)
	{
		...
		if (s_bRTL)
			s_UserPictureRect.left = menuRect.right - (m_rContent2.left + m_rContent2.right + s_Skin.User_bitmapSize.cx)
				/ 2;
		else
			s_UserPictureRect.left = menuRect.left + (m_rContent2.left + m_rContent2.right - s_Skin.User_bitmapSize.cx)
				/ 2;
	...
	if (m_pParent && (m_pParent->m_Items[m_ParentIndex].id == MENU_PROGRAMS || m_pParent->m_Items[m_ParentIndex].id ==
		MENU_APPS))
	{
		ULONGLONG curTime;
		GetSystemTimeAsFileTime((FILETIME*)&curTime);
...
LRESULT CMenuContainer::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	else if ((opacity == MenuSkin::OPACITY_GLASS || opacity == MenuSkin::OPACITY_FULLGLASS) && GetWinVersion() >=
		WIN_VER_WIN10)
	{
		tSetWindowCompositionAttribute SetWindowCompositionAttribute = (tSetWindowCompositionAttribute)GetProcAddress(
...
int CMenuContainer::HitTest(const POINT& pt, bool* bArrow, bool bDrop)
{
	...
	for (int i = start; i < n; i++)
	{
		...
		else if (m_SearchScrollCount > m_SearchScrollHeight && i >= m_OriginalCount)
		{
			...
			rc.bottom = m_Items[i + 1].itemRect.top;
		// when dropping on the padding of the first column, assume the item above the padding was hit
		if (PtInRect(&rc, pt))
		{
			if (bArrow)
			{
...
void CMenuContainer::UpdateUserPicture(void)
{
	if (m_bTwoColumns && s_UserPicture.m_hWnd && s_UserPicture.IsWindowVisible())
	{
		...
		if (bmpIndex >= 0 && bmpIndex < m_OriginalCount && bmpIndex < (int)m_Items.size() && m_Items[bmpIndex].column ==
			1 && m_Items[bmpIndex].pItemInfo && m_Items[bmpIndex].pItemInfo->extraLargeIcon)
	...
	if (pNotify->hwndTarget == m_hWnd)
	{
		...
		if (bScrollable)
		{
			GESTURECONFIG config = {
				GID_PAN,GC_PAN_WITH_SINGLE_FINGER_VERTICALLY | GC_PAN_WITH_INERTIA,
...
{
	m_PointerId = 0;
	if (s_bHasTouch && IS_POINTER_INCONTACT_WPARAM(wParam) && IS_POINTER_PRIMARY_WPARAM(wParam) &&
...
void CMenuContainer::RefreshIcons(void)
{
	...
	HWND first = s_FirstMenu;
	// must copy into a temp variable because we don't want the value to change in the middle of the next two lines
	if (first)
...
void CMenuContainer::SetSearchState(TSearchState state)
{
	...
	if (m_SearchState == SEARCH_TEXT && state != SEARCH_TEXT)
	{
		...
		if (s_MenuMode == MODE_SEARCH)
			...
			CloseSubMenus(CLOSE_ONLY_SEARCH, this);
		// can't use CLOSE_POST here because the menu needs to be closed while m_bInSearchUpdate is set
...
void CMenuContainer::SetActiveWindow(void)
{
	...
	// make sure the top menu stays behind the taskbar
}
...
LRESULT CMenuContainer::OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
		LOG_MENU(LOG_MOUSE, L"End Hover, hover=%d%s, hot=%d, submenu=%d", m_HoverItem, m_bHoverArrow?L"(arrow)":L"",
		         m_HotItem, m_Submenu);
		// the mouse hovers over an item. open it.
		if (m_HoverItem >= 0)
		{
			if (m_HoverItem != m_Submenu && m_HoverItem != m_SubJumpItem && m_HoverItem == m_HotItem && m_bHoverArrow ==
	...
	if (wParam == TIMER_SCROLL_MOUSE || wParam == TIMER_SCROLL_TOUCH)
	{
		...
		{
			scroll -= s_Skin.ItemSettings[m_bSubMenu ? MenuSkin::SUBMENU_ITEM : MenuSkin::COLUMN1_ITEM].itemHeight *
				speed / 6;
...
bool CMenuContainer::CanSelectItem(int index, bool bKeyboard)
{
	...
	if (!m_bSubMenu && s_MenuMode == MODE_SEARCH && index < m_OriginalCount && item.id != MENU_SEARCH_BOX && item.id !=
		MENU_SHUTDOWN_BUTTON && item.id != MENU_MORE_RESULTS && item.id != MENU_SEARCH_INTERNET && item.id !=
...
LRESULT CMenuContainer::OnKeyDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (wParam == VK_PRIOR || wParam == VK_NEXT)
	{
		...
		if (m_ScrollHeight > 0)
		{
			...
			int count = m_ScrollHeight / s_Skin.ItemSettings[m_bSubMenu
				                                                 ? MenuSkin::SUBMENU_ITEM
				                                                 : MenuSkin::COLUMN1_ITEM].itemHeight - 1;
			int newIndex = index;
			for (int i = 0; i < count; i++)
			{
				if (wParam == VK_PRIOR)
		...
		else
		{
			...
			{
				int column = m_Items[index].column;
				if (index < (int)m_Items.size() - 1 && m_Items.rbegin()->column > 0 && m_Items[index + 1].column !=
					column)
					column++;
	...
	if (wParam != VK_UP && wParam != VK_DOWN && wParam != VK_LEFT && wParam != VK_RIGHT && wParam != VK_ESCAPE && wParam
		!= VK_RETURN)
		return TRUE;
	...
	if (wParam == VK_UP)
	{
		...
		if (index < 0)
		{
			...
			{
				const MenuItem& item = m_Items[i];
				if (item.column == col && CanSelectItem(i) && (!item.bInline || item.bInlineFirst) && (s_MenuMode !=
					MODE_SEARCH || item.id != MENU_SHUTDOWN_BUTTON))
				{
					index = i;
					break;
				}
		...
		if (best >= 0)
		{
			...
			if ((m_Items[best].categoryHash & CSearchManager::CATEGORY_MASK) == CSearchManager::CATEGORY_AUTOCOMPLETE &&
				m_Items[best].pItemInfo)
			{
				CString path;
				{
					CItemManager::RWLock lock(&g_ItemManager, false, CItemManager::RWLOCK_ITEMS);
	...
	if (wParam == VK_DOWN)
	{
		...
		if (index < 0)
		{
			...
			{
				const MenuItem& item = m_Items[i];
				if (CanSelectItem(i) && (!item.bInline || item.bInlineFirst) && (s_MenuMode != MODE_SEARCH || item.id !=
	...
	if (wParam == VK_ESCAPE || (bBack && GetKeyState(VK_CONTROL) >= 0 && (s_Menus.size() > 1 || (s_Menus.size() == 1 &&
		m_bSubMenu))))
	{
		// close top menu
	...
	if (wParam == VK_RETURN || bForward)
	{
		...
		if (index >= 0)
		{
			if (m_Items[index].bFolder && (bForward || !m_Items[index].bSplit) && (wParam == VK_RETURN ||
			...
			else if (bForward)
			{
				...
				else
				{
					...
					for (int i = 0; i < n; i++)
					{
						if (m_Items[i].column == column && CanSelectItem(i) && (!m_Items[i].bInline || m_Items[i].
							bInlineFirst))
						{
							int ytop = m_Items[i].itemRect.top + 1, ybottom = m_Items[i].itemRect.bottom - 1;
							if (i < m_ScrollCount)
...
LRESULT CMenuContainer::OnChar(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	ActivateData data;
	data.bNoModifiers = true;
	if (!m_Items[first].bFolder || (!m_Items[first].bHasJumpList && m_Items[first].bSplit) || (m_Items[first].
		bHasJumpList && GetSettingInt(L"JumplistKeys") == 1))
	{
...
LRESULT CMenuContainer::OnRefresh(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if ((m_Options & CONTAINER_SEARCH) || (!m_bSubMenu && s_MenuMode == MODE_SEARCH))
	{
		...
		if (index >= 0)
		{
			...
			else if (s_bWin7Style && index > m_SearchIndex - m_SearchItemCount && index <= m_SearchIndex && m_Items[
				index].id != MENU_SEARCH_EMPTY)
...
LRESULT CMenuContainer::OnMouseMove(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (s_HotPos == GetMessagePos())
		return 0;
	// HACK - ignore the mouse if it hasn't moved since last time. otherwise the mouse can override the keyboard navigation
	s_HotPos = GetMessagePos();
	POINT pt = {(short)LOWORD(lParam), (short)HIWORD(lParam)};
	bool bArrow = false;
	...
	else
	{
		...
		if (index >= 0)
		{
			if ((m_Submenu >= 0 && index != m_Submenu) || (m_Submenu < 0 && m_Items[index].bFolder) || (s_MenuMode ==
				MODE_JUMPLIST && m_SubJumpItem >= 0 && index != m_SubJumpItem && index < m_OriginalCount))
			{
				// initialize the hover timer
				if ((m_HoverItem != index || m_bHoverArrow != bArrow) && !s_bDisableHover && m_SearchState <=
					SEARCH_BLANK)
				{
					m_HoverItem = index;
					...
					else
					{
						...
						else if (s_bWin7Style && index == m_ProgramButtonIndex && GetSettingInt(L"ProgramsStyle") ==
							PROGRAMS_INLINE)
	...
	if (item.id == MENU_NO && item.categoryHash == CSearchManager::CATEGORY_METROSETTING)
	{
		...
		if (SUCCEEDED(pDoc.CoCreateInstance(L"Msxml2.FreeThreadedDOMDocument")))
		{
			...
			if (pDoc->load(CComVariant(item.pItemInfo->PATH), &loaded) == S_OK && loaded == VARIANT_TRUE)
			{
				...
				HRESULT res = pDoc->selectSingleNode(
					CComBSTR(L"PCSettings/SearchableContent/SettingInformation/Description"), &pDescription);
				if (res == S_OK)
				{
					CComBSTR desc;
...
LRESULT CMenuContainer::OnLButtonUp(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	POINT pt2 = pt;
	ClientToScreen(&pt2);
	if (!item.bFolder || (s_bSingleClickFolders && item.id != MENU_PROGRAMS && item.id != MENU_APPS && !bArrow))
	// never open All Programs, Apps folder, or jumplists with single click
	{
		if (item.jumpIndex >= 0 && m_bHotArrow)
		{
			const CJumpGroup& group = s_JumpList.groups[LOWORD(item.jumpIndex)];
			const CJumpItem& jumpItem = group.items[HIWORD(item.jumpIndex)];
			PinJumpItem(s_JumpAppInfo, s_JumpList,LOWORD(item.jumpIndex),HIWORD(item.jumpIndex),
			            group.type != CJumpGroup::TYPE_PINNED, -1);
			PostRefreshMessage();
		}
	...
	else
	{
		...
		if (item.bSplit)
		{
			...
			if ((index == m_Submenu || index == m_SubJumpItem) && (!m_SubShowTime || (int)(GetTickCount() -
				m_SubShowTime) > 500))
			{
		...
		else if (s_bWin7Style && item.id == MENU_PROGRAMS && GetSettingInt(L"ProgramsStyle") == PROGRAMS_INLINE &&
			m_SubShowTime && (int)(GetTickCount() - m_SubShowTime) < 500)
...
{
	CRegKey regKey;
	if (regKey.Open(HKEY_CURRENT_USER, bAppId ? USERASSIST_APPIDS_KEY : USERASSIST_LINKS_KEY,KEY_READ | KEY_WRITE) ==
		ERROR_SUCCESS)
	{
		for (int idx = 0;; idx++)
		{
...
static bool CheckForUpdates(void)
{
	...
	if (GetWinVersion() < WIN_VER_WIN8)
	{
		CRegKey regKey;
		if (regKey.Open(
			HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\CurrentVersion\\WindowsUpdate\\Auto Update\\UAS",
			KEY_READ) == ERROR_SUCCESS)
		{
			DWORD val;
	...
	if (!bHasUpdates && GetWinVersion() >= WIN_VER_WIN8)
	{
		...
		if (mod)
		{
			FGetAutoUpdateNotification fun = (FGetAutoUpdateNotification)GetProcAddress(
				mod, "GetAutoUpdateNotification");
			if (fun)
			{
				DWORD a, b, c;
...
bool CMenuContainer::HasMoreResults(void)
{
	if (s_HasMoreResults == -1)
		s_HasMoreResults = (GetSettingBool(L"MoreResults") && GetSettingBool(L"SearchFiles") && HasSearchService())
			                   ? 1
			                   : 0;
...
RECT CMenuContainer::CalculateWorkArea(const RECT& taskbarRect)
{
	...
	if (!s_bLockWorkArea)
	{
		...
		if (s_pFrameworkInputPane)
		{
			...
			{
				if (rc.top < kbdRect.bottom && rc.bottom > kbdRect.top && rc.left < kbdRect.right && rc.right > kbdRect.
					left)
				{
					...
					if (monitor)
					{
						...
						if (kbdRect.bottom == info.rcMonitor.bottom && kbdRect.left == info.rcMonitor.left && kbdRect.
...
HWND CMenuContainer::ToggleStartMenu(int taskbarId, bool bKeyboard, bool bAllPrograms)
{
...

	{
		...
		if (regKey.QueryDWORDValue(L"ShowedStyle2", val) != ERROR_SUCCESS)
		{
			...
			if (GetSettingBool(L"EnableSettings") && !IsSettingLocked(L"MenuStyle") && !IsSettingForcedDefault(
				L"MenuStyle"))
			{
	...
	{
		CRegKey regKey;
		if (regKey.Open(HKEY_CURRENT_USER, L"Software\\OpenShell\\StartMenu\\Settings",KEY_READ | KEY_WOW64_64KEY) ==
			ERROR_SUCCESS)
		{
			DWORD log;
	...
	if (bAllPrograms)
	{
		bErr = !s_Skin.LoadMenuSkin(GetSettingString(L"SkinA"), GetSettingString(L"SkinVariationA"),
		                            GetSettingString(L"SkinOptionsA"), MenuSkin::SKIN_TYPE_ALL_PROGRAMS,
		                            LOADMENU_RESOURCES, dpi);
		if (bErr)
			s_Skin.LoadDefaultMenuSkin(MenuSkin::SKIN_TYPE_ALL_PROGRAMS, LOADMENU_RESOURCES, dpi);
	}
	else if (s_bWin7Style)
	{
		bErr = !s_Skin.LoadMenuSkin(GetSettingString(L"SkinW7"), GetSettingString(L"SkinVariationW7"),
		                            GetSettingString(L"SkinOptionsW7"), MenuSkin::SKIN_TYPE_WIN7, LOADMENU_RESOURCES,
		                            dpi);
		if (bErr)
	...
	else if (GetSettingInt(L"MenuStyle") == 1)
	{
		bErr = !s_Skin.LoadMenuSkin(GetSettingString(L"SkinC2"), GetSettingString(L"SkinVariationC2"),
		                            GetSettingString(L"SkinOptionsC2"), MenuSkin::SKIN_TYPE_CLASSIC2,
		                            LOADMENU_RESOURCES, dpi);
		if (bErr)
	...
	{
		bErr = !s_Skin.LoadMenuSkin(GetSettingString(L"SkinC1"), GetSettingString(L"SkinVariationC1"),
		                            GetSettingString(L"SkinOptionsC1"), MenuSkin::SKIN_TYPE_CLASSIC1,
		                            LOADMENU_RESOURCES, dpi);
		if (bErr)
			s_Skin.LoadDefaultMenuSkin(MenuSkin::SKIN_TYPE_CLASSIC1, LOADMENU_RESOURCES, dpi);
	}

	const MenuSkin& s_Skin = CMenuContainer::s_Skin;
	// shadow s_Skin with a const reference to catch any modifications to non-mutable members
	...
	s_bHasTouch = GetWinVersion() >= WIN_VER_WIN8 && GetSettingBool(L"EnableTouch") && (GetSystemMetrics(SM_DIGITIZER) &
		NID_INTEGRATED_TOUCH) != 0;
	s_HasMoreResults = -1;
	s_bDisableHover = false;
	s_bDragClosed = false;
	s_bPendingSearchEnter = false;
	...
	if (GetWinVersion() >= WIN_VER_WIN8 && !s_pFrameworkInputPane && !(GetSettingInt(L"CompatibilityFixes") &
		COMPATIBILITY_NO_TOUCH_KBD))
	{
		s_pFrameworkInputPane.CoCreateInstance(CLSID_FrameworkInputPane);
	}

	s_bHasUpdates = (!bRemote || GetSettingBool(L"RemoteShutdown")) && GetSettingBool(L"CheckWinUpdates") &&
		CheckForUpdates();
	...
	bool bHibernate = true, bSleep = true, bLock = true;
	{
		...
		if (regKeyButtons.Open(
			HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FlyoutMenuSettings",
			KEY_READ) == ERROR_SUCCESS)
		{
			DWORD dwValue = 1;
	...
	for (int i = 0; i < _countof(g_StdOptions); i++)
	{
		...
		case MENU_SHUTDOWN:
		case MENU_RESTART:
			g_StdOptions[i].options = (!s_bNoClose && (!bRemote || GetSettingBool(L"RemoteShutdown")))
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
		case MENU_SHUTDOWN_NOUPDATE:
			g_StdOptions[i].options = (s_bHasUpdates && !s_bNoClose && (!bRemote || GetSettingBool(L"RemoteShutdown")))
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
		case MENU_RESTART_NOUPDATE:
			g_StdOptions[i].options = (s_bHasUpdates && GetWinVersion() >= WIN_VER_WIN8 && !s_bNoClose && (!bRemote ||
				                          GetSettingBool(L"RemoteShutdown")))
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
		...
		case MENU_SHUTDOWN_BUTTON:
			g_StdOptions[i].options = GetSettingInt(L"ShutdownCommand") > SHUTDOWN_TYPE_NONE
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
		case MENU_UNDOCK:
		...
		case MENU_SEARCH_COMPUTERS:
			g_StdOptions[i].options = (s_bActiveDirectory == 1 && !SHRestricted(REST_HASFINDCOMPUTERS))
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
		...
		case MENU_TASKBAR:
			...
			break;
		case MENU_FEATURES:
			g_StdOptions[i].options = (!SHRestricted(REST_NOSETFOLDERS) && !SHRestricted(REST_NOCONTROLPANEL))
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
		...
		case MENU_SEARCH_BOX:
			g_StdOptions[i].options = GetSettingInt(L"SearchBox") != SEARCHBOX_HIDDEN
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
		case MENU_USERFILES:
		...
		case MENU_SWITCHUSER:
			{
				...
				{
					long val;
					if (SUCCEEDED(pShellDisp->IsRestricted(CComBSTR(L"System"),CComBSTR(L"HideFastUserSwitching"),&val))
						&& val)
						g_StdOptions[i].options = 0;
				}
			}
			break;
		case MENU_APPS:
			g_StdOptions[i].options = (s_bWin7Style || (GetWinVersion() >= WIN_VER_WIN8 &&
				                          GetSettingBool(L"ShowAppsMenu")))
				                          ? MENU_ENABLED | MENU_EXPANDED
				                          : 0;
			break;
	...
	RECT dummyRc;
	{
		...
		if (!pStartMenu->Create(owner, &rc, bAllPrograms ? s_SubmenuStyle : dwStyle,
		                        WS_EX_TOOLWINDOW | ((bTopMost || !s_bBehindTaskbar) ? WS_EX_TOPMOST : 0) | (
			                        s_bRTL ? WS_EX_LAYOUTRTL : 0)))
		{
			delete pStartMenu;
			return NULL;
		}
	...
	HMONITOR s_MenuMonitor = MonitorFromWindow(bAllPrograms ? g_ProgramsButton : pStartMenu->m_hWnd,
	...
	else
	{
		...
		{
		case ABE_LEFT: rc.right = info.rcWork.right;
			break;
		case ABE_RIGHT: rc.left = info.rcWork.left;
			break;
		case ABE_TOP: rc.bottom = info.rcWork.bottom;
			break;
		case ABE_BOTTOM: rc.top = info.rcWork.top;
			break;
		}
	...
	LOG_MENU(LOG_OPEN, L"Main Rect: %d, %d, %d, %d", s_MenuLimits.left, s_MenuLimits.top, s_MenuLimits.right,
	         s_MenuLimits.bottom);
	LOG_MENU(LOG_OPEN, L"Start Rect: %d, %d, %d, %d", s_StartRect.left, s_StartRect.top, s_StartRect.right,
	         s_StartRect.bottom);
...

	{
		...
		s_Skin.Submenu_padding.left += margin.left;
		if (s_Skin.Submenu_padding.left < 0) s_Skin.Submenu_padding.left = 0;
		s_Skin.Submenu_padding.right -= margin.right;
		if (s_Skin.Submenu_padding.right < 0) s_Skin.Submenu_padding.right = 0;
		s_Skin.Submenu_padding.top += margin.top;
		if (s_Skin.Submenu_padding.top < 0) s_Skin.Submenu_padding.top = 0;
		s_Skin.Submenu_padding.bottom -= margin.bottom;
		if (s_Skin.Submenu_padding.bottom < 0) s_Skin.Submenu_padding.bottom = 0;
	}
	...
	else
	{
		...
		{
			s_Skin.Caption_padding.left += margin.left;
			if (s_Skin.Caption_padding.left < 0) s_Skin.Caption_padding.left = 0;
			s_Skin.Caption_padding.top += margin.top;
			if (s_Skin.Caption_padding.top < 0) s_Skin.Caption_padding.top = 0;
			s_Skin.Caption_padding.bottom -= margin.bottom;
			if (s_Skin.Caption_padding.bottom < 0) s_Skin.Caption_padding.bottom = 0;
		}
		else
		{
			// no caption
			s_Skin.Main_padding.left += margin.left;
			if (s_Skin.Main_padding.left < 0) s_Skin.Main_padding.left = 0;
			if (s_Skin.Main2_padding.left >= 0)
			{
				s_Skin.Main2_padding.left += margin.left;
				if (s_Skin.Main2_padding.left < 0) s_Skin.Main2_padding.left = 0;
			}
		}
		s_Skin.Main_padding.right -= margin.right;
		if (s_Skin.Main_padding.right < 0) s_Skin.Main_padding.right = 0;
		s_Skin.Main_padding.top += margin.top;
		if (s_Skin.Main_padding.top < 0) s_Skin.Main_padding.top = 0;
		s_Skin.Main_padding.bottom -= margin.bottom;
		if (s_Skin.Main_padding.bottom < 0) s_Skin.Main_padding.bottom = 0;
		if (s_Skin.Main2_padding.left >= 0)
		{
			s_Skin.Main2_padding.right -= margin.right;
			if (s_Skin.Main2_padding.right < 0) s_Skin.Main2_padding.right = 0;
			s_Skin.Main2_padding.top += margin.top;
			if (s_Skin.Main2_padding.top < 0) s_Skin.Main2_padding.top = 0;
			s_Skin.Main2_padding.bottom -= margin.bottom;
			if (s_Skin.Main2_padding.bottom < 0) s_Skin.Main2_padding.bottom = 0;
		}
		if (s_bWin7Style)
		{
			s_Skin.Main_search_padding.left += margin.left;
			if (s_Skin.Main_search_padding.left < 0) s_Skin.Main_search_padding.left = 0;
			s_Skin.Main_search_padding.right -= margin.right;
			if (s_Skin.Main_search_padding.right < 0) s_Skin.Main_search_padding.right = 0;
			s_Skin.Main_search_padding.top += margin.top;
			if (s_Skin.Main_search_padding.top < 0) s_Skin.Main_search_padding.top = 0;
			s_Skin.Main_search_padding.bottom -= margin.bottom;
			if (s_Skin.Main_search_padding.bottom < 0) s_Skin.Main_search_padding.bottom = 0;

			s_Skin.Main_jump_padding.right -= margin.right;
			if (s_Skin.Main_jump_padding.right < 0) s_Skin.Main_jump_padding.right = 0;
			s_Skin.Main_jump_padding.top += margin.top;
			if (s_Skin.Main_jump_padding.top < 0) s_Skin.Main_jump_padding.top = 0;
			s_Skin.Main_jump_padding.bottom -= margin.bottom;
			if (s_Skin.Main_jump_padding.bottom < 0) s_Skin.Main_jump_padding.bottom = 0;
		}
	...
	pStartMenu->SetWindowPos(HWND_TOPMOST, corner.x, corner.y, 0, 0,
	                         ((initialMonitor != s_MenuMonitor && !bAllPrograms) ? SWP_NOMOVE | SWP_NOSIZE : 0) |
	                         SWP_NOACTIVATE);

	pStartMenu->InitItems();
	pStartMenu->m_MaxWidth = s_MainMenuLimits.right - s_MainMenuLimits.left;
	g_ItemManager.UpdateNewPrograms(CPoint((s_StartRect.left + s_StartRect.right) / 2,
	                                       (s_StartRect.top + s_StartRect.bottom) / 2));
	...
	if (s_bWin7Style && GetSettingInt(L"ProgramsStyle") == PROGRAMS_INLINE && GetSettingBool(L"OpenPrograms"))
	{
		...
		if (pStartMenu->m_SearchIndex < 0 || GetSettingInt(L"SearchBox") != SEARCHBOX_NORMAL || !GetSettingBool(
			L"SearchSelect"))
		{
			bTreeSelected = true;
	...
	if (s_TaskBar && s_bBehindTaskbar)
		::SetWindowPos(s_TaskBar, bTopMost ? HWND_TOPMOST : HWND_TOP, 0, 0, 0, 0,
		               SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE); // bring the start button on top
		...
		L"SearchSelect"))
	{
		...
		if (pStartMenu->m_bTwoColumns && pStartMenu->m_Items[pStartMenu->m_SearchIndex].column == 0 && pStartMenu->
			m_SearchIndex + 1 < (int)pStartMenu->m_Items.size() && pStartMenu->m_Items[pStartMenu->m_SearchIndex + 1].
			column == 1)
	...
	if (s_TaskBar && s_bBehindTaskbar)
	{
		...
		if (s_StartButton)
			::SetWindowPos(s_StartButton, bTopMost ? HWND_TOPMOST : HWND_TOP, 0, 0, 0, 0,
	...
	if (*MenuSkin::s_SkinError && GetSettingBool(L"ReportSkinErrors") && s_StartButton)
	{
		...
		s_TooltipBalloon = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | (s_bRTL ? WS_EX_LAYOUTRTL : 0),
		                                  TOOLTIPS_CLASS,NULL,WS_POPUP | TTS_BALLOON | TTS_CLOSE | TTS_NOPREFIX, 0, 0,
		                                  0, 0, pStartMenu->m_hWnd,NULL, g_Instance,NULL);
...
void CMenuContainer::SetMenuMode(TMenuMode mode, bool bKeyboard)
{
	...
	if (s_MenuMode == MODE_PROGRAMS)
	{
		...
		if (bNewTree)
		{
			for (int i = 0; i < s_ProgramsScrollPos; i++)
				m_pProgramsTree->SendMessage(WM_VSCROLL,SB_LINEDOWN);
			// using SetSrollPos doesn't quite work because the contents are not scrolled during WM_PRINT
		}
	...
	if (s_UserPicture.m_hWnd && s_MenuMode != MODE_NORMAL && s_MenuMode != MODE_PROGRAMS)
		s_UserPicture.SetWindowPos(NULL, 0, 0, 0, 0,
		                           SWP_NOZORDER | SWP_NOSIZE | SWP_NOMOVE | SWP_HIDEWINDOW | SWP_NOACTIVATE);
	if (!m_bClosing && s_Skin.Main_opacity != MenuSkin::OPACITY_SOLID && GetSettingBool(L"MainMenuAnimate") &&
		IsWindowVisible())
		AnimateBackground();
	else
	{
	...
	if (s_UserPicture.m_hWnd && (s_MenuMode == MODE_NORMAL || s_MenuMode == MODE_PROGRAMS) && !m_bClosing &&
		IsWindowVisible())
		s_UserPicture.SetWindowPos(NULL, 0, 0, 0, 0,
		                           SWP_NOZORDER | SWP_NOSIZE | SWP_NOMOVE | SWP_SHOWWINDOW | SWP_NOACTIVATE);
	ClearOldState();
}
...
void CMenuContainer::AnimateBackground(void)
{
	...
	for (s_OldMenuState.blend = step; s_OldMenuState.blend < 100; s_OldMenuState.blend += step)
	{
		...
		{
			Assert(!m_Bitmap);
			CreateBackground(s_OldMenuState.w1, s_OldMenuState.w2 + dw, s_OldMenuState.h1, s_OldMenuState.h2, tw, th,
			                 false);
		...
		bool bOpaqueA1 = (s_Skin.Main_opacity == MenuSkin::OPACITY_GLASS || s_Skin.Main_opacity ==
			MenuSkin::OPACITY_ALPHA);
		bool bOpaqueB1 = (s_Skin.Main2_opacity == MenuSkin::OPACITY_GLASS || s_Skin.Main2_opacity ==
			MenuSkin::OPACITY_ALPHA);
		if (s_MenuMode == MODE_JUMPLIST)
			bOpaqueB1 = (s_Skin.Jumplist_opacity == MenuSkin::OPACITY_GLASS || s_Skin.Jumplist_opacity ==
				MenuSkin::OPACITY_ALPHA);
		...
		{
			if (s_BackgroundW2 > 0)
				CreateBackground(s_BackgroundW1, s_BackgroundW2 + dw - totalDW, s_BackgroundH1, s_BackgroundH2, tw, th,
				                 true);
			else
				CreateBackground(s_BackgroundW1 + dw - totalDW, s_BackgroundW2, s_BackgroundH1, s_BackgroundH2, tw, th,
				                 true);
			Assert(tw==m_rMenu.right-m_rMenu.left && th==m_rMenu.bottom-m_rMenu.top);
		...
		{
			if (s_BackgroundW2 > 0)
				CreateContentRects(s_BackgroundW1, s_BackgroundW2 + dw - totalDW, s_BackgroundH1, s_BackgroundH2, tw,
				                   th);
			else
				CreateContentRects(s_BackgroundW1 + dw - totalDW, s_BackgroundW2, s_BackgroundH1, s_BackgroundH2, tw,
				                   th);
			rContentA2 = m_rContent;
			rContentB2 = m_rContent2;
			OffsetRect(&m_rContent, m_BitmapOffset, 0);
		...
		bool bOpaqueA2 = (s_Skin.Main_opacity == MenuSkin::OPACITY_GLASS || s_Skin.Main_opacity ==
			MenuSkin::OPACITY_ALPHA);
		bool bOpaqueB2 = (s_Skin.Main2_opacity == MenuSkin::OPACITY_GLASS || s_Skin.Main2_opacity ==
			MenuSkin::OPACITY_ALPHA);
		if (s_MenuMode == MODE_JUMPLIST)
			bOpaqueB2 = (s_Skin.Jumplist_opacity == MenuSkin::OPACITY_GLASS || s_Skin.Jumplist_opacity ==
				MenuSkin::OPACITY_ALPHA);
		...
		for (int y = 0, i = 0; y < -dib.bmiHeader.biHeight; y++)
		{
			for (int x = 0; x < dib.bmiHeader.biWidth; x++, i++)
			{
				...
				int a1 = pixel1 >> 24;
				if ((bOpaqueA1 && x >= rContentA1.left && x < rContentA1.right && y >= rContentA1.top && y < rContentA1.
						bottom) ||
					(bOpaqueB1 && x >= rContentB1.left && x < rContentB1.right && y >= rContentB1.top && y < rContentB1.
						bottom))
					a1 = 255;
				...
				int a2 = pixel2 >> 24;
				if ((bOpaqueA2 && x >= rContentA2.left && x < rContentA2.right && y >= rContentA2.top && y < rContentA2.
						bottom) ||
					(bOpaqueB2 && x >= rContentB2.left && x < rContentB2.right && y >= rContentB2.top && y < rContentB2.
						bottom))
					a2 = 255;
