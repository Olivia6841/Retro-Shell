...
struct StdMenuItem
{
	...
	enum
	{
		...
		MENU_NORECENT = 0x008000,
		// don't show recent items in the root menu (because a sub-menu uses MENU_RECENT_ITEMS)
...
struct SpecialFolder
{
	...
	enum
	{
		...
		FOLDER_NODROP = 4,
		// don't allow reordering, don't show "Sort" and "Auto Arrange" (also implies FOLDER_NONEWFOLDER)
...
public:
	CUserWindow(void)
	{
		...
		m_Bitmap = m_UserBitmap = m_TimerBitmap = NULL;
		m_Timer = 0;
...
public:
	virtual ATL::CWndClassInfo& GetWndClassInfo(void)
	{
		...
		{
			{
				sizeof(WNDCLASSEX),CS_DROPSHADOW | CS_DBLCLKS, StartWindowProc, 0, 0,NULL,NULL,NULL,
				(HBRUSH)(COLOR_MENU + 1),NULL, L"OpenShell.CMenuContainer",NULL
			},
...
private:
	...
	enum
	{
		...
		CONTAINER_PROGRAMS = 0x0000008,
		// this is a folder from the Start Menu hierarchy (drop operations prefer link over move)
		CONTAINER_DOCUMENTS = 0x0000010, // sort by time, limit the count (for recent documents)
		CONTAINER_ALLPROGRAMS = 0x0000020,
		// this is the main menu of All Programs (combines the Start Menu and Programs folders)
	...
	struct MenuItem
	{
		MenuItem(TMenuID _id = MENU_NO)
		{
			...
			bFolder = bLink = bPrograms = bAlignBottom = bBreak = bInline = bInlineFirst = bInlineLast = bSplit =
				bHasJumpList = bMetroLink = bMetroApp = bBlankSeparator = bNew = bStartScreen = bCustomAccelerator =
				false;
			priority = 0;
			pItem1 = pItem2 = NULL;
			mfuHash = 0;
		...
		char priority;
		// used for sorting of the All Programs menu (0 for Start Menu, 1 for the separator, 2 for the Programs)
		char drive;
		...
		struct MruNameComparator
		{
			...
			{
				return CompareMenuString((const wchar_t*)item1.name + item1.nameOffset,
				                         (const wchar_t*)item2.name + item2.nameOffset) < 0;
			}
		};
	...
	struct SortMenuItem
	{
		...
		SortMenuItem(const CString& _name, unsigned _nameHash, bool _bFolder, bool _bHasJumpList, char _priority,
		             char _drive)
		{
			name = _name;
			nameHash = _nameHash;
			bFolder = _bFolder;
			bHasJumpList = _bHasJumpList;
			priority = _priority;
			drive = _drive;
		}

		SortMenuItem(const MenuItem& item)
		{
			name = item.name;
			nameHash = item.nameHash;
			bFolder = item.bFolder;
			bHasJumpList = item.bHasJumpList;
	...
	// keep the view alive because some buggy namespace extensions clean up if there is no view
	...
	int m_ExtraTop, m_ExtraBottom;
	// amount of padding that the menu background virtually extends beyond the screen (not actually drawn)
	int m_ExtraBorder;
	// hight of the border by which the menu extends beyond the screen (solid background with window border)
	...
	struct SearchItem
	{
		SearchItem(const CString& _name, const CItemManager::ItemInfo* _info)
		{
			name = _name;
			info = _info;
		}
	...
	int AddSearchItems(const std::vector<SearchItem>& items, const CString& categoryName, unsigned int categoryHash,
	                   int originalCount);
	...
	struct ActivateData
	{
		ActivateData(void)
		{
			bNoModifiers = bProgramsTree = bExpanded = bArrow = bAutoSort = bApps = false;
			command = 0;
			pNewItemInfo = NULL;
			hTreeItem = NULL;
		}
	...
	void ActivateTreeItem(const void* treeItem, RECT& itemRect, TActivateType type, const POINT* pPt,
	                      ActivateData* pData = NULL);
	void DragTreeItem(const void* treeItem, bool bApps);
	void ShowKeyboardCues(bool alt);
	void SetActiveWindow(void);
	void CreateBackground(int width1, int width2, int height1, int height2, int& totalWidth, int& totalHeight,
	                      bool bCreateRegion); // width1/2, height1/2 - the first and second content area
...
#define AW_TOPMOST 0x00100000
	...
	static DWORD s_HotPos;
	// last mouse position over a hot item (used to ignore WM_MOUSEMOVE when the mouse didn't really move)
	...
	friend LRESULT CALLBACK SubclassTopMenuProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
	                                            UINT_PTR uIdSubclass, DWORD_PTR dwRefData);
	friend HRESULT CreatePinLink(PCIDLIST_ABSOLUTE sourcePidl, const wchar_t* name, const wchar_t* iconPath,
	                             int iconIndex);
	...
	static LRESULT CALLBACK SubclassSearchBox(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
	                                          DWORD_PTR dwRefData);
	static LRESULT CALLBACK SubclassScrollbar(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
	                                          DWORD_PTR dwRefData);
...
public:
	...
	CSubMenuContainer(CMenuContainer* pParent, int index, int options, const StdMenuItem* pStdItem,
