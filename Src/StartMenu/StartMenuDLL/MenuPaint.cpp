	...
	IUserTileStore8 : public IUnknown
{
	...
	virtual HRESULT STDMETHODCALLTYPE GetUserTilePathBySize(const wchar_t* userSid, unsigned int size, wchar_t** pPath)
	= 0;
	virtual HRESULT STDMETHODCALLTYPE GetDynamicPathBySize(const wchar_t* userSid, unsigned int size, wchar_t** pPath) =
	0;
};
...
static void StretchBlt2(HDC hdcDest, int xDest, int yDest, int wDest, int hDest, HDC hdcSrc, int xSrc, int ySrc,
                        int wSrc, int hSrc, bool bAlpha)
{
...
HBITMAP CMenuContainer::LoadUserImage(int size, HBITMAP hMask)
{
	...
	if (str.IsEmpty())
	{
		if (GetWinVersion() == WIN_VER_WIN8)
		{
			...
			{
				CComString pPath;
				if (SUCCEEDED(pStore->GetDynamicPathBySize(NULL,size,&pPath)) || SUCCEEDED(
					pStore->GetUserTilePathBySize(NULL,size,&pPath)))
		...
		if (!path[0])
		{
			...
			tSHGetUserPicturePath SHGetUserPicturePath = (tSHGetUserPicturePath)GetProcAddress(
				hShell32,MAKEINTRESOURCEA(261));
	...
	if (hBitmap && hMask)
	{
		...
		if (info1.bmBits && info2.bmBits && info1.bmWidth == info2.bmWidth && info1.bmHeight == info2.bmHeight && info1.
			bmBitsPixel == 32 && info2.bmBitsPixel == 32)
		{
...
static HBITMAP CreateMaskedEmblem(HBITMAP bmpEmblem, RECT& rcEmblem, const unsigned int* EMaskBits, int maskWidth,
                                  int maskHeight, int channel)
{
	...
	const unsigned char* srcBits = (unsigned char*)info.bmBits + (info.bmHeight - rcEmblem.top + miny - 1) * info.
		bmWidthBytes + (minx - rcEmblem.left) * (info.bmBitsPixel / 8);
...
void CMenuContainer::CreateBackground(int width1, int width2, int height1, int height2, int& totalWidth,
                                      int& totalHeight, bool bCreateRegion)
{
	...
	if (m_SearchIndex >= 0 && s_bWin7Style)
	{
		...
		int searchH = settings.textMetrics.tmHeight * 12 / 8 + s_Skin.Search_padding.top + s_Skin.Search_padding.bottom;
		// 12 DLUs
	...
	if (s_Skin.Patterns[0].GetBitmap() || s_Skin.Patterns[1].GetBitmap() || s_Skin.Patterns[2].GetBitmap() || s_Skin.
		Patterns[3].GetBitmap())
	{
		bHasPattern = (bmpSkinPattern.GetBitmap() || bmpSkinPattern.GetColor() || bmpSearchPattern.GetBitmap() ||
			bmpSearchPattern.GetColor());
	}

	bool bHasEmblemMask = false;
	if (s_Skin.Main_emblems[0].GetBitmap() || s_Skin.Main_emblems[1].GetBitmap() || s_Skin.Main_emblems[2].GetBitmap()
		|| s_Skin.Main_emblems[3].GetBitmap())
	{
		bHasEmblemMask = (bmpSkinEmblem.GetBitmap() || bmpSkinEmblem.GetColor() || bmpSearchEmblem.GetBitmap() ||
			bmpSearchEmblem.GetColor());
	}
	...
	if (bmpSkin.GetBitmap())
	{
		...
		if (totalWidth2 > 0)
		{
			...
			if (searchHeight > 0)
			{
				...
				rMargins2.top = slicesY[3];
				rMargins2.bottom = slicesY[5];
				MarginsBlit(hdcTemp, hdc, rSrc2, rDst2, rMargins2,
				            (opacity == MenuSkin::OPACITY_SOLID && bmpSkin.bIs32));
				if (bmpSkinPattern.GetBitmap())
					MarginsBlit(hdcPMaskTemp, hdcPMask, rSrc2, rDst2, rMargins2, false);
				if (bmpSkinEmblem.GetBitmap())
					MarginsBlit(hdcEMaskTemp, hdcEMask, rSrc2, rDst2, rMargins2, false);
		...
		if (bmpSearch.GetBitmap())
		{
			...
			for (int y = 0; y < 3; y++)
			{
				for (int x = 0; x < 3; x++)
				{
					...
					RECT rMargins = {
						searchSlicesX[x * 3], searchSlicesY[y * 3], searchSlicesX[x * 3 + 2], searchSlicesY[y * 3 + 2]
					};
	...
	if (width2)
	{
		if (s_Skin.Main_separatorV.GetBitmap())
		{
			...
			RECT rSrc2 = {
				0, 0, s_Skin.Main_separatorWidth,
				s_Skin.Main_separator_slices_Y[0] + s_Skin.Main_separator_slices_Y[1] + s_Skin.Main_separator_slices_Y[
					2]
			};
			RECT rDst2 = {
				totalWidth1, menuPadding[0].top, totalWidth1 + s_Skin.Main_separatorWidth,
				totalHeight - menuPadding[0].bottom
			};
			if (s_bRTL)
			{
	...
	for (int i = 0; i < _countof(s_Skin.Main_emblems); i++)
	{
		if (s_Skin.Main_emblems[i].GetBitmap())
		{
			...
			if (width2 == 0 && (alignH == MenuSkin::HALIGN_LEFT2 || alignH == MenuSkin::HALIGN_RIGHT2 || alignH ==
				MenuSkin::HALIGN_CENTER2))
			...
			if (alignH == MenuSkin::HALIGN_CENTER1 || alignH == MenuSkin::HALIGN_LEFT1 || alignH ==
				MenuSkin::HALIGN_RIGHT1)
				x1 = totalWidth1;
			else if (alignH == MenuSkin::HALIGN_CENTER2 || alignH == MenuSkin::HALIGN_LEFT2 || alignH ==
				MenuSkin::HALIGN_RIGHT2)
				x0 = totalWidth1;
			if (alignH == MenuSkin::HALIGN_LEFT || alignH == MenuSkin::HALIGN_LEFT1 || alignH == MenuSkin::HALIGN_LEFT2)
				x1 = x0 + w;
			else if (alignH == MenuSkin::HALIGN_RIGHT || alignH == MenuSkin::HALIGN_RIGHT1 || alignH ==
				MenuSkin::HALIGN_RIGHT2)
				x0 = x1 - w;
			else if (alignH == MenuSkin::HALIGN_CENTER || alignH == MenuSkin::HALIGN_CENTER1 || alignH ==
				MenuSkin::HALIGN_CENTER2)
			...
			if (bHasEmblemMask && i < 4)
				maskedEmblem = CreateMaskedEmblem(s_Skin.Main_emblems[i].GetBitmap(), rcEmblem, EMaskBits, totalWidth,
				                                  totalHeight, i);
	...
	if (bCaption)
	{
		...
		RECT rc = {
			s_Skin.Caption_padding.bottom, s_bRTL ? s_Skin.Caption_padding.right : s_Skin.Caption_padding.left,
	...
	if (s_Skin.User_image_size && !s_bWin7Style)
	{
		// draw user image
		HBITMAP userPicture;
		bool bMask = (s_Skin.User_mask.bIsBitmap && s_Skin.User_mask.bIs32 && s_Skin.User_maskSize.cx == s_Skin.
			User_image_size && s_Skin.User_maskSize.cy == s_Skin.User_image_size);
		if (bMask)
		...
		if (userPicture)
		{
			...
			else
			{
				...
				AlphaBlend(hdc, pos.x, pos.y, s_Skin.User_image_size, s_Skin.User_image_size, hdcTemp, 0, 0,
				           s_Skin.User_image_size, s_Skin.User_image_size, func);
			}
			...
			if (s_Skin.User_bitmap.GetBitmap())
			{
				...
				BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
				AlphaBlend(hdc, pos.x, pos.y, frameSize.cx, frameSize.cy, hdcTemp, 0, 0, frameSize.cx, frameSize.cy,
				           func);
			}
	...
	if (s_Skin.User_name_position.left != s_Skin.User_name_position.right)
	{
		RECT rc0;
		int x0 = 0, x1 = totalWidth;
		if (s_Skin.User_name_align == MenuSkin::HALIGN_CENTER1 || s_Skin.User_name_align == MenuSkin::HALIGN_LEFT1 ||
			s_Skin.User_name_align == MenuSkin::HALIGN_RIGHT1)
			x1 = totalWidth1;
		else if (s_Skin.User_name_align == MenuSkin::HALIGN_CENTER2 || s_Skin.User_name_align == MenuSkin::HALIGN_LEFT2
			|| s_Skin.User_name_align == MenuSkin::HALIGN_RIGHT2)
			x0 = totalWidth1;
		...
		if (Strlen(name) > 0)
		{
			...
			if (s_Skin.User_name_align == MenuSkin::HALIGN_LEFT || s_Skin.User_name_align == MenuSkin::HALIGN_LEFT1 ||
				s_Skin.User_name_align == MenuSkin::HALIGN_LEFT2)
			...
			else
			{
				...
				SetTextColor(hdcTemp, 0xFFFFFF);
				SetBkMode(hdcTemp,TRANSPARENT);
				DrawText(hdcTemp, name, -1, &rc,
				         align | DT_VCENTER | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOCLIP);
				SelectObject(hdcTemp, bmp02);
	...
	if (bCreateRegion)
	{
		...
		m_Region = NULL;
		if (opacity == MenuSkin::OPACITY_REGION || opacity == MenuSkin::OPACITY_GLASS || opacity ==
			MenuSkin::OPACITY_FULLGLASS)
		{
			for (int y = 0; y < totalHeight; y++)
			{
				int minx = -1, maxx = -1;
...
void CMenuContainer::CreateContentRects(int width1, int width2, int height1, int height2, int& totalWidth,
                                        int& totalHeight)
{
	const int* slicesY;
	RECT menuPadding[2];
...
void CMenuContainer::DrawBackground(HDC hdc, const RECT& drawRect)
{
	...
	if (m_Bitmap)
	{
		...
		bmpMain = m_Bitmap;
		{
			RECT rc = {m_rMenu.right - m_rMenu.left, m_rMenu.bottom, 0, 0};
			rMarginsMain = rc;
		}
		{
			RECT rc = {0, 0, m_rMenu.right - m_rMenu.left, m_rMenu.bottom};
			rSrcMain = rc;
		}
		bAlphaMain = false;
	}
	...
	for (int index = 0;; index++)
	{
		...
		if (m_ScrollHeight > 0 && index == m_ScrollCount)
		{
			...
			if (m_bScrollUp)
			{
				...
				if (bmpPager.GetBitmap() && bmpPagerArrows.GetBitmap())
				{
					...
					RECT rSrc = {
						0, 0, pagSlicesX[0] + pagSlicesX[1] + pagSlicesX[2],
						pagSlicesY[0] + pagSlicesY[1] + pagSlicesY[2]
					};
					if (m_bScrollUpHot)
					...
					{
						BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
						AlphaBlend(hdc, x, y, pagArrowSize.cx, pagArrowSize.cy, hdc2,
						           m_bScrollUpHot ? pagArrowSize.cx : 0, 0, pagArrowSize.cx, pagArrowSize.cy, func);
					}
					else
						BitBlt(hdc, x, y, pagArrowSize.cx, pagArrowSize.cy, hdc2, m_bScrollUpHot ? pagArrowSize.cx : 0,
						       0,SRCCOPY);
					SelectObject(hdc2, bmp0);
				}
				else
				{
					if (s_PagerTheme)
						DrawThemeBackground(s_PagerTheme, hdc, SBP_ARROWBTN, (m_bScrollUpHot ? ABS_UPHOT : ABS_UPHOVER),
						                    &rc,NULL);
					else
						DrawFrameControl(hdc, &rc,DFC_SCROLL,
						                 DFCS_SCROLLUP | DFCS_FLAT | (m_bScrollUpHot ? DFCS_PUSHED : 0));
				}
			}
			if (m_bScrollDown)
			{
				int bottom = clipBottom + s_Skin.ItemSettings[m_bSubMenu
					                                              ? MenuSkin::SUBMENU_ITEM
					                                              : MenuSkin::COLUMN1_ITEM].itemHeight;
				if (bottom >= m_rMenu.bottom) bottom = m_rMenu.bottom;
				if (bottom > clipBottom && (glow || opacity == MenuSkin::OPACITY_FULLALPHA || opacity ==
					MenuSkin::OPACITY_FULLGLASS))
				{
					// fix background behind the up button (DrawThemeTextEx may spill onto the tablecloth)
				...
				if (bmpPager.GetBitmap() && bmpPagerArrows.GetBitmap())
				{
					...
					if (bmpPagerArrows.bIs32)
					{
						BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
						AlphaBlend(hdc, x, y, pagArrowSize.cx, pagArrowSize.cy, hdc2,
						           m_bScrollDownHot ? pagArrowSize.cx : 0, pagArrowSize.cy, pagArrowSize.cx,
						           pagArrowSize.cy, func);
					}
					else
						BitBlt(hdc, x, y, pagArrowSize.cx, pagArrowSize.cy, hdc2,
						       m_bScrollDownHot ? pagArrowSize.cx : 0, pagArrowSize.cy,SRCCOPY);
					SelectObject(hdc2, bmp0);
				}
				else
				{
					if (s_PagerTheme)
						DrawThemeBackground(s_PagerTheme, hdc, SBP_ARROWBTN,
						                    (m_bScrollDownHot ? ABS_DOWNHOT : ABS_DOWNHOVER), &rc,NULL);
					else
						DrawFrameControl(hdc, &rc,DFC_SCROLL,
						                 DFCS_SCROLLDOWN | DFCS_FLAT | (m_bScrollDownHot ? DFCS_PUSHED : 0));
				}
			}
		}
		...
		{
			if (index - m_OriginalCount < m_SearchScrollPos || index - m_OriginalCount >= m_SearchScrollPos +
				m_SearchScrollHeight)
				continue;
		...
		if (bHot || bNew || item.id == MENU_SHUTDOWN_BUTTON || index == m_DropTargetIndex)
		{
			...
			{
				bSplit = (item.bSplit && (item.bFolder || item.jumpIndex >= 0 || item.id == MENU_SEARCH_CATEGORY || item
				...
				else
				{
					if ((index == m_HotItem && !m_bHotArrow) || index == m_ContextItem)
						stateLeft = 1;
					if ((index == m_HotItem && m_bHotArrow) || index == m_Submenu || index == m_SubJumpItem || index ==
						m_ContextItem)
						stateRight = 1;
				}
			...
			if (bSplit && settings.bmpSelection.GetBitmap())
			{
				...
				HGDIOBJ bmp0 = SelectObject(hdc2, settings.bmpSelection.GetBitmap());
				{
					// draw left half
					RECT rSrc = {
						0, 0, settings.selSlicesX[0] + settings.selSlicesX[1] + settings.selSlicesX[2], srcHeight
					};
					OffsetRect(&rSrc, 0, srcHeight * stateLeft);
					RECT rMargins = {
						settings.selSlicesX[0], settings.selSlicesY[0], settings.selSlicesX[2], settings.selSlicesY[2]
					};
				...
				}
				{
					// draw right half
					RECT rSrc = {
						settings.selSlicesX[0] + settings.selSlicesX[1] + settings.selSlicesX[2], 0, 0, srcHeight
					};
					...
					RECT rMargins = {
						settings.selSlicesX[3], settings.selSlicesY[0], settings.selSlicesX[5], settings.selSlicesY[2]
					};
					RECT itemRect2 = itemRect;
			...
			else if (settings.bmpSelection.GetBitmap())
			{
				HGDIOBJ bmp0 = SelectObject(hdc2, settings.bmpSelection.GetBitmap());
				RECT rSrc = {
					0, 0, settings.selSlicesX[0] + settings.selSlicesX[1] + settings.selSlicesX[2],
					settings.selSlicesY[0] + settings.selSlicesY[1] + settings.selSlicesY[2]
				};
				{
					RECT rMargins = {
						settings.selSlicesX[0], settings.selSlicesY[0], settings.selSlicesX[2], settings.selSlicesY[2]
		...
		if (item.id == MENU_SEPARATOR || item.id == MENU_SEARCH_CATEGORY)
		{
			...
			if (!item.bBlankSeparator && itemRect.bottom > itemRect.top)
			{
				...
				if (item.id == MENU_SEARCH_CATEGORY)
				{
					...
					if (s_Skin.Search_arrow.GetBitmap())
					{
						...
						{
							BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
							AlphaBlend(hdc, x, y, s_Skin.Search_arrow_size.cx, s_Skin.Search_arrow_size.cy, hdc2, 0, v,
							           s_Skin.Search_arrow_size.cx, s_Skin.Search_arrow_size.cy, func);
						}
						else
							BitBlt(hdc, x, y, s_Skin.Search_arrow_size.cx, s_Skin.Search_arrow_size.cy, hdc2, 0, v,
							       SRCCOPY);
						SelectObject(hdc2, bmp0);
					...
					else
					{
						...
						HGDIOBJ bmp0 = SelectObject(
							hdc2, GetArrowsBitmap(settings.arrColors[((bHot && (!bSplit || stateLeft > 0)) ? 1 : 0)]));
						BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
						if (m_SearchCategoryHash == item.categoryHash)
						{
							SIZE s = {sizes[5].y - sizes[5].x, sizes[6].y};
							AlphaBlend(hdc, x, y, s.cx, s.cy, hdc2, s_bRTL ? sizes[6].x - sizes[5].y : sizes[5].x, 0,
							           s.cx, s.cy, func);
						}
						else
						{
							...
							AlphaBlend(hdc, x + dx, y, s.cx, s.cy, hdc2, s_bRTL ? sizes[6].x - sizes[4].y : sizes[3].x,
							           0, s.cx, s.cy, func);
						}
						SelectObject(hdc2, bmp0);
				...
				{
					if (item.id == MENU_SEARCH_CATEGORY && !item.bSplit && item.categoryHash >=
						CSearchManager::CATEGORY_FILE && index == m_HotItem && m_bHotArrow)
					...
					RECT rc = {
						itemRect2.left + settings.textPadding.left,
						itemRect2.top + settings.textTopOffset + settings.textPadding.top - settings.textPadding.bottom,
						itemRect2.right - settings.arrSize.cx - settings.arrPadding.cx - settings.arrPadding.cy -
						settings.textPadding.right,
						itemRect2.bottom - settings.textTopOffset
					};
					DWORD flags = DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOPREFIX;
					if (s_Theme)
					{
						DTTOPTS opts = {sizeof(opts),DTT_TEXTCOLOR};
						if (glowSize || settings.opacity == MenuSkin::OPACITY_FULLALPHA || settings.opacity ==
							MenuSkin::OPACITY_FULLGLASS)
				...
				if (itemRect2.left < itemRect2.right)
				{
					if (settings.bmpSeparator.GetBitmap())
					{
						...
						RECT rSrc = {
							0, 0, settings.sepSlicesX[0] + settings.sepSlicesX[1] + settings.sepSlicesX[2],
							settings.sepHeight
						};
			...
			if (bHot && item.bSplit && item.id == MENU_SEARCH_CATEGORY)
			{
				...
				{
					BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
					AlphaBlend(hdc, x, y, s_Skin.More_bitmap_Size.cx, s_Skin.More_bitmap_Size.cy, hdc2, 0,
					           stateRight == 0 ? s_Skin.More_bitmap_Size.cy : 0, s_Skin.More_bitmap_Size.cx,
					           s_Skin.More_bitmap_Size.cy, func);
				}
				else
				{
					BitBlt(hdc, x, y, s_Skin.More_bitmap_Size.cx, s_Skin.More_bitmap_Size.cy, hdc2, 0,
					       stateRight == 0 ? s_Skin.More_bitmap_Size.cy : 0,SRCCOPY);
				}
				SelectObject(hdc2, bmp0);
			}
			continue;
		...
		if (item.id == MENU_SEARCH_BOX)
		{
			...
			OffsetRect(&rDst, (itemRect.right + itemRect.left - iconSize) / 2,
			           (itemRect.bottom + itemRect.top - iconSize) / 2);
		...
		else if (settings.iconSize == MenuSkin::ICON_SIZE_PROGRAMS)
		{
			...
			{
				iconSize.cx = s_Skin.ItemSettings[MenuSkin::COLUMN1_ITEM].iconSize == MenuSkin::ICON_SIZE_SMALL
					              ? g_ItemManager.SMALL_ICON_SIZE
		...
		if (drawType == MenuSkin::PROGRAMS_BUTTON || drawType == MenuSkin::PROGRAMS_BUTTON_NEW)
		{
			if (s_Skin.Programs_icon.GetBitmap())
			{
				...
				{
					BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
					AlphaBlend(hdc, iconX, iconY, s_Skin.Programs_icon_size.cx, s_Skin.Programs_icon_size.cy, hdc2, 0,
					           s_MenuMode == MODE_PROGRAMS ? s_Skin.Programs_icon_size.cy : 0,
					           s_Skin.Programs_icon_size.cx, s_Skin.Programs_icon_size.cy, func);
				}
				else
					BitBlt(hdc, iconX, iconY, s_Skin.Programs_icon_size.cx, s_Skin.Programs_icon_size.cy, hdc2, 0,
					       s_MenuMode == MODE_PROGRAMS ? s_Skin.Programs_icon_size.cy : 0,SRCCOPY);
				SelectObject(hdc2, bmp0);
			}
			else
			{
		...
		else if (item.pItemInfo && !bNoIcon)
		{
			...
			if (settings.bmpIconFrame.GetBitmap())
			{
				...
				RECT rSrc = {
					0, 0, settings.frameSlicesX[0] + settings.frameSlicesX[1] + settings.frameSlicesX[2],
					settings.frameSlicesY[0] + settings.frameSlicesY[1] + settings.frameSlicesY[2]
				};
				if (bHot)
				...
				RECT rMargins = {
					settings.frameSlicesX[0], settings.frameSlicesY[0], settings.frameSlicesX[2],
					settings.frameSlicesY[2]
				};
			...
			if (pIcon && pIcon->bitmap)
			{
				...
				{
					BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
					AlphaBlend(hdc, iconX, iconY, iconSize.cx, iconSize.cy, hdc2, 0, 0, info.bmWidth, info.bmHeight,
					           func);
					SelectObject(hdc2, bmp0);
				}
		...
		else if (item.id == MENU_SHUTDOWN_BUTTON && s_bHasUpdates && s_Skin.Shutdown_bitmap.GetBitmap())
		{
			...
			{
				BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
				AlphaBlend(hdc, iconX, iconY, s_Skin.Shutdown_bitmap_Size.cx, s_Skin.Shutdown_bitmap_Size.cy, hdc2, 0,
				           0, s_Skin.Shutdown_bitmap_Size.cx, s_Skin.Shutdown_bitmap_Size.cy, func);
			}
			else
				BitBlt(hdc, iconX, iconY, s_Skin.Shutdown_bitmap_Size.cx, s_Skin.Shutdown_bitmap_Size.cy, hdc2, 0, 0,
				       SRCCOPY);
			SelectObject(hdc2, bmp0);
		}
		...
		SelectObject(hdc, settings.font);
		RECT rc = {
			itemRect.left + settings.iconPadding.left + settings.iconPadding.right + settings.textPadding.left,
		...
		if (drawType == MenuSkin::PROGRAMS_BUTTON || drawType == MenuSkin::PROGRAMS_BUTTON_NEW || drawType ==
			...
			name = s_MenuMode == MODE_PROGRAMS
				       ...
				       : FindTranslation(L"Menu.AllPrograms", L"All Programs");
		else
			name = item.name;
		...
		if (item.bFolder && drawType != MenuSkin::PROGRAMS_BUTTON && drawType != MenuSkin::PROGRAMS_BUTTON_NEW)
		{
			...
			if (settings.bmpArrow.GetBitmap())
			{
				...
				{
					BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
					AlphaBlend(hdc, x, y, settings.arrSize.cx, settings.arrSize.cy, hdc2, 0,
					           bHotArrow ? settings.arrSize.cy : 0, settings.arrSize.cx, settings.arrSize.cy, func);
				}
				else
				{
					BitBlt(hdc, x, y, settings.arrSize.cx, settings.arrSize.cy, hdc2, 0,
					       bHotArrow ? settings.arrSize.cy : 0,SRCCOPY);
				}
				SelectObject(hdc2, bmp0);
			}
			else
			{
				...
				BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
				AlphaBlend(hdc, x, y, s.cx, s.cy, hdc2, s_bRTL ? sizes[6].x - sizes[4].y : sizes[3].x, 0, s.cx, s.cy,
				           func);
				SelectObject(hdc2, bmp0);
			}
		}
		else if (bHot && item.bSplit && item.jumpIndex >= 0)
		{
			...
			{
				BLENDFUNCTION func = {AC_SRC_OVER, 0, 255,AC_SRC_ALPHA};
				AlphaBlend(hdc, x, y, s_Skin.Pin_bitmap_Size.cx, s_Skin.Pin_bitmap_Size.cy, hdc2,
				           bPinned ? s_Skin.Pin_bitmap_Size.cx : 0, stateRight == 0 ? s_Skin.Pin_bitmap_Size.cy : 0,
				           s_Skin.Pin_bitmap_Size.cx, s_Skin.Pin_bitmap_Size.cy, func);
			}
			else
			{
				BitBlt(hdc, x, y, s_Skin.Pin_bitmap_Size.cx, s_Skin.Pin_bitmap_Size.cy, hdc2,
				       bPinned ? s_Skin.Pin_bitmap_Size.cx : 0, stateRight == 0 ? s_Skin.Pin_bitmap_Size.cy : 0,
				       SRCCOPY);
			}
			SelectObject(hdc2, bmp0);
	...
	// draw insert mark
	{
		...
		if (GetInsertRect(rc))
		{
			HGDIOBJ bmp0 = SelectObject(hdc2, GetArrowsBitmap(
				                            s_Skin.ItemSettings[m_bSubMenu
					                                                ? MenuSkin::SUBMENU_ITEM
					                                                : MenuSkin::COLUMN1_ITEM].arrColors[0]));
			// the insert mask can't be in the second column of the main menu
			const POINT* sizes = s_Skin.GetArrowsBitmapSizes();
			RECT rSrc = {
				s_bRTL ? sizes[6].x - sizes[2].y : sizes[0].x, 0, s_bRTL ? sizes[6].x - sizes[0].x : sizes[2].y,
				sizes[1].y
			};
...
LRESULT CMenuContainer::OnPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if ((!m_bSubMenu && m_Bitmap) || (m_bSubMenu && s_Skin.Submenu_bitmap.GetBitmap()))
	{
		...
		{
			DWM_BLURBEHIND blur = {
				DWM_BB_ENABLE | ((uMsg == WM_PRINTCLIENT) ? DWM_BB_BLURREGION : 0U), (uMsg == WM_PRINTCLIENT), m_Region,
				FALSE
			};
			DwmEnableBlurBehindWindow(m_hWnd, &blur);
		}
	...
	if (hdcPaint)
	{
		if (s_OldMenuState.mode != MODE_UNKNOWN)
		{
			...
			BitBlt(hdcPaint, rc2.left, rc2.top, rc2.right - rc2.left, rc2.bottom - rc2.top, hSrc,
			       rc2.left - m_BitmapOffset, rc2.top,SRCCOPY);
		...
		if (m_SearchBox.m_hWnd && ((uMsg == WM_PRINTCLIENT && (lParam & PRF_CHILDREN)) || (uMsg == WM_PAINT && !
			m_bSearchDrawn)))
		{
			RECT rc;
			GetWindowRect(&rc);
		...
		{
			bool bDrawTree = (uMsg == WM_PAINT && (m_PaintOffset.x != 0 || m_PaintOffset.y != 0)) || (uMsg ==
				WM_PRINTCLIENT && (lParam & PRF_CHILDREN));
			if (bDrawTree)
			{
				RECT rc;
		...
		if (opacity == MenuSkin::OPACITY_REGION && uMsg == WM_PRINTCLIENT && m_bSubMenu && s_Skin.Submenu_bitmap.
			bIsBitmap && !s_Skin.Submenu_bitmap.bIs32)
		{
		...
		else
		{
			if (s_OldMenuState.mode == MODE_UNKNOWN)
			{
				if (opacity == MenuSkin::OPACITY_GLASS || opacity == MenuSkin::OPACITY_ALPHA || (opacity ==
					MenuSkin::OPACITY_REGION && uMsg == WM_PRINTCLIENT))
				{
					RECT rc;
					IntersectRect(&rc, &ps.rcPaint, &m_rContent);
					BufferedPaintSetAlpha(hBufferedPaint, &rc, 255);
				}
				if (m_bTwoColumns && (opacity2 == MenuSkin::OPACITY_GLASS || opacity2 == MenuSkin::OPACITY_ALPHA || (
					opacity2 == MenuSkin::OPACITY_REGION && uMsg == WM_PRINTCLIENT)))
				{
					RECT rc;
			...
			if (m_SearchIndex >= 0 && s_bWin7Style && ((s_Skin.Search_background_jump.GetBitmap() && !s_Skin.
				Search_background_jump.bIs32) || !s_Skin.Search_background_jump.bIsBitmap))
			{
				RECT padding;
				if (s_MenuMode == MODE_SEARCH)
...
void CProgramsTree::DrawTree(HDC hdc, const RECT& drawRect)
{
	...
	if (hdcPaint)
	{
		...
		if (skin.Programs_background == 0)
		{
			...
			BitBlt(hdcPaint, drawRect.left, drawRect.top, drawRect.right - drawRect.left,
			       drawRect.bottom - drawRect.top, hsrc, bmpRect.left, bmpRect.top,SRCCOPY);
			SelectObject(hsrc, bmp0);
			alpha = (skin.Main_opacity == MenuSkin::OPACITY_ALPHA || skin.Main_opacity == MenuSkin::OPACITY_GLASS)
				        ? 255
				        : 0;
		}
		else
		...
		for (HTREEITEM hItem = TreeView_GetFirstVisible(m_hWnd); hItem; hItem = TreeView_GetNextVisible(m_hWnd, hItem))
		{
			...
			if (itemRect.top >= drawRect.bottom)
				break;
			bool bHot = ((hItem == hSelection && m_DropLocation == DROP_NOWHERE) || hItem == hHilight) && (m_pOwner->
				m_HotItem == m_pOwner->m_ProgramTreeIndex || focus == m_hWnd);
		...
		if (GetInsertRect(rc))
		{
			...
			HGDIOBJ bmp0 = SelectObject(
				hsrc, CMenuContainer::GetArrowsBitmap(
					CMenuContainer::s_Skin.ItemSettings[MenuSkin::PROGRAMS_TREE_ITEM].textColors[0]));
			RECT rSrc = {
				CMenuContainer::s_bRTL ? sizes[6].x - sizes[2].y : sizes[0].x, 0,
				CMenuContainer::s_bRTL ? sizes[6].x - sizes[0].x : sizes[2].y, sizes[1].y
			};
			RECT rMargins = {sizes[0].y - sizes[0].x, 0, sizes[2].y - sizes[2].x, 0};
...
void CProgramsTree::DrawTreeItem(HDC hdc, HDC hsrc, HTREEITEM hItem, const RECT& itemRect, bool bHot) const
{
	...
	{
		DTTOPTS opts = {sizeof(opts),DTT_TEXTCOLOR};
		if (glowSize || skin.ItemSettings[MenuSkin::PROGRAMS_TREE_ITEM].opacity == MenuSkin::OPACITY_FULLALPHA || skin.
			ItemSettings[MenuSkin::PROGRAMS_TREE_ITEM].opacity == MenuSkin::OPACITY_FULLGLASS)
			opts.dwFlags |= DTT_COMPOSITED;
		if (glowSize)
		{
			opts.dwFlags |= DTT_GLOWSIZE;
		...
		if (shadowColor != 0xFFFFFFFF)
		{
			...
			RECT rc2 = rc;
			OffsetRect(&rc2, 1, 1);
			DrawThemeTextEx(m_TreeTheme, hdc, 0, 0, pItem->name, pItem->name.GetLength(),DT_SINGLELINE | DT_NOPREFIX,
			                &rc2, &opts);
		}
		opts.crText = textColor;
		DrawThemeTextEx(m_TreeTheme, hdc, 0, 0, pItem->name, pItem->name.GetLength(),DT_SINGLELINE | DT_NOPREFIX, &rc,
		                &opts);
	}
	else
	{
...
void CProgramsTree::DrawScrollbarBackground(HDC hdc, int iPartId, int iStateId, LPCRECT pRect)
{
	...
	if (iPartId == SBP_ARROWBTN)
	{
		...
		if (skin.Scrollbar_button.bIsBitmap)
		{
			...
			int w = skin.Scrollbar_button_slices_X[0] + skin.Scrollbar_button_slices_X[1] + skin.
				Scrollbar_button_slices_X[2];
			int h = skin.Scrollbar_button_slices_Y[0] + skin.Scrollbar_button_slices_Y[1] + skin.
				Scrollbar_button_slices_Y[2];
			RECT rSrc = {0, h * state, w, h * (state + 1)};
			RECT rMargins = {
				skin.Scrollbar_button_slices_X[0], skin.Scrollbar_button_slices_Y[0], skin.Scrollbar_button_slices_X[2],
				skin.Scrollbar_button_slices_Y[2]
			};
		...
		StretchBlt2(hdc, x, y, skin.Scrollbar_arrows_size.cx, skin.Scrollbar_arrows_size.cy, hSrc, arrowOffset,
		            skin.Scrollbar_arrows_size.cy * state, skin.Scrollbar_arrows_size.cx, skin.Scrollbar_arrows_size.cy,
		            true);
		SelectObject(hSrc, bmp0);
	}
	else if (iPartId == SBP_LOWERTRACKVERT || iPartId == SBP_UPPERTRACKVERT)
	{
		...
		if (skin.Scrollbar_background.bIsBitmap)
		{
			...
			int w = skin.Scrollbar_background_slices_X[0] + skin.Scrollbar_background_slices_X[1] + skin.
				Scrollbar_background_slices_X[2];
			int h = skin.Scrollbar_background_slices_Y[0] + skin.Scrollbar_background_slices_Y[1] + skin.
				Scrollbar_background_slices_Y[2];
			RECT rSrc = {0, h * state, w, h * (state + 1)};
			RECT rMargins = {
				skin.Scrollbar_background_slices_X[0], skin.Scrollbar_background_slices_Y[0],
				skin.Scrollbar_background_slices_X[2], skin.Scrollbar_background_slices_Y[2]
			};
			MarginsBlit(hSrc, hdc, rSrc, *pRect, rMargins, false);
			SelectObject(hSrc, bmp0);
		}
	...
	else if (iPartId == SBP_THUMBBTNVERT)
	{
		...
		if (skin.Scrollbar_thumb.bIsBitmap)
		{
			...
			int w = skin.Scrollbar_thumb_slices_X[0] + skin.Scrollbar_thumb_slices_X[1] + skin.Scrollbar_thumb_slices_X[
				2];
			int h = skin.Scrollbar_thumb_slices_Y[0] + skin.Scrollbar_thumb_slices_Y[1] + skin.Scrollbar_thumb_slices_Y[
				2];
			RECT rSrc = {0, h * state, w, h * (state + 1)};
			RECT rMargins = {
				skin.Scrollbar_thumb_slices_X[0], skin.Scrollbar_thumb_slices_Y[0], skin.Scrollbar_thumb_slices_X[2],
				skin.Scrollbar_thumb_slices_Y[2]
			};
			MarginsBlit(hSrc, hdc, rSrc, *pRect, rMargins, false);
		}
		else
	...
	else if (iPartId == SBP_GRIPPERVERT)
	{
		...
		if (skin.Scrollbar_gripper.GetBitmap())
		{
			...
			StretchBlt2(hdc, x, y, skin.Scrollbar_gripper_size.cx, skin.Scrollbar_gripper_size.cy, hSrc, 0,
			            skin.Scrollbar_gripper_size.cy * state, skin.Scrollbar_gripper_size.cx,
			            skin.Scrollbar_gripper_size.cy, true);
			SelectObject(hSrc, bmp0);
		}
	}
	DeleteDC(hSrc);
...
void CMenuContainer::AnimateMenu(int flags, int speed, const RECT& rect)
{
	...
	bool bUserPic = (!m_bSubMenu && s_bWin7Style && s_UserPicture.m_hWnd && s_UserPictureRect.top < s_UserPictureRect.
		bottom);
	int frames = 0;
	...
	else if ((flags & AW_SLIDE) && speed > 0)
	{
		...
		if (!SetWindowRgn(rgn,FALSE)) // set empty region
			DeleteObject(rgn);
		SetWindowPos((flags & AW_TOPMOST) ? HWND_TOPMOST : HWND_TOP, &rect,
		             SWP_SHOWWINDOW | ((flags & AW_ACTIVATE) ? 0 : SWP_NOACTIVATE));
		if (!m_bSubMenu && s_TaskBar && s_bBehindTaskbar)
		{
			// position the start button on top
			if (s_StartButton)
				::SetWindowPos(s_StartButton, (flags & AW_TOPMOST) ? HWND_TOPMOST : HWND_TOP, 0, 0, 0, 0,
		...
		HRGN clipRgn = CreateRectRgn(clipRect.left - rect.left, clipRect.top - rect.top, clipRect.right - rect.left,
		                             clipRect.bottom - rect.top); // clip region in window space
		while (true)
		{
			auto dt = duration_cast<milliseconds>(steady_clock::now() - time0).count();
			if (dt > speed) break;
