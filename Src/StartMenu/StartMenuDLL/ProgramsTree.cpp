...
void CProgramsTree::Create(CMenuContainer* pOwner)
{
	...
	HWND hWnd = CreateWindowEx(
		...
		WC_TREEVIEW,
		NULL,
		WS_CHILD | TVS_EDITLABELS | TVS_FULLROWSELECT | (CMenuContainer::s_TipHideTime ? TVS_INFOTIP : 0) |
		TVS_NOHSCROLL | TVS_SHOWSELALWAYS | TVS_NONEVENHEIGHT,
		0,
		0,
		0,
		0,
	...
	m_ImageList = ImageList_Create(
		CItemManager::SMALL_ICON_SIZE + settings.iconPadding.left + settings.iconPadding.right,
		CItemManager::SMALL_ICON_SIZE,ILC_COLOR32 | ILC_MASK | ILC_MIRROR, 1, 16);
...
LRESULT CProgramsTree::OnKeyDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (wParam == VK_DELETE || wParam == VK_F2)
	{
		...
		if (pItem->pItemInfo1)
		{
			...
			CMenuContainer::ActivateData data;
			data.bProgramsTree = true;
			m_pOwner->ActivateTreeItem(
				pItem, rc, wParam == VK_F2 ? CMenuContainer::ACTIVATE_RENAME : CMenuContainer::ACTIVATE_DELETE,NULL,
				&data);
...
LRESULT CProgramsTree::OnMouseMove(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (m_LastMousePos.x != -1 && (m_LastMousePos.x != test.pt.x || m_LastMousePos.y != test.pt.y) &&
...
bool CProgramsTree::GetInsertRect(RECT& rc) const
{
	...
	GetClientRect(&rc);
	rc.left = itemRect.left - CItemManager::SMALL_ICON_SIZE - 3 - CMenuContainer::s_Skin.ItemSettings[
		MenuSkin::PROGRAMS_TREE_ITEM].iconPadding.right;
...
void CProgramsTree::AddFirstFolder(std::vector<CTreeItem*>& items, IShellItem* pParent, bool bPrograms)
{
	...
	while (enumerator.GetNext(pChild, childPidl))
	{
		...
		if (SUCCEEDED(pChild->GetAttributes(SFGAO_FOLDER|SFGAO_HIDDEN,&itemFlags)))
		{
			...
			pItem->bEmpty = false;
			pItem->pItemInfo1 = g_ItemManager.GetItemInfo(pChild, childPidl,
			                                              CItemManager::INFO_SMALL_ICON | CItemManager::INFO_LINK |
			                                              CItemManager::INFO_METRO);
			{
				...
				{
					if (GetSettingBool(L"HideProgramsMetro") || _wcsicmp(pItem->pItemInfo1->GetAppid(),DESKTOP_APP_ID)
						== 0)
						continue;
...
void CProgramsTree::AddMetroApps(std::vector<CTreeItem*>& items)
{
	...
	for (std::vector<MetroLink>::const_iterator it = links.begin(); it != links.end(); ++it)
	{
		const CItemManager::ItemInfo* pItemInfo = g_ItemManager.GetItemInfo(
			it->pItem, it->pidl, CItemManager::INFO_SMALL_ICON | CItemManager::INFO_LINK | CItemManager::INFO_METRO);
		CItemManager::RWLock lock(&g_ItemManager, false, CItemManager::RWLOCK_ITEMS);
...
void CProgramsTree::GetFolderItems(std::vector<CTreeItem*>& items, HTREEITEM hParent)
{
	...
	else
	{
		...
		if (GetSettingInt(L"PinnedPrograms") == PINNED_PROGRAMS_PINNED)
		{
			...
			if (!CMenuContainer::s_bNoCommonFolders &&
				SUCCEEDED(ShGetKnownFolderIDList(FOLDERID_CommonStartMenu,&pidl)))
			{
				SHCreateItemFromIDList(pidl, IID_IShellItem, (void**)&pRoot);
				if (pRoot)
				{
		...
		if (nextIndex >= 0)
		{
			for (int i = index; i < nextIndex; i++)
			{
				TVINSERTSTRUCT insert = {
					hParent, hAfter ? hAfter : TVI_FIRST, {TVIF_CHILDREN | TVIF_PARAM | TVIF_TEXT}
				};
...
void CProgramsTree::GetDragEffect(DWORD& grfKeyState, DWORD* pdwEffect)
{
	...
	if (m_pOwner->m_pDragObject->QueryGetData(&format1) != S_OK && m_pOwner->m_pDragObject->QueryGetData(&format2) !=
		S_OK && m_pOwner->m_pDragObject->QueryGetData(&format3) != S_OK)
	{
		*pdwEffect = DROPEFFECT_NONE;
		return;
	}

	bool bDragApp = m_pOwner->m_pDragObject->QueryGetData(&format3) == S_OK;
	// dragging a wrapped metro link or the Apps folder
	...
	if (CMenuContainer::s_pDragSource)
	{
		if (CMenuContainer::s_pDragSource->m_Items[CMenuContainer::s_pDragSource->m_DragIndex].id == MENU_RECENT)
			...
			*pdwEffect &= (bDragApp && !bDropApps) ? DROPEFFECT_LINK : DROPEFFECT_MOVE;
		// dragging normal item - default to move
	}

	if (bDragApp)
	{
		if (bDropApps)
			*pdwEffect &= (m_DragItem && hDropFolder && TreeView_GetParent(m_hWnd, m_DragItem) == hDropFolder)
				              ? DROPEFFECT_MOVE
				              : DROPEFFECT_NONE; // dragging a metro link to Apps folder
		else
...
HRESULT CProgramsTree::Drop(IDataObject* pDataObj, DWORD grfKeyState, POINTL pt, DWORD* pdwEffect)
{
	...
	if (m_DropTarget && (m_DropLocation == DROP_BEFORE || m_DropLocation == DROP_AFTER || m_DropLocation == DROP_LAST))
	{
		...
		else
		{
			...
			CString name;
			if (CMenuContainer::s_pDragSource && CMenuContainer::s_pDragSource->m_Items[CMenuContainer::s_pDragSource->
				m_DragIndex].pItemInfo)
			{
				const CItemManager::ItemInfo* pInfo = CMenuContainer::s_pDragSource->m_Items[
					CMenuContainer::s_pDragSource->m_DragIndex].pItemInfo;
				{
					CItemManager::RWLock lock(&g_ItemManager, false, CItemManager::RWLOCK_ITEMS);
				...
				if (name.IsEmpty())
				{
					...
					CComString pName;
					if (pItem && (SUCCEEDED(pItem->GetDisplayName(SIGDN_PARENTRELATIVEPARSING,&pName)) || SUCCEEDED(
	...
	if (!pDesktop || FAILED(pDesktop->BindToObject(dropFolder,NULL,IID_IShellFolder,(void**)&pFolder)) || FAILED(
		pFolder->CreateViewObject(g_OwnerWindow,IID_IDropTarget,(void**)&pTarget)))
		return S_OK;
	...
	for (std::vector<CMenuContainer*>::iterator it = CMenuContainer::s_Menus.begin(); it != CMenuContainer::s_Menus.
	     end(); ++it)
		if (!(*it)->m_bDestroyed)
...
void CProgramsTree::OrderElements(HTREEITEM hChild, HTREEITEM hParent, const std::vector<unsigned int>& order,
                                  bool bAutoSort, bool bResort)
{
...
void CProgramsTree::SaveRenamedOrder(HTREEITEM hItem, const CItemManager::ItemInfo* pNewInfo)
{
	...
	std::vector<unsigned int> order;
	for (HTREEITEM hChild = hParent ? TreeView_GetChild(m_hWnd, hParent) : TreeView_GetRoot(m_hWnd); hChild; hChild =
	     TreeView_GetNextSibling(m_hWnd, hChild))
	{
		...
		if (hChild == hItem)
		{
			...
			if (FAILED(SHCreateItemFromIDList(pNewInfo->GetPidl(),IID_IShellItem,(void**)&pChild)))
				return;
			CComString pName;
			if (FAILED(pChild->GetDisplayName(SIGDN_PARENTRELATIVEPARSING,&pName)) && FAILED(
				pChild->GetDisplayName(SIGDN_NORMALDISPLAY,&pName)))
				return;
			pName.MakeUpper();
