...
public:
	...
	struct CTreeItem
	{
		...
		CTreeItem(void)
		{
			nameHash = nameHashProg = 0;
			pItemInfo1 = pItemInfo2 = NULL;
			bFolder = bEmpty = bApps = bApp = bAutoSort = bPrograms = bNew = false;
			order = 0;
			folderHash = 0;
		}
		...
		{
			return pItemInfo1 == item.pItemInfo1 && pItemInfo2 == item.pItemInfo2 && name == item.name && bFolder ==
				item.bFolder && (bFolder || bNew == item.bNew);
		}
	...
	void OrderElements(HTREEITEM hChild, HTREEITEM hParent, const std::vector<unsigned int>& order, bool bAutoSort,
	                   bool bResort);
