...
bool CSearchManager::AddSearchItem(IShellItem* pItem, const wchar_t* name, int flags, TItemCategory category,
                                   SearchRequest& searchRequest)
{
	...
	item.pInfo = g_ItemManager.GetItemInfo(pItem, pidl,
	                                       CItemManager::INFO_LINK | ((flags & COLLECT_METRO)
		                                                                  ? CItemManager::INFO_METRO
		                                                                  : 0));
	{
		...
		if ((flags & COLLECT_ONLY_METRO) && !item.bMetroLink)
			return false;
		if (item.bMetroLink && (!searchRequest.bSearchMetroApps || _wcsicmp(item.pInfo->GetAppid(),DESKTOP_APP_ID) ==
			0))
			return false;
	...
	if (category == CATEGORY_PROGRAM || category == CATEGORY_SETTING || category == CATEGORY_METROSETTING)
	{
		...
		if (searchRequest.bUseRanks)
		{
			...
			ItemRank rank(hash);
			std::vector<ItemRank>::const_iterator it = std::lower_bound(m_ItemRanks.begin() + 1, m_ItemRanks.end(),
			                                                            rank);
			if (it != m_ItemRanks.end() && it->hash == rank.hash)
				item.rank = it->rank * 2;
		}
...
void CSearchManager::CollectSearchItems(IShellItem* pFolder, int flags, TItemCategory category,
                                        SearchRequest& searchRequest)
{
	...
	while (pChild = NULL, pEnum->Next(1, &pChild,NULL) == S_OK)
	{
		...
		if (SUCCEEDED(pChild->GetAttributes(SFGAO_FOLDER|SFGAO_STREAM|SFGAO_LINK|SFGAO_HIDDEN,&itemFlags)))
		{
			...
			if ((flags & COLLECT_FOLDERS) || !(itemFlags & SFGAO_FOLDER))
			{
				CComString pName;
				if (SUCCEEDED(
					pChild->GetDisplayName((flags&COLLECT_PROGRAMS)?SIGDN_PARENTRELATIVEPARSING:
						SIGDN_PARENTRELATIVEEDITING,&pName)))
				{
					pName.MakeUpper();
					bool bSkip = false;
					...
					if (!bSkip)
						AddSearchItem(pChild, pName, flags | ((itemFlags & SFGAO_FOLDER) ? COLLECT_IS_FOLDER : 0),
...
bool CSearchManager::SearchScope::ParseSearchConnector(const wchar_t* fname)
{
		...
		while (pScopeItem)
		{
			...
			if (_wcsicmp(name, L"scopeItem") == 0)
			{
				...
				if (pScopeItem->selectSingleNode(CComBSTR(L"url"), &pUrl) == S_OK && pUrl->get_text(&url) == S_OK)
				{
					...
					CComBSTR mode, depth;
					bool bExclude = (pScopeItem->selectSingleNode(CComBSTR(L"mode"), &pMode) == S_OK && pMode->
						get_text(&mode) == S_OK && _wcsicmp(mode, L"exclude") == 0);
					bool bShallow = (pScopeItem->selectSingleNode(CComBSTR(L"depth"), &pDepth) == S_OK && pDepth->
						get_text(&depth) == S_OK && _wcsicmp(depth, L"shallow") == 0);
					CString url2;
					if (bExclude)
	...
	if (res == S_OK)
	{
		...
		{
			CComBSTR type;
			bCommunications = (pType->get_text(&type) == S_OK && _wcsicmp(
				type, L"{91475FE5-586B-4EBA-8D75-D17434B8CDF6}") == 0);
		}
	}
...
void CSearchManager::SearchThread(void)
{
	...
	while (1)
	{
			...
			if (m_ProgramItems.empty() && searchRequest.bSearchPrograms)
			{
				...
				// collect programs from the start menu
				{
					...
					if (SUCCEEDED(ShGetKnownFolderItem(FOLDERID_StartMenu,&pFolder)))
						CollectSearchItems(pFolder, COLLECT_RECURSIVE | COLLECT_METRO | COLLECT_NOREFRESH,
						                   CATEGORY_PROGRAM, searchRequest);
					if (searchRequest.requestId < m_LastProgramsRequestId)
						continue;
				}
				...
				if (searchRequest.bSearchPath)
				{
					...
					for (const wchar_t* pPath = PATH; *pPath;)
					{
						...
						if (SUCCEEDED(SHCreateItemFromParsingName(token,NULL,IID_IShellItem,(void**)&pFolder)))
							CollectSearchItems(pFolder, COLLECT_PROGRAMS | COLLECT_NOREFRESH, CATEGORY_PROGRAM,
							                   searchRequest);
						if (searchRequest.requestId < m_LastProgramsRequestId)
							continue;
					}
				...
				if (GetWinVersion() >= WIN_VER_WIN8 && searchRequest.bSearchMetroApps)
				{
					...
					for (std::vector<MetroLink>::const_iterator it = links.begin(); it != links.end(); ++it)
					{
						if (GetWinVersion() < WIN_VER_WIN10)
							AddSearchItem(it->pItem, L"", COLLECT_PROGRAMS | COLLECT_METRO | COLLECT_ONLY_METRO,
							              CATEGORY_PROGRAM, searchRequest);
						else
						{
							CComString pName;
							if (SUCCEEDED(it->pItem->GetDisplayName(SIGDN_NORMALDISPLAY,&pName)))
								AddSearchItem(it->pItem, pName, COLLECT_PROGRAMS | COLLECT_METRO, CATEGORY_PROGRAM,
								              searchRequest);
						}
						if (searchRequest.requestId < m_LastProgramsRequestId)
							continue;
			...
			if (m_SettingsItems.empty() && searchRequest.bSearchSettings)
			{
				...
				// collect items from the control panel, admin tools, and the god mode
				{
					...
					if (SUCCEEDED(ShGetKnownFolderItem(FOLDERID_ControlPanelFolder,&pFolder)))
						CollectSearchItems(pFolder, COLLECT_FOLDERS | COLLECT_NOREFRESH, CATEGORY_SETTING,
						                   searchRequest);
					if (searchRequest.requestId < m_LastProgramsRequestId)
						continue;
				}
				{
					...
					if (SUCCEEDED(ShGetKnownFolderItem(FOLDERID_AdminTools,&pFolder)))
						CollectSearchItems(pFolder, COLLECT_RECURSIVE | COLLECT_NOREFRESH, CATEGORY_SETTING,
						                   searchRequest);
					if (searchRequest.requestId < m_LastProgramsRequestId)
						continue;
				}
				...
				}
				{
					CComPtr<IShellItem> pFolder;
					if (SUCCEEDED(
						SHCreateItemFromParsingName(L"shell:::{ED7BA470-8E54-465E-825C-99712043E01C}",NULL,
							IID_IShellItem,(void**)&pFolder)))
						CollectSearchItems(
							pFolder, (searchRequest.bSearchKeywords ? COLLECT_KEYWORDS : 0) | COLLECT_NOREFRESH,
							CATEGORY_SETTING, searchRequest);
					if (searchRequest.requestId < m_LastProgramsRequestId)
						continue;
				}
				if (searchRequest.bSearchMetroSettings)
				{
					CComPtr<IShellItem> pFolder;
					if (SUCCEEDED(
						SHCreateItemFromParsingName(L"shell:::{82E749ED-B971-4550-BAF7-06AA2BF7E836}",NULL,
							IID_IShellItem,(void**)&pFolder)))
						CollectSearchItems(
							pFolder, (searchRequest.bSearchKeywords ? COLLECT_KEYWORDS : 0) | COLLECT_NOREFRESH,
							CATEGORY_METROSETTING, searchRequest);
					if (searchRequest.requestId < m_LastProgramsRequestId)
						continue;
				}
			}
		...
		if (!searchRequest.autoCompletePath.IsEmpty())
		{
			...
			{
				SFGAOF itemFlags;
				if (SUCCEEDED(pFolder->GetAttributes(SFGAO_FOLDER|SFGAO_STREAM|SFGAO_LINK,&itemFlags)) && (itemFlags & (
					SFGAO_FOLDER | SFGAO_STREAM | SFGAO_LINK)) == SFGAO_FOLDER)
		...
		if (searchRequest.requestId != m_LastRequestId || (!searchRequest.bSearchFiles && !searchRequest.
			bSearchMetroSettings))
			continue;
		CMenuContainer::RefreshSearch();
		searchRequest.searchTime = GetTickCount();
		...
		CSession session;
		if (SUCCEEDED(
			dataSource.OpenFromInitializationString(
				L"provider=Search.CollatorDSO.1;EXTENDED PROPERTIES=\"Application=Windows\"")) && SUCCEEDED(
			session.Open(dataSource)))
		{
			...
			if (searchRequest.bSearchFiles)
			{
				...
				if (searchRequest.bSearchTypes && pLibrary)
				{
					...
					if (SUCCEEDED(ShGetKnownFolderItem(FOLDERID_Libraries,&pLibraries)))
					{
						...
						if (pEnum)
						{
							...
							while (pItem = NULL, pEnum->Next(1, &pItem,NULL) == S_OK)
							{
								...
								{
									int order = 1;
									if (libraries[i] && SUCCEEDED(libraries[i]->Compare(pItem,SICHINT_CANONICAL,&order))
										&& order)
									{
										bFound = true;
										break;
					...
					for (std::vector<CComPtr<IShellItem>>::const_iterator it = libraries.begin(); it != libraries.end();
					     ++it)
					{
						if (!*it) continue;
						CComString pName;
						if (SUCCEEDED(pLibrary->LoadLibraryFromItem(*it,STGM_READ)) && SUCCEEDED(
							(*it)->GetDisplayName(SIGDN_NORMALDISPLAY,&pName)))
						{
							scopeList.push_back(SearchScope());
							SearchScope& scope = *scopeList.rbegin();
							scope.bFiles = true;
							...
							if (SUCCEEDED(pLibrary->GetFolders(LFF_FORCEFILESYSTEM,IID_IShellItemArray,(void**)&pArray))
								&& pArray)
							{
						...
						if (pEnum)
						{
							...
							while (pItem = NULL, pEnum->Next(1, &pItem,NULL) == S_OK)
							{
								...
								if (GetWinVersion() >= WIN_VER_WIN10)
								{
									...
									bool bWinRT = false;
									for (std::vector<CString>::const_iterator it = scope.roots.begin(); it != scope.
									     roots.end(); ++it)
									{
			...
			const wchar_t* columns =
				L"System.ItemUrl, System.ItemType, Path, System.ItemPathDisplay, System.ItemNameDisplay";
			const wchar_t* order = L"System.Search.Rank DESC, System.DateModified DESC, System.ItemNameDisplay ASC";
			const wchar_t* orderComm =
				L"System.Contact.FileAsName ASC, System.Message.DateReceived DESC, System.Search.Rank DESC";
...

			{
				...
				pQueryHelper->put_QueryWhereRestrictions(
					L"AND NOT System.Shell.SFGAOFlagsStrings = SOME ARRAY['superhidden'] AND System.Shell.OmitFromView!='true'");
			...
			for (auto it = scopeList.begin(); it != scopeList.end(); ++it)
			{
				if (it->roots.empty())
					continue;
				wchar_t query[8192];
				int len = Sprintf(query,_countof(query), L"SELECT TOP %d %s FROM SystemIndex WHERE REUSEWHERE(%u)",
				                  MAX_SEARCH_RESULTS, columns, whereid);
				if (it->roots.size() == 1 && it->roots[0] == L"FILE:")
				{
					if (it->categoryHash == CATEGORY_METROSETTING)
					{
						len += Strcpy(query + len,_countof(query) - len,
						              L" AND System.Search.Store='FILE' AND System.FileName NOT LIKE 'Classic_%' AND System.ItemType='.settingcontent-ms'");
						wchar_t userPath[_MAX_PATH] =
							L"%LOCALAPPDATA%\\Packages\\windows.immersivecontrolpanel_cw5n1h2txyewy\\LocalState\\Indexed\\Settings";
					...
					{
						len += Strcpy(query + len,_countof(query) - len,
						              L" AND System.Search.Store='FILE' AND System.ItemType!='.settingcontent-ms'");
						for (auto it2 = scopeList.begin(); it2 != it; ++it2)
						{
							if (it2->categoryHash == CATEGORY_METROSETTING)
								continue;
							for (std::vector<CString>::iterator it3 = it2->roots.begin(); it3 != it2->roots.end(); ++
							     it3)
							{
						...
						if (searchRequest.bSearchPrograms)
						{
							// remove start menu/programs
							{
								...
								if (SUCCEEDED(ShGetKnownFolderPath(FOLDERID_StartMenu,&pPath)))
									len += Sprintf(query + len,_countof(query) - len, L" AND NOT SCOPE='%s'",
									               (const wchar_t*)pPath);
							}
							{
								CComString pPath;
				...
				else
				{
					...
					for (std::vector<CString>::iterator it2 = it->roots.begin(); it2 != it->roots.end(); ++it2)
					{
						...
						{
							if (bShallow)
								len += Sprintf(query + len,_countof(query) - len,
								               bFirst ? L"DIRECTORY='%s'" : L" OR DIRECTORY='%s'", scope);
							else
								len += Sprintf(query + len,_countof(query) - len,
								               bFirst ? L"SCOPE='%s'" : L" OR SCOPE='%s'", scope);
							bFirst = false;
						}
					}
					len += Strcpy(query + len,_countof(query) - len, L")");
				}
				len += Sprintf(query + len,_countof(query) - len, L" ORDER BY %s",
				               it->bCommunications ? orderComm : order);
				...
				if (it->resultCount > 0)
				{
					...
					{
						LOG_MENU(LOG_SEARCH_SQL, L"Result: %s, %s, %s, %s, %s", command.itemUrl, command.itemType,
						         command.parsingPath, command.displayPath, command.displayName);
...
void CSearchManager::GetSearchResults(SearchResults& results)
{
	...
	{
		{
			...
			for (std::vector<SearchItem>::const_iterator it = programs.begin(); it != programs.end(); ++it)
			{
				if (it->category == CATEGORY_PROGRAM && it->MatchText(m_SearchText, bSearchSubWord))
				{
					...
					for (std::vector<const SearchItem*>::const_iterator it2 = foundItems.begin(); it2 != foundItems.
					     end(); ++it2)
					{
...

		{
			...
			{
				int match = (it->category == CATEGORY_SETTING || it->category == CATEGORY_METROSETTING)
					            ? it->MatchText(m_SearchText, bSearchSubWord)
					            : 0;
	...
	else
	{
		...
		if (!filter.IsEmpty())
			filter += '*';
		for (std::vector<SearchItem>::const_iterator it = m_AutoCompleteItems.begin(); it != m_AutoCompleteItems.end();
		     ++it)
		{
	...
	results.bResults = (!results.programs.empty() || !results.settings.empty() || !results.metrosettings.empty() || !
		results.indexed.empty() || !results.autocomplete.empty());
...
bool CSearchManager::SearchItem::MatchTextInt(const wchar_t* search, const CString& text, bool bSearchSubWord)
{
	...
	if (bSearchSubWord)
	{
		...
		for (const wchar_t* pSearch = search; *pSearch;)
		{
			...
			if (FindNLSStringEx(
				LOCALE_NAME_USER_DEFAULT,FIND_FROMSTART | LINGUISTIC_IGNORECASE | LINGUISTIC_IGNOREDIACRITIC, text, -1,
				token, -1,NULL,NULL,NULL, 0) < 0)
				return false;
		}
	}
	else
		...
		for (const wchar_t* pSearch = search; *pSearch;)
		{
			...
			for (const wchar_t* pName = text; *pName;)
			{
				while (*pName && wcschr(L" \t.,$&[]{}();|", *pName))
					pName++;
				if (FindNLSStringEx(
					LOCALE_NAME_USER_DEFAULT,FIND_STARTSWITH | LINGUISTIC_IGNORECASE | LINGUISTIC_IGNOREDIACRITIC,
					pName, -1, token, len,NULL,NULL,NULL, 0) >= 0)
				{
					bFound = true;
					break;
				}
...
void CSearchManager::LaunchExternalSearch(PIDLIST_ABSOLUTE root, unsigned int categoryHash, const CString& searchText)
{
	...
	if (!root)
	{
		if (categoryHash == CATEGORY_FILE)
		{
			...
			if (FAILED(PSGetPropertyKeyFromName(L"System.StartMenu.Group",&keyGroup)))
				return;
			if (FAILED(
				pConditionFactory->CreateStringLeaf(keyGroup,COP_EQUAL,L"files",NULL,CONDITION_CREATION_DEFAULT,
					IID_PPV_ARGS(&pCondition))))
				return;
		...
		else if ((categoryHash & CATEGORY_MASK) == CATEGORY_FILE)
		{
			// all files
			if (FAILED(
				pConditionFactory->CreateStringLeaf(PKEY_Search_Store,COP_EQUAL,L"file",NULL,CONDITION_CREATION_DEFAULT,
					IID_PPV_ARGS(&pCondition))))
				return;
			conditions.push_back(pCondition);
		}
	...
	while (*str)
	{
		...
		if (SUCCEEDED(
			pConditionFactory->CreateStringLeaf(keyString,oper,token,NULL,CONDITION_CREATION_DEFAULT,IID_PPV_ARGS(&
				pCondition))))
		{
			conditions.push_back(pCondition);
			pCondition = NULL;
	...
	else
	{
		...
		if (SUCCEEDED(
			pConditionFactory->CreateCompoundFromArray(CT_AND_CONDITION,&conditions[0].p,(ULONG)conditions.size(),
				CONDITION_CREATION_DEFAULT,IID_PPV_ARGS(&pConditionAnd))))
...
void CSearchManager::LaunchInternetSearch(const CString& searchText)
{
	...
	if (IsEdgeDefaultBrowser())
	{
		...
		ShellExecute(NULL,NULL, L"shell:appsfolder\\Microsoft.MicrosoftEdge_8wekyb3d8bbwe!MicrosoftEdge", search,NULL,
		             SW_SHOWNORMAL);
	}
	else
	{
