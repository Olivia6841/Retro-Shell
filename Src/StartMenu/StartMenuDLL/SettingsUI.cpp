...
static LRESULT CALLBACK SubclassEditProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
                                         DWORD_PTR dwRefData)
{
	...
	if (uMsg == WM_KEYDOWN)
	{
		if (wParam == VK_UP || wParam == VK_DOWN || wParam == VK_PRIOR || wParam == VK_NEXT || (wParam == VK_SPACE &&
...
static LRESULT CALLBACK SubclassInfoTipProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
                                            DWORD_PTR dwRefData)
{
	if (uMsg == TTM_UPDATE)
	{
...
static LRESULT CALLBACK SubclassTreeProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
                                         DWORD_PTR dwRefData)
{
	if (uMsg == WM_COMMAND)
		return SendMessage(GetParent(hWnd), uMsg, wParam, lParam);
...
LRESULT CSkinSettingsDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	m_Tree.SetWindowLong(GWL_STYLE, m_Tree.GetWindowLong(GWL_STYLE) | TVS_SHOWSELALWAYS);
	// should be in the dialog template, but I don't want to have to edit all language DLLs
	if (IsAppThemed())
	{
	...
	*PathFindFileName(find) = 0;
	...
	m_EditBox = CreateWindow(L"EDIT", NULL, WS_CHILD|WS_BORDER|ES_AUTOHSCROLL, 0, 0, 0, 0, m_Tree, (HMENU)IDC_EDITBOX,
	                         hInstance, NULL);
	m_EditBox.SetFont(font);
	SetWindowSubclass(m_EditBox, SubclassEditProc, 'CLSH', 0);
	m_BrowseButton = CreateWindow(L"BUTTON", L"...", WS_CHILD|BS_PUSHBUTTON, 0, 0, 0, 0, m_Tree, (HMENU)IDC_BROWSEBTN,
	                              hInstance, NULL);
	...
	else
	{
		...
		if (!m_CurrentSkin.Variations.empty())
		{
			...
			TVINSERTSTRUCT insert = {
				NULL,TVI_LAST,
				{
					TVIF_STATE | TVIF_TEXT | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM,NULL,TVIS_EXPANDED,
					TVIS_EXPANDED | TVIS_OVERLAYMASK, (LPWSTR)(LPCWSTR)vars, 0, SETTING_STATE_SETTING,
					SETTING_STATE_SETTING
				}
			};
			...
			for (int i = 0; i < n; i++)
			{
				...
				if (_wcsicmp(var.label, variaton) == 0 || _wcsicmp(var.labelEn, variaton) == 0)
					m_VariationIndex = i;
				TVINSERTSTRUCT insert2 = {
					hVars,TVI_LAST, {TVIF_TEXT | TVIF_PARAM,NULL, 0, 0, (LPWSTR)(LPCWSTR)var.label}
				};
		...
		for (int i = 0; i < n; i++)
		{
			...
			TVINSERTSTRUCT insert = {
				NULL,TVI_LAST,
				{
					TVIF_TEXT | TVIF_PARAM | TVIF_STATE | TVIF_IMAGE | TVIF_SELECTEDIMAGE,NULL,TVIS_EXPANDED,
					TVIS_OVERLAYMASK | TVIS_EXPANDED, (LPWSTR)(LPCWSTR)(LPWSTR)(LPCWSTR)option.label
				}
			};
	...
	while (hItem)
	{
		...
		else
		{
			...
			if (option.bValue && option.type > SKIN_OPTION_BOOL)
				Sprintf(text,_countof(text), L"%s: %s", option.label,
				        (option.type == SKIN_OPTION_COLOR) ? BgrToRgb(option.sValue) : option.sValue);
			else
...
LRESULT CSkinSettingsDlg::OnCustomDraw(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
	...
	{
		if (IS_INTRESOURCE(pDraw->nmcd.lItemlParam) && m_CurrentSkin.Options[pDraw->nmcd.lItemlParam].bValue &&
			m_CurrentSkin.Options[pDraw->nmcd.lItemlParam].type == SKIN_OPTION_COLOR)
		{
			RECT rc;
	...
	if (IS_INTRESOURCE(item.lParam))
	{
		...
		if (option.groupId >= 0)
		{
			...
			if (item.iImage & SETTING_STATE_CHECKED)
				return;
			for (std::vector<MenuSkin::Option>::const_iterator it = m_CurrentSkin.Options.begin(); it != m_CurrentSkin.
			     Options.end(); ++it)
			{
	...
	if (TreeView_HitTest(m_Tree, &test))
	{
		...
		if (test.flags & TVHT_ONITEMRIGHT)
		{
			...
			if (item.lParam != m_EditItemIndex && IS_INTRESOURCE(item.lParam))
			{
				...
				{
					RECT rc;
					if (TreeView_GetItemRect(m_Tree, item.hItem, &rc, TRUE) && test.pt.x >= rc.right && test.pt.x < rc.
...
LRESULT CSkinSettingsDlg::OnBrowse(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	if (m_EditMode == SKIN_OPTION_IMAGE)
	{
		...
		if (_wcsicmp(PathFindExtension(str), L".bmp") == 0 || _wcsicmp(PathFindExtension(str), L".png") == 0 ||
			_wcsicmp(PathFindExtension(str), L".jpg") == 0)
		{
...
static HRESULT CALLBACK TaskDialogCallbackProc(HWND hwnd, UINT uNotification, WPARAM wParam, LPARAM lParam,
                                               LONG_PTR dwRefData)
{
...
LRESULT CSkinSettingsDlg::OnAbout(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	TASKDIALOGCONFIG task = {
		sizeof(task), m_hWnd,NULL, TDF_ENABLE_HYPERLINKS | TDF_ALLOW_DIALOG_CANCELLATION | TDF_USE_HICON_MAIN,
		TDCBF_OK_BUTTON
	};
...
static const CStdCommand g_StdCommands[] = {
	...
	{
		L"favorites",IDS_SHOW_FAVORITES,IDS_FAVORITES_TIP, L"FavoritesItem", L"$Menu.Favorites", L"",
		L"imageres.dll,1024", &FOLDERID_Favorites
	},
	{
		L"recent_documents",IDS_SHOW_DOCUMENTS,IDS_DOCUMENTS_TIP, L"RecentDocumentsItem", L"$Menu.Documents", L"",
		L"shell32.dll,327", &FOLDERID_Recent, StdMenuItem::MENU_ITEMS_FIRST
	},
	...
	{
		L"search_box",IDS_SEARCH_BOX_ITEM,IDS_SEARCH_BOX_TIP, L"SearchBoxItem", L"$Menu.SearchBox",NULL, L"none",NULL,
		StdMenuItem::MENU_TRACK | StdMenuItem::MENU_OPENUP
	},
	...
	{
		L"logoff",IDS_SHUTDOWN_LOGOFF,IDS_LOGOFF_TIP, L"LogOffItem", L"$Menu.Logoff", L"$Menu.LogOffTip",
		L"shell32.dll,325",NULL, StdMenuItem::MENU_STYLE_CLASSIC1
	},
	{
		L"logoff",IDS_SHUTDOWN_LOGOFF,IDS_LOGOFF_TIP, L"LogOffItem", L"$Menu.Logoff", L"$Menu.LogOffTip", L"none",NULL,
		StdMenuItem::MENU_STYLE_CLASSIC2
	},
	{
		L"undock",IDS_UNDOCK_ITEM,IDS_UNDOCK_TIP, L"UndockItem", L"$Menu.Undock", L"$Menu.UndockTip",
		L"shell32.dll,331",NULL, StdMenuItem::MENU_STYLE_CLASSIC1
	},
	{
		L"undock",IDS_UNDOCK_ITEM,IDS_UNDOCK_TIP, L"UndockItem", L"$Menu.Undock", L"$Menu.UndockTip", L"none",NULL,
		StdMenuItem::MENU_STYLE_CLASSIC2
	},
	{
		L"disconnect",IDS_DISCONNECT_ITEM,IDS_DISCONNECT_TIP, L"DisconnectItem", L"$Menu.Disconnect",
		L"$Menu.DisconnectTip", L"shell32.dll,329",NULL, StdMenuItem::MENU_STYLE_CLASSIC1
	},
	{
		L"disconnect",IDS_DISCONNECT_ITEM,IDS_DISCONNECT_TIP, L"DisconnectItem", L"$Menu.Disconnect",
		L"$Menu.DisconnectTip", L"none",NULL, StdMenuItem::MENU_STYLE_CLASSIC2
	},
	{
		L"shutdown_box",IDS_SHUTDOWN_SHUTDOWN_BOX,IDS_SHUTDOWNBOX_TIP, L"ShutdownBoxItem", L"$Menu.ShutdownBox",NULL,
		L"shell32.dll,329",NULL, StdMenuItem::MENU_SPLIT_BUTTON
	},
	{
		L"user_files",IDS_SHOW_USERFILES,IDS_USERFILES_TIP, L"UserFilesItem",NULL, L"$Menu.UserFilesTip", L"",
		&FOLDERID_UsersFiles
	},
	{
		L"user_documents",IDS_SHOW_USERDOCS,IDS_USERDOCS_TIP, L"UserDocumentsItem",NULL, L"$Menu.UserDocumentsTip", L"",
		&FOLDERID_Documents
	},
	{
		L"user_music", IDS_SHOW_USERMUSIC, IDS_USERMUSIC_TIP, L"UserMusicItem",NULL, L"$Menu.UserMusicTip", L"",
		&FOLDERID_Music
	},
	{
		L"user_pictures",IDS_SHOW_USERPICS,IDS_USERPICS_TIP, L"UserPicturesItem",NULL, L"$Menu.UserPicturesTip", L"",
		&FOLDERID_Pictures
	},
	{
		L"control_panel",IDS_SHOW_CP,IDS_CONTROLPANEL_TIP, L"ControlPanelItem", L"$Menu.ControlPanel",
		L"$Menu.ControlPanelTip", L"shell32.dll,137", &FOLDERID_ControlPanelFolder, StdMenuItem::MENU_TRACK
	},
	{
		L"pc_settings",IDS_PCSETTINGS,IDS_PCSETTINGS_TIP, L"PCSettingsItem", L"$Menu.PCSettings", L"",
		L"%windir%\\ImmersiveControlPanel\\SystemSettings.exe,10",NULL, StdMenuItem::MENU_TRACK
	},
	{
		L"windows_security",IDS_SECURITY_ITEM,IDS_SECURITY_TIP, L"SecurityItem", L"$Menu.Security",
		L"$Menu.SecurityTip", L"shell32.dll,48"
	},
	{
		L"network_connections",IDS_SHOW_NETWORK,IDS_NETWORK_TIP, L"NetworkItem", L"$Menu.Network", L"$Menu.NetworkTip",
		L"shell32.dll,257", &FOLDERID_ConnectionsFolder
	},
	{
		L"printers",IDS_SHOW_PRINTERS,IDS_PRINTERS_TIP, L"PrintersItem", L"$Menu.Printers", L"$Menu.PrintersTip",
		L"shell32.dll,138", &FOLDERID_PrintersFolder
	},
	{
		L"taskbar_settings",IDS_TASKBAR_ITEM,IDS_TASKBAR_TIP, L"TaskbarSettingsItem", L"$Menu.Taskbar",
		L"$Menu.TaskbarTip", L"shell32.dll,40"
	},
	{
		L"programs_features",IDS_FEATURES_ITEM,IDS_FEATURES_TIP, L"ProgramsFeaturesItem", L"$Menu.Features",
		L"$Menu.FeaturesTip", L"shell32.dll,271"
	},
	{
		L"menu_settings",IDS_MENU_SETTINGS_ITEM,IDS_MENU_TIP, L"MenuSettingsItem", L"$Menu.ClassicSettings",
		L"$Menu.SettingsTip", L",1"
	},
	{
		L"search_files",IDS_SEARCH_FILES_ITEM,IDS_SEARCHFI_TIP, L"SearchFilesItem", L"$Menu.SearchFiles",
		L"$Menu.SearchFilesTip", L"shell32.dll,134"
	},
	{
		L"search_printer",IDS_SEARCH_PRINTER_ITEM,IDS_SEARCHPR_TIP, L"SearchPrinterItem", L"$Menu.SearchPrinter",
		L"$Menu.SearchPrintersTip", L"shell32.dll,1006"
	},
	{
		L"search_computers",IDS_SEARCH_COMP_ITEM,IDS_SEARCHCO_TIP, L"SearchComputersItem", L"$Menu.SearchComputers",
		L"$Menu.SearchComputersTip", L"shell32.dll,135"
	},
	{
		L"search_people",IDS_SEARCH_PEOPLE_ITEM,IDS_SEARCHPE_TIP, L"SearchPeopleItem", L"$Menu.SearchPeople",NULL,
		L"shell32.dll,269"
	},
	{L"sleep",IDS_SHUTDOWN_SLEEP,IDS_SLEEP_TIP, L"SleepItem", L"$Menu.Sleep", L"$Menu.SleepTip", L"none"},
	{
		L"hibernate",IDS_SHUTDOWN_HIBERNATE,IDS_HIBERNATE_TIP, L"HibernateItem", L"$Menu.Hibernate",
		L"$Menu.HibernateTip", L"none"
	},
	{L"restart",IDS_SHUTDOWN_RESTART,IDS_RESTART_TIP, L"RestartItem", L"$Menu.Restart", L"$Menu.RestartTip", L"none"},
	{
		L"restart_noupdate",IDS_SHUTDOWN_RESTART_NU,IDS_RESTART_NU_TIP, L"RestartItem", L"$Menu.Restart",
		L"$Menu.RestartTip", L"none"
	},
	{
		L"shutdown",IDS_SHUTDOWN_SHUTDOWN,IDS_SHUTDOWN_TIP, L"ShutdownItem", L"$Menu.Shutdown", L"$Menu.ShutdownTip",
		L"none"
	},
	{
		L"shutdown_noupdate",IDS_SHUTDOWN_SHUTDOWN_NU,IDS_SHUTDOWN_NU_TIP, L"ShutdownItem", L"$Menu.Shutdown",
		L"$Menu.ShutdownTip", L"none"
	},
	{
		L"switch_user",IDS_SHUTDOWN_SWITCH,IDS_SWITCH_TIP, L"SwitchUserItem", L"$Menu.SwitchUser",
		L"$Menu.SwitchUserTip", L"none"
	},
...
{
public:
	CEditMenuDlg(CTreeItem* pItem, std::vector<HMODULE>& modules, int style, int mask): CEditCustomItemDlg(
		pItem, modules)
	{
		m_Style = style;
		m_StyleMask = mask;
...
LRESULT CEditMenuDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	CheckDlgButton(IDC_CHECKSORTZAREC, (m_pItem->settings & StdMenuItem::MENU_SORTZA_REC)
		                                   ? BST_CHECKED
		                                   : BST_UNCHECKED);
	...
	CheckDlgButton(IDC_CHECKOPENUPREC, (m_pItem->settings & StdMenuItem::MENU_OPENUP_REC)
		                                   ? BST_CHECKED
		                                   : BST_UNCHECKED);
	...
	CheckDlgButton(IDC_CHECKITEMSFIRST, (m_pItem->settings & StdMenuItem::MENU_ITEMS_FIRST)
		                                    ? BST_CHECKED
		                                    : BST_UNCHECKED);
	...
	if (m_pItem->pStdCommand && (wcscmp(m_pItem->pStdCommand->name, L"programs") == 0 || wcscmp(
		m_pItem->pStdCommand->name, L"apps") == 0))
	{
	...
	{
		CheckDlgButton(IDC_CHECKMULTICOLUMN, (m_pItem->settings & StdMenuItem::MENU_MULTICOLUMN)
			                                     ? BST_CHECKED
			                                     : BST_UNCHECKED);
	...
	CWindow tooltip = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_TRANSPARENT,TOOLTIPS_CLASS,NULL,
	                                 WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, 0, 0, 0, 0, m_hWnd,NULL, g_Instance,NULL);
...
LRESULT CEditMenuDlg::OnOK(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	m_pItem->settings = 0;
	bool bForceMultiColumn = (m_pItem->pStdCommand && (wcscmp(m_pItem->pStdCommand->name, L"programs") == 0 || wcscmp(
		m_pItem->pStdCommand->name, L"apps") == 0));
...
protected:
	...
	{
		UpdateWarnings(m_Style == StdMenuItem::MENU_STYLE_CLASSIC1 ? MENU_CLASSIC1 : MENU_CLASSIC2);
	}
};
...
private:
	...
	private:
		...
		static LRESULT CALLBACK SubclassGridCombo(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
...
{
public:
	CEditMenuDlg7(CCustomMenuDlg7* pOwner, CCustomMenuDlg7::MenuItem* pItem)
	{
		m_pOwner = pOwner;
		m_pItem = pItem;
	}
	...
	enum
	{
		SETTINGS_MASK = StdMenuItem::MENU_NOEXPAND | StdMenuItem::MENU_SINGLE_EXPAND | StdMenuItem::MENU_ITEM_DISABLED
	};
...
LRESULT CEditMenuDlg7::OnReset(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	if (m_pItem->stdItemIndex < g_StdCommands7Count - 1)
	{
		...
		CheckDlgButton(IDC_CHECKSORTZAREC, (m_pItem->settings & StdMenuItem::MENU_SORTZA_REC)
			                                   ...
			                                   : BST_UNCHECKED);
		CheckDlgButton(
			IDC_CHECKSORTONCE, (m_pItem->settings & StdMenuItem::MENU_SORTONCE) ? BST_CHECKED : BST_UNCHECKED);
		CheckDlgButton(IDC_CHECKOPENUP, (m_pItem->settings & StdMenuItem::MENU_OPENUP) ? BST_CHECKED : BST_UNCHECKED);
		CheckDlgButton(IDC_CHECKOPENUPREC, (m_pItem->settings & StdMenuItem::MENU_OPENUP_REC)
			                                   ? BST_CHECKED
			                                   : BST_UNCHECKED);
		CheckDlgButton(IDC_CHECKNOEXT, (m_pItem->settings & StdMenuItem::MENU_NOEXTENSIONS)
			                               ? BST_CHECKED
			                               : BST_UNCHECKED);
		CheckDlgButton(IDC_CHECKTRACK, (m_pItem->settings & StdMenuItem::MENU_TRACK) ? BST_CHECKED : BST_UNCHECKED);
		CheckDlgButton(IDC_CHECKMULTICOLUMN, (m_pItem->settings & StdMenuItem::MENU_MULTICOLUMN)
	...
	else
	{
		...
		else if (!item.link.IsEmpty())
		{
			...
			if (pidl && GetWinVersion() >= WIN_VER_WIN8 && _wcsicmp(PathFindExtension(buf), L".lnk") == 0)
			{
				...
				{
					const CItemManager::ItemInfo* pItemInfo = g_ItemManager.GetItemInfo(
						pItem, pidl, CItemManager::INFO_LINK | CItemManager::INFO_METRO);
					if (pItemInfo)
					{
						bool bMetroLink;
						{
						...
						if (bMetroLink)
						{
							g_ItemManager.UpdateItemInfo(
								pItemInfo,
								(bSmall ? CItemManager::INFO_SMALL_ICON : CItemManager::INFO_LARGE_ICON) |
								CItemManager::INFO_REFRESH_NOW);
							const CItemManager::IconInfo* pIconInfo = bSmall
								                                          ? pItemInfo->smallIcon
								                                          : pItemInfo->largeIcon;
							if (pIconInfo && pIconInfo->bitmap)
							{
								...
								if (bmpInfo.bmWidth != iconSize || bmpInfo.bmHeight != iconSize)
								{
									...
									StretchBlt(hDst, 0, 0, bi.bmiHeader.biWidth, bi.bmiHeader.biHeight, hSrc, 0, 0,
			...
			if (networkType != NETWORK_NONE && networkType != NETWORK_FILE)
				return ::LoadIcon(GetSystemMetrics(bSmall ? SM_CXSMICON : SM_CXICON),
				                  GetDefaultNetworkIcon(networkType), m_Modules);
		}
		if (pidl)
...
void CCustomMenuDlg7::CItemList::UpdateItem(int index)
{
	...
	else if ((menuItem.settings & StdMenuItem::MENU_NOEXPAND) && !(g_StdCommands7[menuItem.stdItemIndex].flags &
		CStdCommand7::ITEM_FOLDER))
		str = LoadStringEx(IDS_ITEM_SHOW);
	else if ((menuItem.settings & StdMenuItem::MENU_SINGLE_EXPAND) && !(g_StdCommands7[menuItem.stdItemIndex].flags &
		CStdCommand7::ITEM_NODRIVES))
...
void CCustomMenuDlg7::CItemList::SubclassWindow(HWND hWnd, CCustomMenuDlg7* pOwner)
{
	...
	SHFILEINFO info;
	if (SHGetFileInfo(L"file",FILE_ATTRIBUTE_NORMAL, &info, sizeof(info),
	...
	if (pDraw->nmcd.dwDrawStage == CDDS_ITEMPOSTPAINT)
	{
		...
		if (icon >= 0)
		{
			...
			if (!m_bThemed && (ListView_GetItemState(m_hWnd, pDraw->nmcd.dwItemSpec, LVIS_SELECTED) & LVIS_SELECTED))
				...
				ImageList_DrawEx(m_ImageList, icon, pDraw->nmcd.hdc, x, y, 0, 0,CLR_NONE, GetSysColor(COLOR_WINDOW),
				                 ILD_SELECTED);
			else
...
LRESULT CCustomMenuDlg7::CItemList::OnSelEndOk(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	if (m_Column == 2)
	{
		...
		else if (sel == 1 && !(g_StdCommands7[menuItem.stdItemIndex].flags & (CStdCommand7::ITEM_SINGLE |
			CStdCommand7::ITEM_FOLDER)))
...
LRESULT CALLBACK CCustomMenuDlg7::CItemList::SubclassGridCombo(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
                                                               UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	if (uMsg == WM_KEYDOWN)
	{
...
void CCustomMenuDlg7::CItemList::CreateCombo(int line, int column)
{
	...
	else
	{
		...
		{
			str = LoadStringEx((g_StdCommands7[menuItem.stdItemIndex].flags & CStdCommand7::ITEM_SINGLE)
				                   ? IDS_ITEM_SHOW2
				                   : IDS_ITEM_MENU);
			m_Combo.SendMessage(CB_ADDSTRING, 0, (LPARAM)(const wchar_t*)str);
		}
		...
		else if ((g_StdCommands7[menuItem.stdItemIndex].flags & (CStdCommand7::ITEM_SINGLE | CStdCommand7::ITEM_FOLDER))
			|| (menuItem.settings & StdMenuItem::MENU_NOEXPAND))
			m_Combo.SendMessage(CB_SETCURSEL, 1);
		else if (!(g_StdCommands7[menuItem.stdItemIndex].flags & CStdCommand7::ITEM_NODRIVES) && (menuItem.settings &
		...
		for (int i = 1;; i++)
		{
			...
			if (str)
			{
				while (*str)
				{
					...
					else if (_wcsicmp(token, L"SINGLE_EXPAND") == 0)
						menuItem.settings |=
							StdMenuItem::MENU_SINGLE_EXPAND;
...
LRESULT CCustomMenuDlg7::OnContextMenu(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (res == CMD_RESETALL)
	{
		if (::MessageBox(m_hWnd, LoadStringEx(IDS_RESET_MENU_WARN), LoadStringEx(IDS_RESET_MENU),
...
void CCustomMenuDlg7::SerializeData(void)
{
	...
	for (int i = 0; i < (int)m_Items.size(); i++)
	{
		...
		if (menuItem.settings)
		{
			...
			else if (menuItem.settings & StdMenuItem::MENU_SINGLE_EXPAND)
				AppendString(stringBuilder, L"SINGLE_EXPAND|");
...
LRESULT CMenuStyleDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
		HBITMAP bmp = LoadImageResource(
			g_Instance,MAKEINTRESOURCE(bLargeBitmaps?IDB_STYLE_CLASSIC1150:IDB_STYLE_CLASSIC1), true, true);
		m_ImageClassic1.SendMessage(STM_SETIMAGE,IMAGE_BITMAP, (LPARAM)bmp);
		bmp = LoadImageResource(g_Instance,MAKEINTRESOURCE(bLargeBitmaps?IDB_STYLE_CLASSIC2150:IDB_STYLE_CLASSIC2),
		                        true, true);
	...
	m_Tooltip = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_TRANSPARENT | (bRTL ? WS_EX_LAYOUTRTL : 0),
	                           TOOLTIPS_CLASS,NULL,WS_POPUP | TTS_NOPREFIX, 0, 0, 0, 0, m_hWnd,NULL, g_Instance,NULL);
	...
	m_IconPath = "?";
	SetDlgItemText(
		IDC_CHECKENABLED, LoadStringEx(GetWinVersion() == WIN_VER_WIN8 ? IDS_ENABLE_BUTTON : IDS_ENABLE_BUTTON2));
...
{
	if ((HWND)wParam == m_ImageClassic1.m_hWnd || (HWND)wParam == m_ImageClassic2.m_hWnd || (HWND)wParam == m_ImageWin7.
		m_hWnd || (HWND)wParam == m_ButtonAero.m_hWnd || (HWND)wParam == m_ButtonClassic.m_hWnd || (HWND)wParam ==
		m_ButtonCustom.m_hWnd)
	{
...
void CMenuStyleDlg::UpdateDefaults(void)
{
	...
	UpdateSetting(L"PinnedPrograms",
	              CComVariant(menuStyle != MENU_WIN7 ? PINNED_PROGRAMS_FAST : PINNED_PROGRAMS_PINNED), false);
}
...
CSetting g_Settings[] = {
	{
		L"MenuStyleGroup", CSetting::TYPE_GROUP,IDS_STYLE_SETTINGS, 0, 0, CSetting::FLAG_BASIC,NULL,NULL,
		&g_StyleSettingsPanel
	},
	...
	{
		L"MouseClickCommand", CSetting::TYPE_STRING,IDS_OPEN_CMD_TEXT,IDS_OPEN_CMD_TEXT_TIP, "%userprofile%", 0,
		L"MouseClick=3", L"Command"
	},
	...
	{
		L"ShiftClickCommand", CSetting::TYPE_STRING,IDS_OPEN_CMD_TEXT,IDS_OPEN_CMD_TEXT_TIP, "%systemdrive%", 0,
		L"ShiftClick=3", L"Command"
	},
	...
	{
		L"WinKeyCommand", CSetting::TYPE_STRING,IDS_OPEN_CMD_TEXT,IDS_OPEN_CMD_TEXT_TIP, "cmd", 0, L"WinKey=3",
		L"Command"
	},
	...
	/*		{L"Desktop",CSetting::TYPE_RADIO,IDS_OPEN_DESKTOP,IDS_OPEN_DESKTOP_TIP,0,CSetting::FLAG_HIDDEN},
			{L"Cortana",CSetting::TYPE_RADIO,IDS_OPEN_CORTANA,IDS_OPEN_CORTANA_TIP},*/
	{
		L"ShiftWinCommand", CSetting::TYPE_STRING,IDS_OPEN_CMD_TEXT,IDS_OPEN_CMD_TEXT_TIP, "powershell", 0,
		L"ShiftWin=3", L"Command"
	},
	...
	/*		{L"Both",CSetting::TYPE_RADIO,IDS_OPEN_BOTH,IDS_OPEN_BOTH_TIP,0,CSetting::FLAG_HIDDEN},
			...
			{L"Cortana",CSetting::TYPE_RADIO,IDS_OPEN_CORTANA,IDS_OPEN_CORTANA_TIP},*/
	{
		L"MiddleClickCommand", CSetting::TYPE_STRING,IDS_OPEN_CMD_TEXT,IDS_OPEN_CMD_TEXT_TIP, "taskmgr", 0,
		L"MiddleClick=3", L"Command"
	},
	...
	{
		L"Computer", CSetting::TYPE_INT,IDS_SHOW_COMPUTER,IDS_SHOW_COMPUTER_TIP, 1,
		CSetting::FLAG_BASIC | CSetting::FLAG_MENU_CLASSIC_BOTH
	},
	...
	{
		L"Favorites", CSetting::TYPE_INT,IDS_SHOW_FAVORITES,IDS_SHOW_FAVORITES_TIP, 0,
		CSetting::FLAG_BASIC | CSetting::FLAG_MENU_CLASSIC_BOTH
	},
	{L"Hide", CSetting::TYPE_RADIO,IDS_ITEM_HIDE,IDS_ITEM_HIDE_TIP},
	{L"Show", CSetting::TYPE_RADIO,IDS_ITEM_SHOW,IDS_ITEM_SHOW_TIP},
	{L"Menu", CSetting::TYPE_RADIO,IDS_ITEM_MENU,IDS_ITEM_MENU_TIP},
	{
		L"Documents", CSetting::TYPE_INT,IDS_SHOW_DOCUMENTS,IDS_SHOW_DOCUMENTS_TIP, 2,
		CSetting::FLAG_BASIC | CSetting::FLAG_MENU_CLASSIC_BOTH
	},
	{L"Hide", CSetting::TYPE_RADIO,IDS_ITEM_HIDE,IDS_ITEM_HIDE_TIP},
	{L"Show", CSetting::TYPE_RADIO,IDS_ITEM_SHOW,IDS_ITEM_SHOW_TIP},
	{L"Menu", CSetting::TYPE_RADIO,IDS_ITEM_MENU,IDS_ITEM_MENU_TIP},
	{
		L"MaxRecentDocuments", CSetting::TYPE_INT,IDS_MAX_DOCS,IDS_MAX_DOCS_TIP, 15, CSetting::FLAG_MENU_CLASSIC_BOTH,
		L"Documents=2", L"Documents"
	},
	{
		L"SortRecentDocuments", CSetting::TYPE_INT,IDS_SORT_DOCS,IDS_SORT_DOCS_TIP, 0, CSetting::FLAG_MENU_CLASSIC_BOTH,
		L"Documents=2", L"Documents"
	},
	...
	{
		L"ControlPanel", CSetting::TYPE_INT,IDS_SHOW_CP,IDS_SHOW_CP_TIP, 2,
		CSetting::FLAG_BASIC | CSetting::FLAG_MENU_CLASSIC_BOTH
	},
	{L"Hide", CSetting::TYPE_RADIO,IDS_ITEM_HIDE,IDS_ITEM_HIDE_TIP},
	{L"Show", CSetting::TYPE_RADIO,IDS_ITEM_SHOW,IDS_ITEM_SHOW_TIP},
	{L"Menu", CSetting::TYPE_RADIO,IDS_ITEM_MENU,IDS_ITEM_MENU_TIP},
	{
		L"ControlPanelCategories", CSetting::TYPE_BOOL,IDS_CP_CATEGORIES,IDS_CP_CATEGORIES_TIP, 0,
		CSetting::FLAG_MENU_CLASSIC_BOTH, L"#ControlPanel", L"ControlPanel"
	},
	...
	{L"Menu", CSetting::TYPE_RADIO,IDS_ITEM_MENU,IDS_ITEM_MENU_TIP},
	{
		L"LogOff", CSetting::TYPE_BOOL,IDS_SHOW_LOGOFF,IDS_SHOW_LOGOFF_TIP, 1,
		CSetting::FLAG_MENU_CLASSIC_BOTH | CSetting::FLAG_BASIC
	},
	{
		L"ConfirmLogOff", CSetting::TYPE_BOOL,IDS_CONFIRM_LOGOFF,IDS_CONFIRM_LOGOFF_TIP, 0,
		CSetting::FLAG_MENU_CLASSIC_BOTH, L"#LogOff", L"LogOff"
	},
	...
	{
		L"ShowAppsMenu", CSetting::TYPE_BOOL,IDS_SHOW_APPS_MENU,IDS_SHOW_APPS_MENU_TIP, 1,
		CSetting::FLAG_MENU_CLASSIC_BOTH
	},
	...
	{
		L"ProgramsStyle", CSetting::TYPE_INT,IDS_INLINE_PROGS,IDS_INLINE_PROGS_TIP, 1,
		CSetting::FLAG_MENU_WIN7 | CSetting::FLAG_BASIC
	},
	...
	{
		L"AllProgramsMetro", CSetting::TYPE_BOOL,IDS_SHOW_APPS,IDS_SHOW_APPS_TIP, 1, CSetting::FLAG_MENU_WIN7,NULL,
		L"Inline"
	},
	{
		L"FoldersFirst", CSetting::TYPE_BOOL,IDS_FOLDERS_FIRST,IDS_FOLDERS_FIRST_TIP, 0, CSetting::FLAG_MENU_WIN7,
		L"ProgramsStyle=1", L"Inline"
	},
	{
		L"OpenPrograms", CSetting::TYPE_BOOL,IDS_OPEN_PROGRAMS,IDS_OPEN_PROGRAMS_TIP, 0, CSetting::FLAG_MENU_WIN7,
		L"ProgramsStyle=1", L"Inline"
	},
	{
		L"ProgramsMenuDelay", CSetting::TYPE_INT,IDS_PROGRAMS_DELAY,IDS_PROGRAMS_DELAY_TIP, 200,
		CSetting::FLAG_MENU_WIN7, L"ProgramsStyle=1", L"Inline"
	}, // 2x system delay time
	...
	{
		L"PinnedItemsPath", CSetting::TYPE_DIRECTORY,IDS_PINNED_PATH,IDS_PINNED_PATH_TIP,
		L"%APPDATA%\\OpenShell\\Pinned", 0, L"PinnedPrograms=1", L"PinnedItems"
	},
	{
		L"RecentPrograms", CSetting::TYPE_INT,IDS_RECENT_PROGRAMS,IDS_RECENT_PROGRAMS_TIP, RECENT_PROGRAMS_RECENT,
		CSetting::FLAG_BASIC
	},
	...
	{
		L"RecentMetroApps", CSetting::TYPE_BOOL,IDS_RECENT_METRO,IDS_RECENT_METRO_TIP, 1, 0, L"RecentPrograms",
		L"RecentPrograms"
	},
	{
	...
	{
		L"RecentSortName", CSetting::TYPE_BOOL,IDS_RECENT_NAME,IDS_RECENT_NAME_TIP, 0, 0, L"RecentPrograms",
		L"RecentPrograms"
	},
	{
		L"RecentProgKeys", CSetting::TYPE_INT,IDS_RECENT_KEYS,IDS_RECENT_KEYS_TIP, RECENT_KEYS_DIGITS, 0,
		L"RecentPrograms", L"RecentPrograms"
	},
	...
	{
		L"ShutdownCommand", CSetting::TYPE_INT,IDS_SHUTDOWN_COMMAND,IDS_SHUTDOWN_COMMAND_TIP, 1,
		CSetting::FLAG_MENU_WIN7 | CSetting::FLAG_BASIC
	},
	...
	{
		L"ShutdownW7", CSetting::TYPE_STRING,IDS_SHUTDOWN7,IDS_SHUTDOWN7_TIP, L"", CSetting::FLAG_MENU_WIN7,
		L"ShutdownCommand", L"ShutdownCommand"
	},
	{
		L"RemoteShutdown", CSetting::TYPE_BOOL,IDS_SHOW_RSHUTDOWN,IDS_SHOW_RSHUTDOWN_TIP, 0, 0, L"ShutdownCommand",
		L"ShutdownCommand"
	},
	{
		L"HybridShutdown", CSetting::TYPE_BOOL,IDS_HYBRID_SHUTDOWN,IDS_HYBRID_SHUTDOWN_TIP, 1, 0, L"ShutdownCommand",
		L"ShutdownCommand"
	},
	{
		L"ConfirmLogOff2", CSetting::TYPE_BOOL,IDS_CONFIRM_LOGOFF,IDS_CONFIRM_LOGOFF_TIP, 0, CSetting::FLAG_MENU_WIN7,
		NULL, L"ShutdownCommand"
	},
	{
		L"SelectLastShutdown", CSetting::TYPE_BOOL,IDS_SELECT_LAST,IDS_SELECT_LAST_TIP, 0, CSetting::FLAG_MENU_WIN7,
		NULL, L"ShutdownCommand"
	},
	...
	{
		L"ControlPanelCategories2", CSetting::TYPE_BOOL,IDS_CP_CATEGORIES2,IDS_CP_CATEGORIES_TIP, 0,
		CSetting::FLAG_MENU_WIN7
	},
	...
	{
		L"SearchHintText", CSetting::TYPE_STRING,IDS_NEW_SEARCH_HINT,IDS_NEW_SEARCH_HINT_TIP, L"", 0, L"#SearchHint",
		L"SearchHint"
	},
	{L"SearchTrack", CSetting::TYPE_BOOL,IDS_SEARCH_TRACK,IDS_SEARCH_TRACK_TIP, 1, 0, L"SearchBox"},
	{
		L"SearchResults", CSetting::TYPE_INT,IDS_SEARCH_MAX2,IDS_SEARCH_MAX_TIP2, 5, CSetting::FLAG_MENU_CLASSIC_BOTH,
		L"SearchBox"
	},
	{
		L"SearchResultsMax", CSetting::TYPE_INT,IDS_SEARCH_MAX3,IDS_SEARCH_MAX_TIP3, 20,
		CSetting::FLAG_MENU_CLASSIC_BOTH, L"SearchBox"
	},
	...
	{
		L"SearchPath", CSetting::TYPE_BOOL,IDS_SEARCH_PATH,IDS_SEARCH_PATH_TIP, 1, 0, L"#SearchPrograms",
		L"SearchPrograms"
	},
	{
		L"SearchMetroApps", CSetting::TYPE_BOOL,IDS_SEARCH_METRO,IDS_SEARCH_METRO_TIP, 1, 0, L"#SearchPrograms",
		L"SearchPrograms"
	},
	{
		L"SearchMetroSettings", CSetting::TYPE_BOOL,IDS_SEARCH_METROS,IDS_SEARCH_METROS_TIP, 1, 0, L"#SearchPrograms",
		L"SearchPrograms"
	},
	{
		L"SearchKeywords", CSetting::TYPE_BOOL,IDS_SEARCH_KEYWORDS,IDS_SEARCH_KEYWORDS_TIP, 1, 0, L"#SearchPrograms",
		L"SearchPrograms"
	},
	...
	{
		L"SearchContents", CSetting::TYPE_BOOL,IDS_SEARCH_CONTENTS,IDS_SEARCH_CONTENTS_TIP, 1, 0, L"#SearchFiles",
		L"SearchFiles"
	},
	{
	...
	{L"SmallIconSize", CSetting::TYPE_INT,IDS_SMALL_SIZE_SM,IDS_SMALL_SIZE_SM_TIP, -1, CSetting::FLAG_COLD},
	// 16 for DPI<=96, 20 for DPI<=120, 24 otherwise
	{L"MediumIconSize", CSetting::TYPE_INT, IDS_MEDIUM_SIZE_SM,IDS_MEDIUM_SIZE_SM_TIP, -1, CSetting::FLAG_COLD},
	...
	{L"Random", CSetting::TYPE_RADIO,IDS_ANIM_RANDOM,IDS_ANIM_RANDOM_TIP},
	{
		L"SubMenuAnimationSpeed", CSetting::TYPE_INT,IDS_SUB_ANIM_SPEED,IDS_SUB_ANIM_SPEED_TIP, 200, 0,
		L"SubMenuAnimation"
	},
	{
		L"SubMenuAnimationAlways", CSetting::TYPE_BOOL,IDS_SUB_ANIM_ALWAYS,IDS_SUB_ANIM_ALWAYS_TIP, 0, 0,
		L"SubMenuAnimation"
	},
	{
	...
	{
		L"GlassIntensity", CSetting::TYPE_INT,IDS_GLASS_INTENSITY,IDS_GLASS_INTENSITY_TIP, 0, 0, L"GlassOverride",
		L"GlassOverride"
	},
	{
		L"GlassBlending", CSetting::TYPE_INT,IDS_GLASS_BLENDING,IDS_GLASS_BLENDING_TIP, 0, 0, L"GlassOverride",
		L"GlassOverride"
	},
	{
		L"GlassOpacity", CSetting::TYPE_INT,IDS_GLASS_OPACITY,IDS_GLASS_OPACITY_TIP, DEFAULT_GLASS_OPACITY, 0,
		L"EnableGlass"
	},

	{
		L"Skin", CSetting::TYPE_GROUP,IDS_SKIN_SETTINGS, 0, 0, CSetting::FLAG_BASIC | CSetting::FLAG_MENU_CLASSIC1,NULL,
		NULL, &g_SkinSettingsPanelClassic1
	},
	...
	{
		L"Skin", CSetting::TYPE_GROUP,IDS_SKIN_SETTINGS, 0, 0, CSetting::FLAG_BASIC | CSetting::FLAG_MENU_CLASSIC2,NULL,
		NULL, &g_SkinSettingsPanelClassic2
	},
	...
	{
		L"Skin7", CSetting::TYPE_GROUP,IDS_SKIN_SETTINGS, 0, 0, CSetting::FLAG_BASIC | CSetting::FLAG_MENU_WIN7,NULL,
		NULL, &g_SkinSettingsPanelWin7
	},
	...
	{
		L"StartButtonTip", CSetting::TYPE_STRING,IDS_BUTTON_TIP,IDS_BUTTON_TIP_TIP, L"$Menu.Start", 0,
		L"EnableStartButton"
	},
	...
	{
		L"StartButtonSize", CSetting::TYPE_INT,IDS_BUTTON_SIZE,IDS_BUTTON_SIZE_TIP2, 0, 0, L"#StartButtonType=2",
		L"CustomButton"
	},
	{
		L"StartButtonAlign", CSetting::TYPE_BOOL,IDS_BUTTON_ALIGN,IDS_BUTTON_ALIGN_TIP, 0, 0, L"#StartButtonType=2",
		L"CustomButton"
	},
	{
	...
	{
		L"StartButtonIconSize", CSetting::TYPE_INT,IDS_BUTTON_ICON_SIZE,IDS_BUTTON_ICON_SIZE_TIP, 0, 0,
		L"#StartButtonType=1", L"ClasicButton"
	},
	{
		L"StartButtonText", CSetting::TYPE_STRING,IDS_BUTTON_TEXT,IDS_BUTTON_TEXT_TIP, L"$Menu.Start", 0,
		L"#StartButtonType=1", L"ClasicButton"
	},
	{
		L"XPButton", CSetting::TYPE_BOOL,IDS_CLASSIC_BUTTON_XP,IDS_CLASSIC_BUTTON_TIP, 0, 0,
		L"#StartButtonType=1", L"ClasicButton"
	},
	...
	{
		L"TaskbarLook", CSetting::TYPE_INT,IDS_TASK_LOOK,IDS_TASK_LOOK_TIP, 1, CSetting::FLAG_CALLBACK,
		L"CustomTaskbar", L"CustomTaskbar"
	},
	...
	{
		L"TaskbarOpacity", CSetting::TYPE_INT,IDS_TASK_OPACITY,IDS_TASK_OPACITY_TIP, DEFAULT_TASK_OPACITY10,
		CSetting::FLAG_CALLBACK, L"TaskbarLook", L"CustomTaskbar"
	},
	{
		L"TaskbarColor", CSetting::TYPE_COLOR,IDS_TASK_COLOR,IDS_TASK_COLOR_TIP, 0, CSetting::FLAG_CALLBACK,
		L"CustomTaskbar", L"CustomTaskbar"
	},
	{
		L"TaskbarTextColor", CSetting::TYPE_COLOR,IDS_TASK_TEXTCOLOR,IDS_TASK_TEXTCOLOR_TIP, 0xFFFFFF,
		CSetting::FLAG_COLD | (1 << 24), L"CustomTaskbar", L"CustomTaskbar"
	},
	{
		L"TaskbarTexture", CSetting::TYPE_BITMAP_JPG,IDS_TASK_TEXTURE,IDS_TASK_TEXTURE_TIP, L"", CSetting::FLAG_COLD,
		L"CustomTaskbar", L"CustomTaskbar"
	},
	{
		L"TaskbarTileH", CSetting::TYPE_INT,IDS_TASK_STRETCHH,IDS_TASK_STRETCHH_TIP, 1, CSetting::FLAG_CALLBACK,
		L"#TaskbarTexture", L"TaskbarTexture"
	},
	...
	{
		L"TaskbarBordersH", CSetting::TYPE_STRING,IDS_TASK_BORDERS,IDS_TASK_BORDERS_TIP, L"0,0",
		CSetting::FLAG_CALLBACK, L"#TaskbarTileH=1", L"TaskbarTileH"
	},
	{
		L"TaskbarTileV", CSetting::TYPE_INT,IDS_TASK_STRETCHV,IDS_TASK_STRETCHV_TIP, 1, CSetting::FLAG_CALLBACK,
		L"#TaskbarTexture", L"TaskbarTexture"
	},
	{L"Tile", CSetting::TYPE_RADIO,IDS_TASK_TILE,IDS_TASK_TILE_TIP},
	{L"Stretch", CSetting::TYPE_RADIO,IDS_TASK_STRETCH,IDS_TASK_STRETCH_TIP},
	{
		L"TaskbarBordersV", CSetting::TYPE_STRING,IDS_TASK_BORDERS,IDS_TASK_BORDERS_TIP, L"0,0",
		CSetting::FLAG_CALLBACK, L"#TaskbarTileV=1", L"TaskbarTileV"
	},
	...
	{
		L"CustomMenu1", CSetting::TYPE_GROUP,IDS_CUSTOM_SETTINGS, 0, 0, CSetting::FLAG_MENU_CLASSIC1,NULL,NULL,
		&g_CustomMenuPanel1
	},
	{L"MenuItems1", CSetting::TYPE_MULTISTRING, 0, 0, L""},

	{
		L"CustomMenu2", CSetting::TYPE_GROUP,IDS_CUSTOM_SETTINGS, 0, 0, CSetting::FLAG_MENU_CLASSIC2,NULL,NULL,
		&g_CustomMenuPanel2
	},
	{L"MenuItems2", CSetting::TYPE_MULTISTRING, 0, 0, L""},

	{
		L"CustomMenu7", CSetting::TYPE_GROUP,IDS_CUSTOM_SETTINGS, 0, 0, CSetting::FLAG_BASIC | CSetting::FLAG_MENU_WIN7,
		NULL,NULL, &g_CustomMenuPanel7
	},
	...
	{L"AllProgramsDelay", CSetting::TYPE_INT,IDS_ALL_DELAY,IDS_ALL_DELAY_TIP, -1, 0, L"CascadeAll", L"CascadeAll"},
	// system hover time
	...
	{
		L"ExplorerPath", CSetting::TYPE_STRING,IDS_EXPLORER_PATH,IDS_EXPLORER_PATH_TIP, L"computer", 0,
		L"EnableExplorer", L"EnableExplorer"
	},
	...
	{
		L"Language", CSetting::TYPE_GROUP,IDS_LANGUAGE_SETTINGS_SM, 0, 0, 0,NULL,NULL,
		GetLanguageSettings(COMPONENT_MENU)
	},
...
void UpgradeSettings(bool bShared)
{
	...
	if (!pSettingStyle->IsLocked())
	{
		...
		SetSettingsStyle(bTwoColumns ? CSetting::FLAG_MENU_CLASSIC2 : CSetting::FLAG_MENU_CLASSIC1,
		                 CSetting::FLAG_MENU_MASK);
	}
...
static CString GetWindowsBrandingString()
{
	...
	if (retval.IsEmpty())
	{
		...
		if (CRegKey reg; reg.Open(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows NT\\CurrentVersion", KEY_READ) ==
			ERROR_SUCCESS)
		{
...
{
	{
		...
		if (regKey.Open(HKEY_LOCAL_MACHINE, L"Software\\OpenShell\\OpenShell",KEY_READ | KEY_WOW64_64KEY) ==
			ERROR_SUCCESS)
		{
	...
	if (GetWinVersion() >= WIN_VER_WIN8)
	{
		...
		UpdateSetting(L"CascadeAll", CComVariant(0), false);
		HideSetting(L"CascadeAll", true);
		HIGHCONTRAST contrast = {sizeof(contrast)};
		if (SystemParametersInfo(SPI_GETHIGHCONTRAST, sizeof(contrast), &contrast, 0) && (contrast.dwFlags &
			HCF_HIGHCONTRASTON))
		...
		if (GetWinVersion() >= WIN_VER_WIN81)
		{
			if (GetWinVersion() >= WIN_VER_WIN10)
			{
				...
				UpdateSetting(L"SkipMetro", CComVariant(0), false);
				HideSetting(L"SkipMetro", true);
				UpdateSetting(L"DisableHotCorner", CComVariant(0), false);
				HideSetting(L"DisableHotCorner", true);
			}
			else
			{
			...
			CRegKey regSkip;
			DWORD skip = 0;
			if (regSkip.Open(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\StartPage",
		...
		{
			UpdateSetting(L"SearchMetroSettings", CComVariant(0), false);
			HideSetting(L"SearchMetroSettings", true);
		}
	}
	else
	{
		...
		HideSetting(L"SkipMetro", true);
		UpdateSetting(L"EnableStartButton", CComVariant(0), false);
		UpdateSetting(L"AllTaskbars", CComVariant(0), false);
		HideSetting(L"AllTaskbars", true);
		UpdateSetting(L"AllProgramsMetro", CComVariant(0), false);
		HideSetting(L"AllProgramsMetro", true);
		UpdateSetting(L"HideProgramsMetro", CComVariant(0), false);
		HideSetting(L"HideProgramsMetro", true);
		UpdateSetting(L"ShowAppsMenu", CComVariant(0), false);
		HideSetting(L"ShowAppsMenu", true);
		UpdateSetting(L"InvertMetroIcons", CComVariant(0), false);
		HideSetting(L"InvertMetroIcons", true);
		UpdateSetting(L"AeroGlass", CComVariant(0), false);
		HideSetting(L"AeroGlass", true);
		UpdateSettingText(L"EnableStartButton",IDS_ENABLE_BUTTON2,IDS_ENABLE_BUTTON_TIP2, false);
		...
		UpdateSetting(L"RecentMetroApps", CComVariant(0), false);
		HideSetting(L"RecentMetroApps", true);
		UpdateSetting(L"SearchMetroApps", CComVariant(0), false);
		HideSetting(L"SearchMetroApps", true);
		UpdateSetting(L"SearchMetroSettings", CComVariant(0), false);
		HideSetting(L"SearchMetroSettings", true);
		UpdateSetting(L"HybridShutdown", CComVariant(0), false);
		HideSetting(L"HybridShutdown", true);
		UpdateSetting(L"EnableTouch", CComVariant(0), false);
		HideSetting(L"EnableTouch", true);
#ifndef STARTSCREEN_WIN7
		UpdateSetting(L"StartScreenShortcut", CComVariant(0), false);
		HideSetting(L"StartScreenShortcut", true);
#endif
		UpdateSetting(L"HighlightNewApps", CComVariant(0), false);
		HideSetting(L"HighlightNewApps", true);
	}
	if (GetWinVersion() < WIN_VER_WIN10)
	{
		int dr, dg, db, da, dc;
		...
		UpdateSetting(L"EnableGlass", CComVariant(1), false);
		HideSetting(L"EnableGlass", true);
		UpdateSetting(L"TaskbarOpacity",
		              CComVariant(GetWinVersion() <= WIN_VER_WIN7 ? DEFAULT_TASK_OPACITY7 : DEFAULT_TASK_OPACITY8),
		              false);
...
static void UpdateButtons(bool bForce)
{
	...
	bool bRecreate = (g_bButtonEnable != GetSettingBool(L"EnableStartButton") || path != g_ButtonPath || g_ButtonSize !=
		GetSettingInt(L"StartButtonSize")
		|| g_ButtonIcon != icon || g_ButtonIconSize != GetSettingInt(L"StartButtonIconSize") || g_ButtonText != text ||
		g_ButtonTip != tip);
...
{
	if (wcscmp(pSetting->name, L"CustomTaskbar") == 0 || wcscmp(pSetting->name, L"TaskbarLook") == 0 ||
		wcscmp(pSetting->name, L"TaskbarOpacity") == 0 || wcscmp(pSetting->name, L"TaskbarColor") == 0 || wcscmp(
			pSetting->name, L"TaskbarTextColor") == 0)
	{
	...
	else if (wcscmp(pSetting->name, L"TaskbarTexture") == 0 || wcscmp(pSetting->name, L"TaskbarTileH") == 0 ||
		wcscmp(pSetting->name, L"TaskbarTileV") == 0 || wcscmp(pSetting->name, L"TaskbarBordersH") == 0 || wcscmp(
			pSetting->name, L"TaskbarBordersV") == 0)
	{
		UpdateTaskBars(TASKBAR_UPDATE_TEXTURE);
		g_bTaskbarsChanged = true;
		g_bTaskbarsUpdated = true;
...
void EditSettings(bool bModal, int tab)
{
	...
	if (ver)
		Sprintf(title,_countof(title), LoadStringEx(IDS_SETTINGS_TITLE_VER), ver >> 24, (ver >> 16) & 0xFF,
		        ver & 0xFFFF);
	else
