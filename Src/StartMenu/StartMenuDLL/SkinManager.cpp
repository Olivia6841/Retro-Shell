...
typedef COLORREF (WINAPI *TGetImmersiveColorFromColorSetEx)(UINT dwImmersiveColorSet, UINT dwImmersiveColorType,
...
BOOL WINAPI SystemParametersInfoForDpi(_In_ UINT uiAction, _In_ UINT uiParam,
                                       _Pre_maybenull_ _Post_valid_ PVOID pvParam, _In_ UINT fWinIni, _In_ UINT dpi)
{
	static auto p = static_cast<decltype(&SystemParametersInfoForDpi)>((void*)GetProcAddress(
		GetModuleHandle(L"user32.dll"), "SystemParametersInfoForDpi"));
	if (p)
...
HFONT MenuSkin::LoadSkinFont(const wchar_t* str, const wchar_t* name, int weight, float size, bool bScale) const
{
	...
	return CreateFont((int)size, 0, 0, 0, weight, bItalic ? 1 : 0, 0, 0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
	                  CLIP_DEFAULT_PRECIS, quality,DEFAULT_PITCH, name);
}
...
static bool GetSkinColorizationParameters(DWMCOLORIZATIONPARAMS* params)
{
	...
	tGetColorizationParameters GetColorizationParameters = (tGetColorizationParameters)GetProcAddress(
		hMod,MAKEINTRESOURCEA(127));
...
COLORREF GetMetroTaskbarColor(bool& bTransparent)
{
	...
	{
		CRegKey regTheme;
		if (regTheme.Open(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize",
		                  KEY_READ) == ERROR_SUCCESS)
		{
			DWORD trans = 1;
			bTransparent = regTheme.QueryDWORDValue(L"EnableTransparency", trans) != ERROR_SUCCESS || trans;
		}
		int type = GetImmersiveColorTypeFromName(bTransparent
...
static void BlendColor(int& r, int& g, int& b, int a1, int a2, int a3, COLORREF tintColor1, COLORREF tintColor2,
                       COLORREF tintColor3)
{
	int tr1, tg1, tb1;
...
void MenuSkin::LoadSkinTintColors(CSkinParser& parser, const wchar_t* name, COLORREF& tintColor1, COLORREF& tintColor2,
...
bool MenuSkin::LoadSkinColors(CSkinParser& parser, const wchar_t* name, COLORREF* colors, int count,
                              COLORREF backgroundColor, TSkinNumberType type) const
{
...
MenuBitmap MenuSkin::LoadSkinBitmap(HMODULE hMod, int index, int maskIndex, COLORREF tintColor1, COLORREF tintColor2,
                                    COLORREF tintColor3, bool bPremultiply) const
{
	...
	if (index > 0)
	{
		...
		else if (hMod)
		{
			...
			if (!bmp)
			{
				...
				Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError), LoadStringEx(IDS_SKIN_ERR_BMPRES), index,
				        err);
				return res;
			}
		}
		else
		{
			...
			{
				GetErrorMessage(err,_countof(err), GetLastError());
				Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError), LoadStringEx(IDS_SKIN_ERR_BMPFILE),
				        fname, err);
				return res;
			}
		}
	...
	if (maskIndex > 0)
	{
		...
		else if (hMod)
		{
			...
			{
				GetErrorMessage(err,_countof(err), GetLastError());
				Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError), LoadStringEx(IDS_SKIN_ERR_MASKRES),
				        maskIndex, err);
			}
		}
		else
		{
			...
			{
				GetErrorMessage(err,_countof(err), GetLastError());
				Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError), LoadStringEx(IDS_SKIN_ERR_MASKFILE),
				        fname, err);
			}
		}
		if (!bmpMask)
	...
	{
		Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError), LoadStringEx(IDS_SKIN_ERR_MASKSIZE), index,
		        maskIndex);
	}
...
bool MenuSkin::LoadSkinBackground(HMODULE hMod, CSkinParser& parser, const wchar_t* name, MenuBitmap& bitmap,
                                  COLORREF backgroundColor, int* slicesX, int countX, int* slicesY, int countY,
                                  bool bMirror, bool bAllowColor, bool bPremultiply) const
{
	if (countX)
...
bool MenuSkin::LoadSkinBitmap(HMODULE hMod, CSkinParser& parser, const wchar_t* name, MenuBitmap& bitmap, SIZE* size,
                              bool bMirror, bool bPremultiply) const
{
	int cx, cy;
...
bool MenuSkin::LoadSkinItem(HMODULE hMod, CSkinParser& parser, const wchar_t* name,
                            ...
                            COLORREF backgroundColor, bool bRTL) const
{
	...
	else if (LoadSkinColors(parser, name2, settings.textShadowColors,_countof(settings.textShadowColors),
	                        backgroundColor))
	{
	}
	else if (pDefaults)
	...
	if (str)
	{
		if (!LoadSkinBackground(hMod, parser, name2, settings.bmpSelection, backgroundColor, settings.selSlicesX,
		                        _countof(settings.selSlicesX), settings.selSlicesY,_countof(settings.selSlicesY), bRTL,
		                        true))
			return false;
	}
	else if (pDefaults)
	...
	if (str)
	{
		if (!LoadSkinBackground(hMod, parser, name2, settings.bmpIconFrame, backgroundColor, settings.frameSlicesX,
		                        _countof(settings.frameSlicesX), settings.frameSlicesY,_countof(settings.frameSlicesY),
		                        bRTL))
			return false;
...
bool MenuSkin::ComputeOptionStates(const std::map<CString, CString>& options, std::vector<const wchar_t*>& values,
                                   bool bTranslateValues) const
{
	...
	if (ForceTouch || (GetWinVersion() >= WIN_VER_WIN8 && GetSettingBool(L"EnableTouch") && (
		GetSystemMetrics(SM_DIGITIZER) & NID_INTEGRATED_TOUCH) != 0))
	...
	for (std::vector<Option>::const_iterator it = Options.begin(); it != Options.end(); ++it)
	{
		const Option& option = *it;
		if (option.type == SKIN_OPTION_GROUP) continue;
		option.bEnabled = (option.condition.IsEmpty() || EvalCondition(option.condition,
			...
			if (bTranslateValues)
			{
				if (option.type == SKIN_OPTION_IMAGE)
				{
					...
					else
					{
						...
						{
							Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError),
							        LoadStringEx(IDS_SKIN_ERR_BMPFILE), fname, L"");
							return false;
						}
					}
				}
...
bool MenuSkin::LoadSkin(HMODULE hMod, const wchar_t* variation, const wchar_t* optionsStr, TSkinType skinType,
                        unsigned int flags, int dpi)
{
	...
	if (hMod)
	{
		HRSRC hResInfo = FindResource(hMod,MAKEINTRESOURCE((hMod==g_Instance && skinType==SKIN_TYPE_WIN7)?2:1),
		                              L"SKIN");
		if (!hResInfo)
		{
	...
	if (variation)
	{
		for (std::vector<std::pair<int, Variation>>::const_iterator it = Variations.begin(); it != Variations.end(); ++
		     it)
			if (_wcsicmp(variation, it->second.label) == 0 || _wcsicmp(variation, it->second.labelEn) == 0)
			{
				...
				if (hMod)
				{
					...
					{
						Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError),
						        LoadStringEx(IDS_SKIN_ERR_FIND_RES), it->first);
						break;
					}
					if (!parser.LoadVariation(hMod, hResInfo))
					{
						Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError),
						        LoadStringEx(IDS_SKIN_ERR_LOAD_RES), it->first);
						break;
					}
				}
				else
				{
					...
					{
						Sprintf(MenuSkin::s_SkinError,_countof(MenuSkin::s_SkinError),
						        LoadStringEx(IDS_SKIN_ERR_LOAD_FILE), path);
						break;
					}
				}
	...
	for (int lineIdx = 0;; lineIdx++)
	{
		...
		CString label;
		if (!parser.ParseOption(option.name, option.type, label, option.bDefValue, option.condition, option.disValue,
		                        lineIdx))
			break;
	...
	{
		if (Options[i].type == SKIN_OPTION_GROUP && (i + 1 >= Options.size() || Options[i].groupId != Options[i + 1].
			groupId))
	...
	if (skinType != SKIN_TYPE_ALL_PROGRAMS && (flags & LOADMENU_RESOURCES))
	{
		...
		if (!LoadSkinBackground(hMod, parser, L"Main_bitmap", Main_bitmap, Main_background,
		                        Main_bitmap_slices_X + (TwoColumns ? 3 : 0), 6, Main_bitmap_slices_Y, 6, false))
			return false;
		...
		if (!LoadSkinBackground(hMod, parser, L"Main_emblem_mask", Main_emblem_mask, 0,NULL, 0,NULL, 0, false, true,
		                        false))
			return false;
		...
		if (!LoadSkinBitmap(hMod, parser, L"Pattern4", Patterns[3], &Pattern_sizes[3], bRTL))
			return false;
		if (!LoadSkinBackground(hMod, parser, L"Main_pattern_mask", Main_pattern_mask, 0,NULL, 0,NULL, 0, false, true,
		                        false))
			return false;
		...
		if (!LoadSkinItem(hMod, parser, L"Main", ItemSettings[COLUMN1_ITEM],NULL, Main_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Main_split", ItemSettings[COLUMN1_SPLIT], &ItemSettings[COLUMN1_ITEM],
		                  Main_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Main_new", ItemSettings[COLUMN1_NEW], &ItemSettings[COLUMN1_ITEM],
		                  Main_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Main_separator", ItemSettings[COLUMN1_SEPARATOR], &ItemSettings[COLUMN1_ITEM],
		                  Main_background, bRTL))
			return false;
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Main_separator", ItemSettings[COLUMN1_SEPARATOR].bmpSeparator,
			                        Main_background, ItemSettings[COLUMN1_SEPARATOR].sepSlicesX,
			                        _countof(ItemSettings[COLUMN1_SEPARATOR].sepSlicesX),
			                        &ItemSettings[COLUMN1_SEPARATOR].sepHeight, 0, bRTL))
				return false;
		}
		...
		// second column
		if (!LoadSkinItem(hMod, parser, L"Main2", ItemSettings[COLUMN2_ITEM], &ItemSettings[COLUMN1_ITEM],
		                  Main_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Main2_split", ItemSettings[COLUMN2_SPLIT], &ItemSettings[COLUMN2_ITEM],
		                  Main_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Main2_new", ItemSettings[COLUMN2_NEW], &ItemSettings[COLUMN2_ITEM],
		                  Main_background, bRTL))
			return false;
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Main2_separator", ItemSettings[COLUMN2_SEPARATOR].bmpSeparator,
			                        Main_background, ItemSettings[COLUMN2_SEPARATOR].sepSlicesX,
			                        _countof(ItemSettings[COLUMN2_SEPARATOR].sepSlicesX),
			                        &ItemSettings[COLUMN2_SEPARATOR].sepHeight, 0, bRTL))
				return false;
		}
		else
		{
			...
			memcpy(ItemSettings[COLUMN2_SEPARATOR].sepSlicesX, ItemSettings[COLUMN1_SEPARATOR].sepSlicesX,
			       sizeof(ItemSettings[COLUMN2_SEPARATOR].sepSlicesX));
		}
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Main_separatorV", Main_separatorV, Main_background,
			                        &Main_separatorWidth, 0, Main_separator_slices_Y,_countof(Main_separator_slices_Y),
			                        false))
				return false;
		}

		if (!LoadSkinBackground(hMod, parser, L"Main_pager", Main_pager, Main_background, Main_pager_slices_X,
		                        _countof(Main_pager_slices_X), Main_pager_slices_Y,_countof(Main_pager_slices_Y), bRTL))
			return false;
		...
		if (str)
		{
			LoadSkinNumbers(str, (int*)&User_image_size, 1,
			                (skinType == SKIN_TYPE_WIN7) ? NUMBERS_OTHER : NUMBERS_SIZE);
		...
		if (skinType == SKIN_TYPE_WIN7)
		{
			...
			if (!LoadSkinBackground(hMod, parser, L"Main_bitmap_search", Main_bitmap_search, Main_background,
			                        Main_bitmap_search_slices_X + 3, 6, Main_bitmap_search_slices_Y, 6, false))
				return false;
			memset(Main_bitmap_jump_slices_X, 0, sizeof(Main_bitmap_jump_slices_X));
			if (!LoadSkinBackground(hMod, parser, L"Main_bitmap_jump", Main_bitmap_jump, Main_background,
			                        Main_bitmap_jump_slices_X + 3, 6, Main_bitmap_jump_slices_Y, 6, false))
				return false;

			if (!LoadSkinBackground(hMod, parser, L"Main_emblem_search_mask", Main_emblem_search_mask, 0,NULL, 0,NULL,
			                        0, false, true, false))
				return false;
			if (!LoadSkinBackground(hMod, parser, L"Main_emblem_jump_mask", Main_emblem_jump_mask, 0,NULL, 0,NULL, 0,
			                        false, true, false))
				return false;
			if (!LoadSkinBackground(hMod, parser, L"Main_pattern_search_mask", Main_pattern_search_mask, 0,NULL, 0,NULL,
			                        0, false, true, false))
				return false;
			if (!LoadSkinBackground(hMod, parser, L"Main_pattern_jump_mask", Main_pattern_jump_mask, 0,NULL, 0,NULL, 0,
			                        false, true, false))
				return false;

			// Shutdown button
			if (!LoadSkinItem(hMod, parser, L"Shutdown", ItemSettings[SHUTDOWN_BUTTON], &ItemSettings[COLUMN1_ITEM],
			                  Main_background, bRTL))
				return false;
			str = parser.FindSetting(L"Shutdown_text_color");
			if (!str)
				ItemSettings[SHUTDOWN_BUTTON].textColors[0] = ItemSettings[SHUTDOWN_BUTTON].textColors[1] = GetSysColor(
					COLOR_BTNTEXT);
			str = parser.FindSetting(L"Shutdown_arrow_color");
			if (!str)
				ItemSettings[SHUTDOWN_BUTTON].arrColors[0] = ItemSettings[SHUTDOWN_BUTTON].arrColors[1] = GetSysColor(
					COLOR_BTNTEXT);
			if (!LoadSkinItem(hMod, parser, L"Shutdown_search", ItemSettings[SHUTDOWN_BUTTON_SEARCH],
			                  &ItemSettings[SHUTDOWN_BUTTON], Main_background, bRTL))
				return false;
			memcpy(ItemSettings[SHUTDOWN_BUTTON_SEARCH].selSlicesX, ItemSettings[SHUTDOWN_BUTTON].selSlicesX,
			       sizeof(ItemSettings[SHUTDOWN_BUTTON_SEARCH].selSlicesX));
			memcpy(ItemSettings[SHUTDOWN_BUTTON_SEARCH].selSlicesY, ItemSettings[SHUTDOWN_BUTTON].selSlicesY,
			       sizeof(ItemSettings[SHUTDOWN_BUTTON_SEARCH].selSlicesY));
			if (!LoadSkinItem(hMod, parser, L"Shutdown_jump", ItemSettings[SHUTDOWN_BUTTON_JUMP],
			                  &ItemSettings[SHUTDOWN_BUTTON], Main_background, bRTL))
				return false;
			memcpy(ItemSettings[SHUTDOWN_BUTTON_JUMP].selSlicesX, ItemSettings[SHUTDOWN_BUTTON].selSlicesX,
			       sizeof(ItemSettings[SHUTDOWN_BUTTON_JUMP].selSlicesX));
			memcpy(ItemSettings[SHUTDOWN_BUTTON_JUMP].selSlicesY, ItemSettings[SHUTDOWN_BUTTON].selSlicesY,
			       sizeof(ItemSettings[SHUTDOWN_BUTTON_JUMP].selSlicesY));
			...
			// List items
			if (!LoadSkinItem(hMod, parser, L"List", ItemSettings[LIST_ITEM], &ItemSettings[COLUMN1_ITEM],
			                  Main_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"List_split", ItemSettings[LIST_SPLIT], &ItemSettings[LIST_ITEM],
			                  Main_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"List_separator", ItemSettings[LIST_SEPARATOR], &ItemSettings[LIST_ITEM],
			                  Main_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"List_separator_split", ItemSettings[LIST_SEPARATOR_SPLIT],
			                  &ItemSettings[LIST_SPLIT], Main_background, bRTL))
				return false;
			ItemSettings[LIST_SEPARATOR].sepHeight = ScaleSkinElement(DEFAULT_SEPARATOR_HEIGHT);
			str = parser.FindSetting(L"List_separator");
			if (str)
			{
				if (!LoadSkinBackground(hMod, parser, L"List_separator", ItemSettings[LIST_SEPARATOR].bmpSeparator,
				                        Main_background, ItemSettings[LIST_SEPARATOR].sepSlicesX,
				                        _countof(ItemSettings[LIST_SEPARATOR].sepSlicesX),
				                        &ItemSettings[LIST_SEPARATOR].sepHeight, 0, bRTL))
					return false;
			}
			else
			{
				...
				memcpy(ItemSettings[LIST_SEPARATOR].sepSlicesX, ItemSettings[COLUMN1_SEPARATOR].sepSlicesX,
				       sizeof(ItemSettings[COLUMN1_SEPARATOR].sepSlicesX));
			}
			str = parser.FindSetting(L"List_separator_split");
			if (str)
			{
				if (!LoadSkinBackground(hMod, parser, L"List_separator_split",
				                        ItemSettings[LIST_SEPARATOR_SPLIT].bmpSeparator, Main_background,
				                        ItemSettings[LIST_SEPARATOR_SPLIT].sepSlicesX,
				                        _countof(ItemSettings[LIST_SEPARATOR_SPLIT].sepSlicesX),
				                        &ItemSettings[LIST_SEPARATOR_SPLIT].sepHeight, 0, bRTL))
					return false;
			}
			else
			{
				...
				memcpy(ItemSettings[LIST_SEPARATOR_SPLIT].sepSlicesX, ItemSettings[COLUMN1_SEPARATOR].sepSlicesX,
				       sizeof(ItemSettings[COLUMN1_SEPARATOR].sepSlicesX));
			}
			...
			{
				if (!LoadSkinColors(parser, L"Programs_background", &Programs_background, 1, Main_background,
				                    NUMBERS_COLORS_ABGR))
			...
			if (!LoadSkinItem(hMod, parser, L"Programs_button", ItemSettings[PROGRAMS_BUTTON],
			                  &ItemSettings[COLUMN1_ITEM], Programs_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"Programs_button_new", ItemSettings[PROGRAMS_BUTTON_NEW],
			                  &ItemSettings[COLUMN1_NEW], Programs_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"Programs_cascading", ItemSettings[PROGRAMS_CASCADING],
			                  &ItemSettings[PROGRAMS_BUTTON], Programs_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"Programs_cascading_new", ItemSettings[PROGRAMS_CASCADING_NEW],
			                  &ItemSettings[PROGRAMS_BUTTON_NEW], Programs_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"Programs", ItemSettings[PROGRAMS_TREE_ITEM], &ItemSettings[COLUMN1_ITEM],
			                  Programs_background, bRTL))
				return false;
			if (!LoadSkinItem(hMod, parser, L"Programs_new", ItemSettings[PROGRAMS_TREE_NEW],
			                  &ItemSettings[PROGRAMS_TREE_ITEM], Programs_background, bRTL))
				return false;
			...
			if (Programs_icon.GetBitmap())
			{
				...
				if (!LoadSkinBitmap(hMod, parser, L"Programs_icon_selected", Programs_icon_selected, &selSize, bRTL))
					return false;
				if (!Programs_icon_selected.GetBitmap() || selSize.cx != Programs_icon_size.cx || selSize.cy !=
					Programs_icon_size.cy)
				{
					Programs_icon_selected.Reset();
			...
			if (parser.FindSetting(L"Scrollbar_button") && parser.FindSetting(L"Scrollbar_thumb") && parser.
				FindSetting(L"Scrollbar_background") && parser.FindSetting(L"Scrollbar_arrows"))
			{
				BHasScrollbar = true;
				if (!LoadSkinBackground(hMod, parser, L"Scrollbar_button", Scrollbar_button, Programs_background,
				                        Scrollbar_button_slices_X,_countof(Scrollbar_button_slices_X),
				                        Scrollbar_button_slices_Y,_countof(Scrollbar_button_slices_Y), bRTL, true))
					return false;
				if (!LoadSkinBackground(hMod, parser, L"Scrollbar_thumb", Scrollbar_thumb, Programs_background,
				                        Scrollbar_thumb_slices_X,_countof(Scrollbar_thumb_slices_X),
				                        Scrollbar_thumb_slices_Y,_countof(Scrollbar_thumb_slices_Y), bRTL, true))
					return false;
				if (!LoadSkinBackground(hMod, parser, L"Scrollbar_background", Scrollbar_background,
				                        Programs_background, Scrollbar_background_slices_X,
				                        _countof(Scrollbar_background_slices_X), Scrollbar_background_slices_Y,
				                        _countof(Scrollbar_background_slices_Y), bRTL, true))
					return false;
				...
				{
					if (!LoadSkinBitmap(hMod, parser, L"Scrollbar_gripper", Scrollbar_gripper, &Scrollbar_gripper_size,
					                    bRTL))
						return false;
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Search_background", Search_background, searchColor,
			                        Search_background_slices_X,_countof(Search_background_slices_X),
			                        Search_background_slices_Y,_countof(Search_background_slices_Y), false, true))
				return false;
		}
		...
		if (!LoadSkinBackground(hMod, parser, L"Search_emblem_mask", Search_emblem_mask, 0,NULL, 0,NULL, 0, false, true,
		                        false))
			return false;
		if (!LoadSkinBackground(hMod, parser, L"Search_pattern_mask", Search_pattern_mask, 0,NULL, 0,NULL, 0, false,
		                        true, false))
			return false;
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Search_background_search", Search_background_search, searchColor,
			                        Search_background_search_slices_X,_countof(Search_background_search_slices_X),
			                        Search_background_search_slices_Y,_countof(Search_background_search_slices_Y),
			                        false, true))
				return false;
		}
		else
		{
			...
			memcpy(Search_background_search_slices_X, Search_background_slices_X,
			       sizeof(Search_background_search_slices_X));
			memcpy(Search_background_search_slices_Y, Search_background_slices_Y,
			       sizeof(Search_background_search_slices_Y));
		}
		...
		if (str)
			...
			memcpy(&Search_background_search_padding, &Search_background_padding,
			       sizeof(Search_background_search_padding));

		str = parser.FindSetting(L"Search_emblem_search_mask");
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Search_emblem_search_mask", Search_emblem_search_mask, 0,NULL, 0,
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Search_pattern_search_mask", Search_pattern_search_mask, 0,NULL, 0,
			                        NULL, 0, false, true, false))
				return false;
		}
		else
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Search_background_jump", Search_background_jump, searchColor,
			                        Search_background_jump_slices_X,_countof(Search_background_jump_slices_X),
			                        Search_background_jump_slices_Y,_countof(Search_background_jump_slices_Y), false,
			                        true))
				return false;
		}
		else
		{
			...
			memcpy(Search_background_jump_slices_X, Search_background_slices_X,
			       sizeof(Search_background_jump_slices_X));
			memcpy(Search_background_jump_slices_Y, Search_background_slices_Y,
			       sizeof(Search_background_jump_slices_Y));
		}
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Search_emblem_jump_mask", Search_emblem_jump_mask, 0,NULL, 0,NULL,
			                        0, false, true, false))
				return false;
		}
		else
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Search_pattern_jump_mask", Search_pattern_jump_mask, 0,NULL, 0,NULL,
			                        0, false, true, false))
				return false;
		}
		else
		...
		else
		{
			...
			if (hMod)
			{
				HICON hIcon = (HICON)LoadImage(hMod,MAKEINTRESOURCE(5101),IMAGE_ICON, iconSize, iconSize,
				                               LR_DEFAULTCOLOR);
		...
		else
		{
			...
			if (hMod)
			{
				HICON hIcon = (HICON)LoadImage(hMod,MAKEINTRESOURCE(323),IMAGE_ICON, iconSize, iconSize,
				                               LR_DEFAULTCOLOR);
				DrawIconEx(hdc, 0, 0, hIcon, iconSize, iconSize, 0,NULL,DI_NORMAL);
				DrawIconEx(hdc, 0, iconSize, hIcon, iconSize, iconSize, 0,NULL,DI_NORMAL);
				DestroyIcon(hIcon);
		...
		if (skinType == SKIN_TYPE_WIN7)
		{
			...
			else
			{
				...
				if (hMod)
				{
					...
					HICON hIcon = (HICON)LoadImage(hMod,MAKEINTRESOURCE(47),IMAGE_ICON, iconSize, iconSize,
					                               LR_DEFAULTCOLOR);
					if (hIcon)
					{
	...
	if (flags & LOADMENU_RESOURCES)
	{
		...
		if (!LoadSkinBackground(hMod, parser, L"Submenu_bitmap", Submenu_bitmap, Submenu_background,
		                        Submenu_bitmap_slices_X + 3, 6, Submenu_bitmap_slices_Y,
		                        _countof(Submenu_bitmap_slices_Y), bRTL))
			return false;
		...
		if (!LoadSkinItem(hMod, parser, L"Submenu", ItemSettings[SUBMENU_ITEM],NULL, Submenu_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Submenu_split", ItemSettings[SUBMENU_SPLIT], &ItemSettings[SUBMENU_ITEM],
		                  Submenu_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Submenu_new", ItemSettings[SUBMENU_NEW], &ItemSettings[SUBMENU_ITEM],
		                  Submenu_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Submenu_separator", ItemSettings[SUBMENU_SEPARATOR],
		                  &ItemSettings[SUBMENU_ITEM], Submenu_background, bRTL))
			return false;
		if (!LoadSkinItem(hMod, parser, L"Submenu_separator_split", ItemSettings[SUBMENU_SEPARATOR_SPLIT],
		                  &ItemSettings[SUBMENU_SPLIT], Submenu_background, bRTL))
			return false;
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Submenu_separator", ItemSettings[SUBMENU_SEPARATOR].bmpSeparator,
			                        Main_background, ItemSettings[SUBMENU_SEPARATOR].sepSlicesX,
			                        _countof(ItemSettings[SUBMENU_SEPARATOR].sepSlicesX),
			                        &ItemSettings[SUBMENU_SEPARATOR].sepHeight, 0, bRTL))
				return false;
		}
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Submenu_separator_split",
			                        ItemSettings[SUBMENU_SEPARATOR_SPLIT].bmpSeparator, Main_background,
			                        ItemSettings[SUBMENU_SEPARATOR_SPLIT].sepSlicesX,
			                        _countof(ItemSettings[SUBMENU_SEPARATOR_SPLIT].sepSlicesX),
			                        &ItemSettings[SUBMENU_SEPARATOR_SPLIT].sepHeight, 0, bRTL))
				return false;
		}
		else
		{
			...
			memcpy(ItemSettings[SUBMENU_SEPARATOR_SPLIT].sepSlicesX, ItemSettings[SUBMENU_SEPARATOR].sepSlicesX,
			       sizeof(ItemSettings[SUBMENU_SEPARATOR].sepSlicesX));
		}
		...
		if (str)
		{
			if (!LoadSkinBackground(hMod, parser, L"Submenu_separatorV", Submenu_separatorV, Submenu_background,
			                        &Submenu_separatorWidth, 0, Submenu_separator_slices_Y,
			                        _countof(Submenu_separator_slices_Y), bRTL))
				return false;
		}
		if (!Submenu_separatorV.GetBitmap())
			Submenu_separatorWidth = ScaleSkinElement(DEFAULT_SEPARATOR_WIDTH);
		if (!LoadSkinBackground(hMod, parser, L"Submenu_pager", Submenu_pager, Submenu_background,
		                        Submenu_pager_slices_X,_countof(Submenu_pager_slices_X), Submenu_pager_slices_Y,
		                        _countof(Submenu_pager_slices_Y), bRTL))
			return false;

		if (!LoadSkinBitmap(hMod, parser, L"Submenu_pager_arrows", Submenu_pager_arrows, &Submenu_pager_arrow_Size,
		                    bRTL))
			return false;
	...
	if ((winVer >= WIN_VER_WIN8 && winVer < WIN_VER_WIN10 && !GetSettingBool(L"AeroGlass")) || (winVer >= WIN_VER_WIN10
		&& !GetSettingBool(L"EnableGlass")))
	{
	...
	for (int i = 0; i < _countof(ItemSettings); i++)
	{
		...
		case PROGRAMS_TREE_NEW:
			settings.iconSize = ICON_SIZE_SMALL;
			settings.opacity = (((Main_opacity == OPACITY_FULLALPHA || Main_opacity == OPACITY_FULLGLASS) &&
				                   Programs_background < 0xFF000000) || Programs_background == 0)
				                   ? Main_opacity
				                   : OPACITY_SOLID;
			break;
		case LIST_ITEM:
		...
		case LIST_SEPARATOR_SPLIT:
			settings.iconSize = ICON_SIZE_SMALL;
			settings.opacity = (Search_opacity == OPACITY_FULLALPHA || Search_opacity == OPACITY_FULLGLASS)
				                   ? Search_opacity
				                   : OPACITY_SOLID;
			break;
		...
		case PROGRAMS_CASCADING_NEW:
			...
			if (Programs_icon.GetBitmap())
			{
				int dx = (Main_icon_size == ICON_SIZE_LARGE
					          ? g_ItemManager.LARGE_ICON_SIZE
					          : g_ItemManager.SMALL_ICON_SIZE) - Programs_icon_size.cx;
				if (dx > 0)
		...
		int textHeight = settings.font
			                 ? settings.textMetrics.tmHeight + settings.textPadding.top + settings.textPadding.bottom
			                 : 0;
		if (i == COLUMN2_INLINE)
			textHeight = 0;
		int iconHeight = 0;
	...
	{
		LOGFONT lfont;
		GetObject(ItemSettings[(skinType == SKIN_TYPE_WIN7) ? LIST_SEPARATOR : SUBMENU_SEPARATOR].font, sizeof(lfont),
		          &lfont);
...
bool MenuSkin::LoadMenuSkin(const wchar_t* fname, const wchar_t* variation, const wchar_t* optionsStr,
                            TSkinType skinType, unsigned int flags, int dpi)
{
	s_SkinError[0] = 0;
	wchar_t path[_MAX_PATH];
	GetSkinsPath(path);
	...
	if (!bTheme || FAILED(DwmIsCompositionEnabled(&bComp)))
		bComp = FALSE;
	BOOL bEnableTouch = (GetWinVersion() >= WIN_VER_WIN8 && GetSettingBool(L"EnableTouch") && (
		GetSystemMetrics(SM_DIGITIZER) & NID_INTEGRATED_TOUCH) != 0);
	...
	else
	{
		...
		if (Hash == skinHash && MetroColorHash == metroColorHash && dpi == Dpi)
		{
			...
			for (std::vector<CustomBitmap>::const_iterator it = CustomBitmaps.begin(); it != CustomBitmaps.end(); ++it)
			{
				if (!GetFileAttributesEx(it->path, GetFileExInfoStandard, &attributes) || CompareFileTime(
					&attributes.ftLastWriteTime, &it->timestamp) != 0)
				{
					bDifferent = true;
					break;
				}
...
static HRESULT STDAPICALLTYPE DrawThemeBackgroundSkin(HTHEME hTheme, HDC hdc, int iPartId, int iStateId, LPCRECT pRect,
                                                      LPCRECT pClipRect)
{
...
void InitializeSkinManager(bool bInitIat)
{
	...
	if (hComCtl32)
		g_DrawThemeBackgroundHook = SetIatHook(hComCtl32, "uxtheme.dll", "DrawThemeBackground",
		                                       DrawThemeBackgroundSkin);
#ifdef SIMULATE_METRO_COLORS
	char path[_MAX_PATH];
	...
	*PathFindFileNameA(path)=0;
	...
	{
		GetImmersiveUserColorSetPreference = (TGetImmersiveUserColorSetPreference)GetProcAddress(
			hUxTheme,MAKEINTRESOURCEA(98));
		GetImmersiveColorFromColorSetEx = (TGetImmersiveColorFromColorSetEx)GetProcAddress(
			hUxTheme,MAKEINTRESOURCEA(95));
