...
static HRESULT CreateLink(const wchar_t* source, LPITEMIDLIST sourcePidl, const wchar_t* dir, const wchar_t* fname)
{
	...
	HANDLE h = CreateFile(fname,GENERIC_WRITE,FILE_SHARE_READ | FILE_SHARE_WRITE,NULL,OPEN_EXISTING,
	...
	if (pData && SUCCEEDED(pData->GetData(&fmt1,&stg)))
	{
		...
		if (hDrop)
		{
			if (DragQueryFile(hDrop, 0xFFFFFFFF,NULL, 0) == 1)
			{
				...
				m_bInPinFolder1 = (fname - m_FileName == Strlen(m_PinFolder1) && _wcsnicmp(
					m_PinFolder1, m_FileName, (fname - m_FileName)) == 0);
				m_bInPinFolder2 = (m_PinFolder2[0] && fname - m_FileName == Strlen(m_PinFolder2) && _wcsnicmp(
					m_PinFolder2, m_FileName, (fname - m_FileName)) == 0);
	...
	else if (pData && SUCCEEDED(pData->GetData(&fmt2,&stg)))
	{
		...
		if (pIda)
		{
			if (pIda->cidl == 1)
			{
				...
				if (SUCCEEDED(SHGetKnownFolderIDList(FOLDERID_AppsFolder2,0,NULL,&appsFolder)) && ILIsEqual(
					folderPidl, appsFolder))
...
STDMETHODIMP CStartMenuExt::QueryContextMenu(HMENU menu, UINT uMenuIndex, UINT uidFirstCmd, UINT uidLastCmd,
                                             UINT uFlags)
{
	...
	if (bDirectory || m_FilePidl || _wcsicmp(ext, L".exe") == 0 || _wcsicmp(ext, L".lnk") == 0)
	{
		if (m_bInPinFolder1)
		{
			...
			InsertMenu(menu, uMenuIndex,MF_BYPOSITION, uidFirstCmd,
			           FindTranslation(L"Menu.UnpinStartCs", L"Unpin from Start menu (Retro-Shell)"));
		}
		else
		{
			if (bFakeDirectory)
		...
		if (GetFileVersionInfoEx(FILE_VER_GET_LOCALISED | FILE_VER_GET_NEUTRAL, fname, 0, size, &buf[0]))
		{
			...
			if (VerQueryValue(&buf[0], L"\\VarFileInfo\\Translation", (void**)&langs, &count))
			{
				...
				for (DWORD i = 0; i < count; i++)
				{
					...
					{
						wchar_t key[100];
						Sprintf(key,_countof(key), L"\\StringFileInfo\\%04X%04X\\FileDescription",LOWORD(langs[i]),
						        1200);
						const wchar_t* text = NULL;
						UINT q;
						if (VerQueryValue(&buf[0], key, (void**)&text, &q))
...
STDMETHODIMP CStartMenuExt::InvokeCommand(CMINVOKECOMMANDINFO* pInfo)
{
	if (pInfo->lpVerb == NULL || (!IS_INTRESOURCE(pInfo->lpVerb) && strcmp(pInfo->lpVerb,PIN_VERB) == 0))
	{
		...
		else if (!m_bInPinFolder2)
		{
			...
			int locIndex;
			bool bHasLocName = (*m_FileName && SHGetLocalizedName(m_FileName, locName,_countof(locName), &locIndex) ==
				S_OK);
