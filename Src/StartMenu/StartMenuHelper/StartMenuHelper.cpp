...
static void AddShellExt(const wchar_t* progID, const LPSECURITY_ATTRIBUTES sa)
{
	...
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT, CString(progID) + L"\\ShellEx\\ContextMenuHandlers\\" SHELLEXT_NAME, NULL,
	                   NULL, REG_OPTION_BACKUP_RESTORE, KEY_WRITE, sa, &hkey, NULL) == ERROR_SUCCESS)
	{
	...
	if (SetEntriesInAcl(2, ea,NULL, &pACL) == ERROR_SUCCESS)
	{
		...
		if (InitializeSecurityDescriptor(psd,SECURITY_DESCRIPTOR_REVISION))
		{
			if (SetSecurityDescriptorDacl(psd,TRUE, pACL,FALSE))
			{
				...
				HKEY hkey = NULL;
				if (RegCreateKeyEx(HKEY_CLASSES_ROOT, L"CLSID\\" EXPLORER_CLSID L"\\" EMULATION_KEY,NULL,NULL,
				                   REG_OPTION_BACKUP_RESTORE,KEY_WRITE, &sa, &hkey,NULL) == ERROR_SUCCESS)
				{
...
{
	HKEY hkey = NULL;
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT, CString(progID) + L"\\ShellEx\\ContextMenuHandlers", NULL, NULL,
	                   REG_OPTION_BACKUP_RESTORE, KEY_WRITE | DELETE, NULL, &hkey, NULL) == ERROR_SUCCESS)
	{
...
static void RemoveRegistryKeys(bool bPin)
{
	AdjustPrivileges();
	HKEY hkey = NULL;
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT, L"CLSID\\" EXPLORER_CLSID,NULL,NULL,REG_OPTION_BACKUP_RESTORE,
	                   KEY_WRITE | DELETE,NULL, &hkey,NULL) == ERROR_SUCCESS)
	{
...
static void InstallUpgradeTask(bool bInstall)
{
	...
	{
		CRegKey regKey;
		if (regKey.Open(HKEY_LOCAL_MACHINE, L"Software\\OpenShell\\OpenShell",
