...
public:
	CUpdateDlg(void)
	{
		m_Version = GetVersionEx(g_Instance);
		m_Data.Clear();
		m_Data.newVersion = 0xFFFFFFFF;
		m_Font = NULL;
	}
...
LRESULT CUpdateDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	HICON icon = (HICON)LoadImage(g_Instance,MAKEINTRESOURCE(IDI_APPICON),IMAGE_ICON, GetSystemMetrics(SM_CXICON),
	                              GetSystemMetrics(SM_CYICON),LR_DEFAULTCOLOR);
	SendMessage(WM_SETICON,ICON_BIG, (LPARAM)icon);
	icon = (HICON)LoadImage(g_Instance,MAKEINTRESOURCE(IDI_APPICON),IMAGE_ICON, GetSystemMetrics(SM_CXSMICON),
	...
	::ReleaseDC(NULL, hdc);
	m_Font = CreateFont(-MulDiv(9, dpi, 72), 0, 0, 0,FW_NORMAL, 0, 0, 0,DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,
	...
	GetDlgItem(IDC_BUTTONCHECKNOW).EnableWindow(
		!(g_Settings[SETTING_UPDATE].flags & CSetting::FLAG_LOCKED_MASK) || check);
	...
	GetDlgItem(IDC_CHECKNIGHTLY).EnableWindow(
		!(g_Settings[SETTING_NIGHTLY].flags & CSetting::FLAG_LOCKED_MASK) && check);
...
LRESULT CUpdateDlg::OnDownload(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	if (m_Data.bNewVersion)
	{
		...
		{
			CString fname, error;
			DWORD res = DownloadNewVersion(m_hWnd, COMPONENT_UPDATE, m_Data.downloadUrl, m_Data.downloadSigner, fname,
			                               error);
			if (res == 2)
				return 0;
			if (res)
			...
			else
			{
				...
				TASKDIALOGCONFIG task = {
					sizeof(task), m_hWnd,NULL,
					TDF_ENABLE_HYPERLINKS | TDF_ALLOW_DIALOG_CANCELLATION | TDF_USE_HICON_MAIN, TDCBF_OK_BUTTON
				};
...
void CUpdateDlg::UpdateUI(void)
{
	if (m_Data.bValid)
	{
		if (m_Data.bNewVersion)
		{
			...
			TOOLINFO tool = {
				sizeof(tool),TTF_SUBCLASS | TTF_IDISHWND, m_hWnd, (UINT_PTR)GetDlgItem(IDC_BUTTONDOWNLOAD).m_hWnd
			};
...
LRESULT CALLBACK SubclassBalloonProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass,
                                     DWORD_PTR dwRefData)
{
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpstrCmdLine, int nCmdShow)
{
	...
	if (wcsstr(lpstrCmdLine, L"-popup") != NULL)
	{
		...
		{
			HWND balloon = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | (IsLanguageRTL() ? WS_EX_LAYOUTRTL : 0),
			                              TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_CLOSE | TTS_NOPREFIX, 0, 0, 0, 0, NULL,
			                              NULL, g_Instance, NULL);
			SendMessage(balloon, TTM_SETMAXTIPWIDTH, 0, 500);
			TOOLINFO tool = {
				sizeof(tool),TTF_ABSOLUTE | TTF_TRANSPARENT | TTF_TRACK | (IsLanguageRTL() ? TTF_RTLREADING : 0U)
			};
			tool.uId = 1;
			tool.lpszText = (wchar_t*)(const wchar_t*)message;
			SendMessage(balloon, TTM_ADDTOOL, 0, (LPARAM)&tool);
			SendMessage(balloon, TTM_SETTITLE, (WPARAM)LoadIcon(g_Instance, MAKEINTRESOURCE(IDI_APPICON)),
			            (LPARAM)(const wchar_t*)title);
			...
			if (appbar.uEdge == ABE_LEFT)
				...
				pos = MAKELONG(IsLanguageRTL() ? info.rcWork.left : info.rcWork.right - rc.right + rc.left,
				               info.rcWork.top);
			else
				pos = MAKELONG(IsLanguageRTL() ? info.rcWork.left : info.rcWork.right - rc.right + rc.left,
				               info.rcWork.bottom - rc.bottom + rc.top);
			SendMessage(balloon, TTM_TRACKPOSITION, 0, pos);
			SetWindowSubclass(balloon, SubclassBalloonProc, 0, 'CLSH');
			PlaySound(L"SystemNotification", NULL,
			          SND_APPLICATION | SND_ALIAS | SND_ASYNC | SND_NODEFAULT | SND_SYSTEM);
			int time0 = timeGetTime();
